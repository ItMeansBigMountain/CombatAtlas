...
class TweetListener(tweepy.Stream):
    def __init__(self,   file_name=None , limit=10 ,  cli_display=False ):
        # INDEXING
        self.tweet_count = 0
    ...
    def on_status(self, status):
        ...
        if self.cli_display:
            ...
            print(f"Tweet Sentiment: {TextBlob(tweet_text).sentiment}\n")
...
def main():
    ...
    if search[-1] == '0' and len(search) == 1:
        ...
        # INIT stream
        STREAM = TweetListener(cli_display=True ) 
        STREAM.sample(threaded=True)
    ...
    # Filtered realtime Tweets
    else:
        # INIT filtered stream
        STREAM = TweetListener( "Stream_Output_File" , cli_display=False ) 
        STREAM.filter(track=search , threaded=True )
        
        # # disconnect after count limit
        import time
