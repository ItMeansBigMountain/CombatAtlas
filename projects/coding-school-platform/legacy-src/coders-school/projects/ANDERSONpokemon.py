...
# GLITCH DIARIES
# when player dies, they are able to move onto next round. then after you make a decision
    ...
    if gymLeaders_currentPokemon.health <= 0:
        ...
        for x in range( 0 , len(gymLeaders_CAUGHTPokemon) , 1):
          if gymLeaders_CAUGHTPokemon[x].health > 0 :
            GYMhealthyPokemon += 1
            print('{} : {}'.format(x , gymLeaders_CAUGHTPokemon[x].name ) )
        if GYMhealthyPokemon > 0:
          try:
            gymLeaders_currentPokemon = gymLeaders_CAUGHTPokemon[randy(   0  ,   len(gymLeaders_CAUGHTPokemon)-1 )  ]
