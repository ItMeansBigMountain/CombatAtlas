...
export default function App() {
  ...
  return <SafeAreaView style={styles.safe}>
    ...
    <ScrollView contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled">
      ...
      {drill ? <DrillDetail drill={drill} onBack={() => setDrill(null)} /> : selectedArt ?
        <ArtDetail art={selectedArt} compact={compact} onBack={() => setArtId(null)} onDrill={setDrill} /> :
        <Home arts={arts} matchingDrills={matchingDrills} compact={compact} hasQuery={Boolean(query.trim())} onArt={chooseArt} onDrill={setDrill} />}
...
function Home({ arts, matchingDrills, compact, hasQuery, onArt, onDrill }) {
  return <View>
    {matchingDrills.length > 0 && <><SectionTitle title="Matching drills" subtitle={`${matchingDrills.length} shown`} /><CardGrid compact={compact}>{matchingDrills.map((item) => <DrillCard key={item.id} drill={item} onPress={() => onDrill(item)} />)}</CardGrid></>}
    ...
    <CardGrid compact={compact}>{arts.map((art) => <ArtCard key={art.id} art={art} onPress={() => onArt(art.id)} />)}</CardGrid>
