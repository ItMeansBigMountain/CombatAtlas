...
- Use Wise Old Man/TempleOSRS-style gained summaries as the model for progress detail UX: top gained skills, boss KC, minigame/activity score, selected period, and source/last-updated labels.
- For Who's Grinding Panel specifically, prefer compact inline expand/collapse rows over a separate bottom card: click a player row once to show only the grinding summary below it, click again to collapse. Keep detail text readable (around 12pt), remove source/profile URL/data-links clutter when the user wants grinding-only display, minimize left padding, and avoid right-side row buttons that cause default-sidebar trailing.
