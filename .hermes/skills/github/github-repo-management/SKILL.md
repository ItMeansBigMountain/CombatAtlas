...
- If a parent/control-repo push is blocked by unrelated unpublished commits (for example, a generated file over GitHub's size limit), do not rewrite, discard, or force-push that unrelated history merely to publish submodule pointers. Clone remote `main` into a clean temporary checkout, use `git update-index --add --cacheinfo 160000,<verified-child-sha>,<submodule-path>` for only the intended gitlinks, commit there, and push normally. Verify the remote parent SHA afterward. This preserves the dirty/local history while publishing a narrow pointer update.
