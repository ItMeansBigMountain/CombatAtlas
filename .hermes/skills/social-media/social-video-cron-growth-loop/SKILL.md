...
- For the stricter user-corrected Viral Radar procedure — each newly discovered creator video must be clipped/uploaded itself, minimum 5 clips per found influencer video unless the user explicitly asks for more, no old queue/placeholder substitutions — see `references/viral-radar-strict-discovered-source-procedure-2026-07.md`.
- For the latest user correction on partial uploads, queue semantics, and the 5-clip minimum, see `references/viral-radar-partial-upload-queue-and-minimum-2026-07.md`: `1/5` or `1/10` is PARTIAL/BLOCKED, not OK; an empty queue only means no rendered failed uploads remain, not that missing clips are complete.
