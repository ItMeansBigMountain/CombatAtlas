...
- `youtube.upload` scope is enough to upload videos, but it is not enough for later metadata edits. For title/description/tag cleanup, request both:
  - `https://www.googleapis.com/auth/youtube.upload`
  - `https://www.googleapis.com/auth/youtube`
- A YouTube metadata update can return `403 forbidden` even with valid scopes if the OAuth token belongs to the wrong channel/account. Verify `channels().list(mine=True)` before editing.
