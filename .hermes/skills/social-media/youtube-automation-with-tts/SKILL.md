...
For the user's current content-creation system, account mapping, upload visibility, calendar/cron contract, and faceless-vs-Viral-Clip-Radar boundaries, follow `references/content-creation-account-and-upload-rules-2026-06.md`: **Viral Radar is higher priority than faceless YouTube/newsletter videos** when scheduling jobs, retrying queues, or allocating upload/rate-limit capacity. Faceless/newsletter videos from any approved source account must upload via the Trapiistan/Sosai Oyama YouTube token/channel (`/opt/data/secrets/youtube-trapiistan/youtube_upload_token.json`, channel `UCsxzQlusqwmMUdjMvKAJDfA`); Viral Radar influencer clips should try Classical Echos first (`/opt/data/secrets/youtube-classicalechos/youtube_upload_token.json`, channel `UCcIpxiU2CLEsBdHcc7_lcyA`), fail over to Trapiistan/Sosai when Classical Echos hits YouTube upload limits, and then use fareed320 as an additional upload-limit failover once `/opt/data/secrets/youtube-fareed320/youtube_upload_token.json` is verified and its channel ID is registered. Do not trust inherited `YOUTUBE_UPLOAD_TOKEN` values for either lane; pass fixed lane tokens and expected channel IDs. Keep affan.fareed@gmail.com read-only except source-email cleanup after verified upload, use public YouTube uploads by default, and do not add stock footage to clipping videos.
