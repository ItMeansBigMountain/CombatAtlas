...
5. Inspect broad market and candidate universe. At minimum quote/history: SPY, QQQ, HOOD, NVDA, AMD, AVGO, SOFI, PLTR, SMCI, HIMS, RBLX, RKLB. Use get_equity_historicals where needed for recent trend/support/volume.
6. Confirm quotes are fresh enough for the intended session. If US equity market is closed/holiday or quote timestamps are stale, manage analytically only; do not place new orders from stale data.
7. Manage existing positions first: check whether HOOD/NVDA or any other held equity has breached thesis/invalidation, is down ~8% from entry, has hit target/extension, or needs hold/trim/exit. Never move risk farther away.
8. If current deployment is below target and market state is clear, identify clean additions/entries. Prefer liquid fractional-tradable equities with clear invalidation, normal spread, and R:R >= 1.5:1; prefer 2:1+. Bias toward NVDA/QQQ/AVGO/HOOD/SOFI only if live structure confirms.
9. Before any order placement, call review_equity_order. Only call place_equity_order if review succeeds, alerts are acceptable under policy, and the setup remains inside risk/deployment rules.
10. Journal every run, including no-trade decisions, reviews, placements, exits, and tool failures under /opt/data/HeRmEz/projects/trading-journal/YYYY-MM-DD/ using a concise markdown file.
11. Final response must be concise and include: account value, cash, deployment %, positions summary, action taken or no-trade reason, any order IDs if placed, and journal path.
...
# Agentic Robinhood Monitor — 2026-06-19 16:30 UTC

# Current State

- Account: Agentic ending 1041, verified `agentic_allowed=true`
...
- **HOOD**: 0.535786 shares @ avg $93.32  
  Last usable price: ~$108.00 from stale after-hours data  
  Approx value: **$57.86**  
  Approx P/L: **+$7.87 / +15.7%**  
  Decision: **Hold** — no thesis breach from available data.

- **NVDA**: 0.190150 shares @ avg $210.36  
  Last usable price: ~$210.33 from stale after-hours data  
  Approx value: **$40.00**  
  Approx P/L: **about flat**  
  Decision: **Hold** — not down ~8%, no exit trigger from available data.

# Action Taken

**No trade placed. No order review called.**

Reason: all candidate/holding quotes were stale relative to the run time. Latest regular-session trades were from **2026-06-18 19:59:59Z**, with latest non-regular quotes around **2026-06-19 00:00Z**. Since fresh regular-session data was unavailable, new fractional/dollar orders would violate the policy gate.

# Orders

Recent filled agentic orders inspected:

- HOOD buy, $50 market, filled 2026-06-12  
  Order ID: `6a2c2c8f-e73e-47ca-bec4-124cdd754390`

- NVDA buy, $40 market, filled 2026-06-15  
  Order ID: `6a30134d-3b1f-4a1f-89ce-dfe733aa20be`
...
`/opt/data/HeRmEz/projects/trading-journal/2026-06-19/agentic-monitor-163029Z.md`
