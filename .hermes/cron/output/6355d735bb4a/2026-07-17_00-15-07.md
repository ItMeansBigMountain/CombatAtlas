...
- `queue_count`: **1**
  - Old TOGI MP4 has no upload metadata.
  - Preserved, but cannot be uploaded safely or count toward new sources.
  - Remaining deficits were never rendered, so they are not queued.

- Blockers:
  - Source downloads hit pytubefix HTTP 403 and pytube HTTP 400.
  - Some usable title-named local MP4s were not resolved as the manifests’ expected `source.mp4`.
  - GG33’s fifth clip was never rendered or queued.
  - Recovery replay exposed Python interpreter drift: seed subprocess lacked Google modules.

- Metrics: OK. OAuth metrics fetched for 391 videos.
- Auth health: OK.
- Next: fix local-source resolution and Hermes-venv propagation, finish GG33 5/5, then acquire/replay each blocked exact source using fresh Classical Echos cookies/OAuth, residential proxy, or local MP4.

- Report: `OUTPUTS/strict-viral-radar-run-2026-07-17T001305Z.json`
