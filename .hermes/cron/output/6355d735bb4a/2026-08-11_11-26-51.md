...
Queue was checked and drained first. It is empty because no clips reached rendering—not because the source minimums were completed.
...
- **Andrew Huberman — “Using AI to Increase Your Intelligence & Enrich Humanity | Dr. Fei-Fei Li”**
  ...
  - Planned: 10 distinct clips
  - **Uploaded: 0/5**
  - Blocker: source acquisition failed. yt-dlp encountered YouTube’s bot-check, pytubefix returned HTTP 403, and pytube returned HTTP 400. The local attempted download is 0 bytes and failed `ffprobe`.
...
- **Hamza Unfiltered — “Hoes f*ck for free, good girls f*ck for lunch”**
  ...
  - Planned: 10 distinct clips
  - **Uploaded: 0/5**
  - Blocker: same source-acquisition failure. The local attempted download is 0 bytes and failed `ffprobe`.

### Preflight and metrics

- YouTube auth/channel health check: **OK**, no token or channel problems.
- Metrics monitor: **OK**
  - Videos seen: 457
  - Statistics fetched: 431
  - Token errors: none
- No NASA, space, unknown, filler, or unrelated queued clips were used.

### Raw blocker

```text
yt-dlp: Sign in to confirm you’re not a bot. Use --cookies-from-browser or --cookies.
...
Supply fresh authenticated YouTube browser cookies, a residential proxy, or rights-safe local MP4 files at:
...
Then replay each exact source with its priority plan and five-clip minimum.

Run artifact: `STATE/strict_viral_radar_run_2026-08-11T112622Z.json`
