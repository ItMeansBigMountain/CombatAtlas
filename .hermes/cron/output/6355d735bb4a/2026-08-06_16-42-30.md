...
**Uploaded URLs:** None  
**Queue count:** `0`  
**Never-rendered backlog:** `15` clips  
...
- **Andrew Huberman — 0/5**  
  “Control Your Brain Chemistry for Focus, Motivation & Well-Being”  
  https://www.youtube.com/watch?v=xKvlK7OqZso  
  Duration supports five distinct clips. All five candidates were excluded by the duplicate/review guard, so none were rendered.

- **Chris Williamson — 0/5**  
  “The New Science Of Female Attraction — Psychology Roundtable”  
  https://www.youtube.com/watch?v=YV7xNa3QO0c  
  Duration supports five distinct clips. Exact-source acquisition failed: pytubefix returned HTTP 403 and pytube returned HTTP 400. No clips rendered.
...
`UPLOAD_QUEUE` was checked first and was empty. It remains empty because no rendered clip reached an upload failure—not because the source requirements were completed. The remaining 15 clips were never rendered. One unrelated historical item in `UPLOAD_QUEUE_HOLD` was not counted.

No NASA, space, unknown-creator, filler, old queue, or unrelated clips were counted.
...
Regenerate unique transcript-grounded packaging and rendered stems for the Huberman and Hamza manifests so duplicate protection does not eliminate all candidates. Restore the Chris Williamson source through a fresh Classical Echos pytubefix session, fresh cookies, residential proxy, or local/official MP4. Then replay each exact source independently until it reaches **5/5**, queueing every rendered upload failure and using the configured account failover chain.

Report saved to:

`OUTPUTS/strict-discovered-report-20260806T1640Z.md`
