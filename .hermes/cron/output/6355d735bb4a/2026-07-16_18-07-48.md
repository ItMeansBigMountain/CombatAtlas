...
- Metrics monitor: OK. OAuth metrics active.
- Discovery: 9 creator videos seeded with 5 clips planned each.
- Queue count: **1**
  - Old TOGI render lacks upload metadata.
  - Preserved, not counted.
  - No new failed renders are queued.
  - Remaining deficits were never rendered.

**Per-source results**

- Andrew Huberman `UTB5gAkjevk`: **0/5**
  - Source blocked: pytubefix 403, pytube 400.
...
- GG33 `CvwPfe00CPs`: **4/5**
  ...
  - One deficit was never rendered or queued.
- Mr Belmar `BZ_7DELwN7w`: **2/5**
  - https://youtu.be/cfqyHOyEczI
  - https://youtu.be/kSyj0jG8Lvw
  - Three deficits were never rendered or queued.
...
- Only Alex Hormozi reached the required minimum.
- The runner incorrectly treats five aggregate uploads as success and can drift into older manifests.
- TateSpeech and Capital Club playlist IDs return 404.
- Luke Belmar and TOGI had no recent long-form candidate.

**Next action**

- Finish GG33’s remaining 1 clip and Mr Belmar’s remaining 3 from local source-ready media.
- Acquire blocked sources using fresh Classical Echos pytubefix OAuth/cookies, a residential proxy, or local MP4s.
- Enforce exact priority plans and per-source `5/5`; aggregate uploads must not return OK.

- Durable report: `OUTPUTS/strict-viral-radar-run-2026-07-16T180000Z.json`
