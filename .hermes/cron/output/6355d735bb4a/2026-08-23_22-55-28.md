...
- **Uploaded URLs:** None
- **Queue drained first:** Attempted
- **Queue replay uploads:** 0
- **`queue_count`:** 1 protected rendered clip
- **Fresh watchlist seeding:** `blocked_auth`
- **Metrics:** 477 IDs found, 0 statistics fetched due to OAuth failure

### Per-source result

- **Alex Hormozi — “8 Entrepreneurs Compete for $100,000 (Episode 1)”**  
  Source: https://www.youtube.com/watch?v=zzleYxkf39k  
  **Uploaded: 0/5**
  - Runner reported one rendered clip, but it was neither uploaded nor added as a new protected queue item.
  - Four additional distinct clips were never rendered/planned.
  - These deficits are not represented by `queue_count`.

### Blockers

1. Classical Echos, Trapiistan/Sosai, and A F all return:
```text
google.auth.exceptions.RefreshError:
invalid_grant: Token has been expired or revoked.
```

2. Fresh manifest seeding failed when the Classical Echos token could not refresh:
```text
...
3. Exact creator-source acquisition also encountered YouTube bot confirmation, `pytubefix` unavailable responses, and `pytube` HTTP 400 errors.

4. The existing protected queue item remains:
`UPLOAD_QUEUE/chris-williamson-q-a-becoming-a-dad-health-update-f-02-daily.mp4`

### Next action

Open the fresh OAuth URLs generated by:

```bash
python3 /opt/data/scripts/youtube_auth_healthcheck.py --verbose
```

Reauthorize **Classical Echos**, **Trapiistan/Sosai**, and **fareed320/A F**, then return each complete localhost callback URL. After token exchange and channel verification, rerun queue-first, reseed current long-form sources, and complete five distinct exact-source uploads per selected video. Sources still blocked at acquisition require fresh YouTube cookies, a residential proxy, or a local/official creator MP4.

Artifacts:

- `OUTPUTS/strict-cron-20260823T225307Z.log`
- `OUTPUTS/research-brief-20260823T225307Z-strict-creator-run.json`
