...
**Status:** `BLOCKED/PARTIAL` (`blocked_auth`; discovery and uploads unavailable)

- **Uploaded URLs:** None
- **Newly selected sources:** None; latest watchlist seeding could not authenticate
- **Per-source result:** No source reached **5/5**
- **UPLOAD_QUEUE:** **1**
- **Queue drain:** Attempted first; **0 uploaded**, exact rendered clip and metadata preserved
- **Metrics monitor:** 477 video IDs found, **0 live statistics fetched**
- **Run artifact:** `STATE/strict-viral-radar-run-20260821T094141Z.json`

### Protected queued clip

- **Source:** Chris Williamson, “Q&A: Becoming A Dad, Health Update, Favourite Peptides…”
- **Source URL:** https://www.youtube.com/watch?v=8Oj3NxSLP1U
- **Progress:** **0/5 this run**
- **Queue metadata:** `UPLOAD_QUEUE/chris-williamson-q-a-becoming-a-dad-health-update-f-02-daily.mp4.upload.json`

### Blockers

All three configured upload profiles have expired or revoked refresh grants:

- Classical Echos
- Sosai Oyama / Trapiistan
- A F / fareed320

Classical Echos is also the discovery credential, so fresh watchlist manifests could not be seeded. No expensive source acquisition or rendering was started after this hard preflight failure.
...
uploaded_count=0
queue_count=1
queued clip preserved=true
```

### Required next action

Run the OAuth flow for each profile using the fresh URLs generated by:

```bash
python3 /opt/data/scripts/youtube_auth_healthcheck.py --verbose
```

Return each **full localhost callback URL** in this format:
...
After token exchange and health-check verification, replay the workflow queue-first, seed the latest manifests, and require **at least 5 public clips from every selected newly discovered source**.
