...
- **Uploaded URLs:** None
- **Final `queue_count`:** `1`
- **Metrics:** 477 video IDs found, 0 statistics fetched; Classical Echos OAuth returned `invalid_grant`.
- No NASA/space/unknown placeholders or unrelated filler were used.

### Queue-first result

The protected queue item was attempted first but could not upload because all configured upload profiles have expired or revoked OAuth grants.

- Source: Chris Williamson — `8Oj3NxSLP1U`
- Original: https://www.youtube.com/watch?v=8Oj3NxSLP1U
- Queued media: `UPLOAD_QUEUE/chris-williamson-q-a-becoming-a-dad-health-update-f-02-daily.mp4`
- Validation: 42 seconds, 1080×1920, video and audio present
- Metadata remains protected beside the MP4.

### Per-source status

- **Chris Williamson — `8Oj3NxSLP1U`: 0/5 uploaded this run — BLOCKED/PARTIAL**
  - Historical public ledger: 1 clip
  - Rendered and queued: 1 clip
  - Never-rendered deficit to reach five total: 3 clips
  - The three never-rendered clips are backlog deficits and are **not** represented by `queue_count`.

No new source could be selected because latest-watchlist manifest seeding failed during OAuth refresh. Ten existing source-ready fallback candidates were attempted, but none rendered: YouTube returned cloud-IP bot confirmation, pytubefix reported unavailable, and pytube returned HTTP 400. One candidate was itself a YouTube Short and cannot support five distinct clips.
...
Reauthorize all three upload profiles using the fresh URLs produced by the auth watchdog, returning each **full localhost callback URL**. Then rerun in this order:

1. Drain the protected upload queue.
2. Seed current long-form watchlist manifests.
3. Acquire the exact creator-original source using fresh cookies, residential proxy, Classical Echos pytubefix OAuth, or an official/local MP4.
4. Review five distinct transcript-grounded windows.
5. Render and upload until every selected source reaches **5/5**.

Durable report: `OUTPUTS/research-brief-20260822-strict-creator-run.json`  
Application log: `OUTPUTS/strict-cron-20260822-live.log`  
Protected queue verified; filesystem has **32 GB free**.
