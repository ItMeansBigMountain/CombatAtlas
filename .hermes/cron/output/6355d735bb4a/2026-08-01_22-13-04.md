...
## BLOCKED/PARTIAL — `blocked_auth`

- **Public uploads:** 0
- **Uploaded URLs:** None
- **Newly discovered source result:** **0/5**
- **`queue_count`:** 1 rendered clip with matching metadata, preserved for retry
- **Queue drained first:** Attempted; upload failed during OAuth refresh
- **Watchlist seeding:** Attempted; failed before source selection
- **Metrics monitor:** 383 video IDs found, but `stats_fetched=0` due invalid OAuth

### Preserved queue item
...
The exact rendered clip and upload metadata remain queued. It was not counted toward the newly discovered source minimum.

### Blockers

1. Classical Echos OAuth token is expired/revoked.
2. Trapiistan/Sosai failover OAuth token is also expired/revoked.
3. Latest-watchlist discovery requires Classical Echos OAuth and failed before selecting a long-form creator video.
4. Therefore, the five required clips were never planned or rendered. This is an unrendered backlog deficit—not success.
