...
**UPLOAD_QUEUE:** `queue_count=0`  
**Metrics monitor:** OK — 433 video IDs seen, 407 statistics fetched, no token errors.  
**Auth/channel preflight:** OK.

### Uploaded URLs

**Kinobody — “My Lowest Body Fat Score EVER (DEXA Scan Results)”**
- https://youtu.be/Zuew8I7lA_Q
- https://youtu.be/qnGhvASSaQw
- https://youtu.be/7ydeVYwS9Fg
- https://youtu.be/9fTpt8qEa7A

**Alex Hormozi — “Answering Your Sales Question Live”**
- https://youtu.be/ebFSvowZJzM

### Per-source results

- Andrew Huberman — `xKvlK7OqZso` — **0/5**
  - Blocked: all manifest candidates were already consumed/filtered; pipeline reported no reviewed clips available.
- Chris Williamson — `XqXLijA6CcY` — **0/5**
  - Blocked source: five exact-source windows attempted; pytubefix returned HTTP 403 and pytube HTTP 400.
- Kinobody / Greg O’Gallagher — `hqGNhNxY13c` — **4/5**
  - **PARTIAL:** four public uploads succeeded.
  - The fifth clip rendered but produced no upload, apparently because clip 5 repeated clip 1’s generated packaging and was rejected by the duplicate guard.
  - It was not an upload failure, so it did not enter `UPLOAD_QUEUE`.
- Cobratate / Andrew Tate — `cXJoPxauVEU` — **0/5**
  - Blocked: no reviewed manifest clips available.
- GG33 — `tZHGULA_55U` — **0/5**
  - Blocked: no reviewed manifest clips available.
- Alex Hormozi — `TLgSkSQUql4` — **1/5**
  - **PARTIAL:** one public upload succeeded; remaining candidates were filtered as already consumed/duplicates.
  - No rendered upload failures remained to queue.
- Hamza — `XrU_MlxGFRo` — **0/5**
  - Blocked: no reviewed manifest clips available.
- Hamza Unfiltered — `Kgc7XCtSRXY` — **0/5**
  - Blocked: no reviewed manifest clips available.
- Ryan Holiday / Daily Stoic — `28MUVg0oTGg` — **0/5**
  - Blocked source: remaining exact-source attempts failed with pytubefix HTTP 403; no clips rendered.

### Raw application lines

```json
{"source_video_id":"XqXLijA6CcY","status":"blocked_min_uploads_not_met","uploaded_count":0,"target":5,"queue_count":0,"source_failures":5}
{"source_video_id":"hqGNhNxY13c","status":"blocked_min_uploads_not_met","uploaded_count":4,"target":5,"queue_count":0}
{"source_video_id":"TLgSkSQUql4","status":"blocked_min_uploads_not_met","uploaded_count":1,"target":5,"queue_count":0}
{"source_video_id":"28MUVg0oTGg","status":"blocked_min_uploads_not_met","uploaded_count":0,"target":5,"queue_count":0,"source_failures":2}
```

```text
HTTP Error 403: Forbidden — pytubefix source acquisition
HTTP Error 400: Bad Request — plain pytube source acquisition
no reviewed manifest clips available; add/review a new clip manifest
```

### Blocker interpretation

`queue_count=0` is **not success**. The remaining deficits were not failed uploads:

- source media could not be acquired;
- candidates had already been consumed or rejected by duplicate checks;
- or a rendered candidate duplicated another clip’s packaging.

No NASA/space/unknown placeholder, stock filler, unrelated source, or old queued clip was counted toward these source minimums.

### Next action

1. Repair clip-window packaging so clip 5 cannot repeat clip 1’s title/hook, then generate and upload Kinobody’s missing fifth distinct clip.
2. Plan four new, non-duplicate windows from the Alex Hormozi source and upload them.
3. Restore blocked sources using the approved Classical Echos pytubefix OAuth/session, fresh cookies, residential proxy, or local/official creator MP4.
4. Reseed fresh unconsumed windows for sources returning “no reviewed manifest clips available,” then replay through the Classical Echos → Trapiistan/Sosai → fareed320 failover chain.
