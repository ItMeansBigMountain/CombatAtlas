...
## Viral Radar strict run — **PARTIAL / BLOCKED**

**Preflight**
- YouTube auth/channel health: **OK**
- Metrics monitor: **OK** — 385 uploads logged; 359 metric records fetched
- Existing `UPLOAD_QUEUE` drained first: **0 items**
- Latest watchlist manifests seeded: **7 source videos**, each long enough to target five clips
- Final `queue_count`: **0**

### Public uploads completed this run

**Chris Williamson — “Is Polygamy A Better Deal For Women?” — 2/5**
- https://youtu.be/tIxn7HdSMzs
- https://youtu.be/lTVVjAV-jbw

**GG33 — “GG33 looks into islam” — 4/5**
- https://youtu.be/Rw7xKxwa3Vc
- https://youtu.be/IGs9tLj_H4I
- https://youtu.be/E2rmJEw3QsM
- https://youtu.be/zzYHRyKow_0

### Per-source status

- **Andrew Huberman / Jocko Willink** — “How to Become Resilient…” — **0/5, BLOCKED**
  - All seeded clips matched existing duplicate keys, leaving no eligible reviewed clip.
- **Chris Williamson** — “Is Polygamy A Better Deal For Women?” — **2/5, PARTIAL**
  - Three windows were never rendered because source acquisition initially failed:
    - `pytubefix`: HTTP 403
    - `pytube`: HTTP 400
  - The source later became available for two windows, but the run still ended at 2/5.
- **Cobratate / Andrew Tate** — `MY_DATA_041013_5_0020` — **5/5 already public**
- **GG33** — “GG33 looks into islam” — **4/5, PARTIAL**
  - The fifth planned clip collided with the first clip’s normalized title/hook and was correctly rejected by the duplicate guard.
  - The source was cleaned after the ledger incorrectly considered all associated clip keys covered, so the missing distinct fifth clip was never uploaded.
- **Hamza** — “She’s Amazing, But She Has Red Flags” — **0/5, BLOCKED**
  - No eligible reviewed manifest clips remained after duplicate checks.
- **Hamza Unfiltered** — “What lifestyle makes you the most money?” — **0/5, BLOCKED**
  - No eligible reviewed manifest clips remained after duplicate checks.
- **Ryan Holiday / Daily Stoic** — “Fighting Modern Corruption With Ancient Stoic Wisdom” — **0/5, BLOCKED**
  - No eligible reviewed manifest clips remained after duplicate checks.

### Queue semantics

`queue_count=0` does **not** mean these deficits are complete. No rendered upload failed, so nothing entered `UPLOAD_QUEUE`. The missing clips are **never-rendered/planning/source deficits**:

- Chris Williamson: 3 missing due intermittent source acquisition failure
- GG33: 1 missing due duplicate fifth packaging plus premature source cleanup
- Huberman, Hamza, Hamza Unfiltered, and Daily Stoic: five missing each because duplicate guards found no eligible reviewed clips

### Additional discovery blockers

- TateSpeech watchlist uploads playlist: YouTube API `playlistNotFound`
- Capital Club Community watchlist uploads playlist: YouTube API `playlistNotFound`
- No qualifying recent long-form source found for Kinobody, Luke Belmar, Mr Belmar, Alex Hormozi, or TOGI within the initial 12-video scan

### Next action

Regenerate genuinely distinct, transcript-grounded clip windows and unique public titles for deficient sources; restore/reacquire the exact source files using the approved Classical Echos `pytubefix` OAuth/cookies or local-source path; then rerun only those source manifests until each reaches **5/5**. Also repair the cleanup check so duplicate-equivalent clip keys cannot make a five-clip manifest appear complete after only four public uploads.

Logs: `OUTPUTS/strict-cron-logs/2026-08-02-*.log`  
Seed record: `STATE/strict_seed_20260802_cron.json`
