...
**New source:** Andrew Huberman — *Neuroscience of Emotions & Tools for Improving Emotion Regulation*  
Source: https://www.youtube.com/watch?v=P-h5WSQG1Sw  
Duration: 7,771 seconds

- **Uploaded this run:** None
- **New Huberman source:** **0/5 — BLOCKED**
- **`queue_count=0`**
- Upload queue was checked first, but no rendered retry items existed.
- Five exact-source windows were attempted. None rendered because source acquisition failed.
- No NASA/space/unknown filler, old queue item, or unrelated source was counted.
- YouTube auth/channel healthcheck: **OK**
- Metrics monitor: **OK** — 477 IDs found; 451 statistics fetched

### Source blocker

```text
yt-dlp: ERROR: [youtube] P-h5WSQG1Sw: Sign in to confirm you’re not a bot.
pytubefix: urllib.error.HTTPError: HTTP Error 403: Forbidden
pytube: urllib.error.HTTPError: HTTP Error 400: Bad Request
```

The empty queue is **not success**. These five clips were never rendered, so no valid MP4 exists to queue.

### Selected-source totals

- Andrew Huberman `P-h5WSQG1Sw`: **0/5 — BLOCKED**
- Chris Williamson `-5epM9WG95g`: **4/5 — PARTIAL**
- Kinobody `ERK8aVIa8pw`: **4/5 — PARTIAL**
- Cobratate `cXJoPxauVEU`: **5/5 — COMPLETE**
...
- Hamza Unfiltered `Egwjll72fuU`: **0/5 — BLOCKED**
- Ryan Holiday `faJAvHq9hwE`: **4/5 — PARTIAL**

### Next action

Acquire the exact Huberman source using a meaningfully different path: fresh residential cookies/proxy, approved Classical Echos pytubefix OAuth, an official downloadable copy, or a rights-cleared MP4 at:

`SOURCES/P-h5WSQG1Sw/source.mp4`

Then verify with `ffprobe`, review five materially distinct transcript windows, and render/upload all five. The existing 4/5 sources also need one genuinely distinct, transcript-reviewed fifth window.

Report: `OUTPUTS/strict-discovered-report-20260817-cron2.md`  
Run log: `OUTPUTS/strict-live-P-h5WSQG1Sw-20260817-cron2.log`
