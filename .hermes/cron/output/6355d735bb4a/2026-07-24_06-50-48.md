...
**Uploaded URLs this run:** None  
**UPLOAD_QUEUE drained first:** Yes  
**queue_count:** **0**  
**Watchlist manifests seeded:** **10** long-form sources, each targeting five distinct clips.

The empty queue does **not** mean completion. No rendered upload failed during this run; the remaining deficits were never rendered because source acquisition failed or previously rendered candidates were duplicate-skipped.

### Per-source public totals

- Andrew Huberman — `LQI8tl8S2PE`: **0/5**
- Chris Williamson — `qVdr4CQyiQw`: **0/5**
- Kinobody / Greg O’Gallagher — `X6oP4016kuE`: **0/5**
- Cobratate / Andrew Tate — `cXJoPxauVEU`: **4/5**
  - https://youtu.be/sunS4nZNc2s
  - https://youtu.be/hGfVznx9DJw
  - https://youtu.be/g8xRZizqqi4
  - https://youtu.be/doVi9fN6cV0
- GG33 — `_LMTdi9oyhc`: **0/5**
- Mr Belmar / Nate Belmar — `BZ_7DELwN7w`: **4/5**
  - https://youtu.be/cfqyHOyEczI
  - https://youtu.be/kSyj0jG8Lvw
  - https://youtu.be/qM1OTDnwnOk
  - https://youtu.be/zUCiHuKjKFQ
- Alex Hormozi — `WttZ6b-KUP4`: **4/5**
  - https://youtu.be/ugSE1EeZUrU
  - https://youtu.be/FLqeuOliXwo
  - https://youtu.be/Flz4tU6SfBM
  - https://youtu.be/yd1pS11qI2U
- Hamza — `Yoa67bvOAdI`: **4/5**
  - https://youtu.be/fDmRouJNaN0
  - https://youtu.be/pOtcnKIxKa8
  - https://youtu.be/Hb99gzDE_QQ
  - https://youtu.be/66tkI9d5ScA
- Hamza Unfiltered — `NhLnGyRo4Vw`: **0/5**
- Ryan Holiday / Daily Stoic — `zt5RR2nghzk`: **0/5**

### Blockers

- YouTube upload authentication and channel identity are healthy.
- Source acquisition is blocked from this cloud host:
  - `pytubefix`: HTTP 403
  - plain `pytube`: HTTP 400
  - `yt-dlp` remained disabled under the current lane policy.
- Candidate processing produced no new public upload. Existing render candidates were duplicate-skipped; exact newly seeded sources could not be acquired for additional distinct renders.
- TateSpeech and Capital Club playlist IDs returned 404.
- Luke Belmar and TOGI had no recent qualifying long-form source.
- No NASA, space, unknown-creator, stock, or old-queue filler was used.

```text
source_download_failed: https://www.youtube.com/watch?v=LQI8tl8S2PE
...
- Video IDs seen: 344
- Statistics fetched: 318
- Current strongest pattern: Alex Hormozi truth/power/business clips.

### Next action

Restore source acquisition using a fresh Classical Echos pytubefix OAuth/session, fresh YouTube cookies, a residential proxy, or local/official MP4s for these exact sources. Replay the **4/5 sources first**, then complete each 0/5 source. Any newly rendered clip that fails upload must remain in `UPLOAD_QUEUE`.

Raw application log:  
`STATE/strict_daily_cron_20260724T063727Z.log`
