...
- **Uploaded URLs:** None
- **Queue count:** `0`
- **Metrics monitor:** OK — OAuth metrics active; 457 videos discovered, 431 statistics fetched
- **YouTube auth/channel preflight:** OK
- **Latest watchlist sources selected:** 3 long-form creator videos
...
- **Andrew Huberman — “Using AI to Increase Your Intelligence & Enrich Humanity | Dr. Fei-Fei Li”**  
  ...
  **Uploaded: 0/5**  
  Source acquisition failed for all five windows. yt-dlp encountered YouTube’s cloud-IP bot check, pytubefix returned HTTP 403, and pytube returned HTTP 400. The local placeholder MP4 is zero bytes and fails `ffprobe`.

- **Alex Hormozi — “How To Play To Win | Alex & Leila Answer Your Questions Live”**  
  Source: https://www.youtube.com/watch?v=QgmRlzh0Y7M  
  **Uploaded: 0/5**  
  The refreshed plan has no reviewed manifest clips. This is a never-rendered planning/review deficit.

- **Hamza Unfiltered — “Hoes f*ck for free, good girls f*ck for lunch”**  
  Source: https://www.youtube.com/watch?v=1HC76d9RGH4  
  **Uploaded: 0/5**  
  The refreshed plan has no reviewed manifest clips. Its existing source placeholder is also zero bytes and fails `ffprobe`.

### Queue semantics

`queue_count=0` is **not success**. All 15 required clips remain never-rendered deficits; there were no rendered upload failures to place in `UPLOAD_QUEUE`.
...
Refresh authenticated YouTube browser cookies, configure a residential proxy, or place rights-cleared local MP4s for these exact sources. Then review the five Hormozi and Hamza manifest windows, validate each source with `ffprobe`, and replay each source until it reaches **5/5**.

Durable report: `OUTPUTS/strict-discovered-report-20260811-late.md`
