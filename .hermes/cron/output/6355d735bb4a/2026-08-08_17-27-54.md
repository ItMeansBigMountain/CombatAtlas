...
## Viral Radar strict discovered-source run — BLOCKED/PARTIAL
...
- **Newly selected sources:** 3 qualifying long-form creator videos
- **Public uploads:** 0
- **Uploaded URLs:** none

### Per-source results

- **Andrew Huberman** — “Control Your Brain Chemistry for Focus, Motivation & Well-Being”  
  Source: https://www.youtube.com/watch?v=xKvlK7OqZso  
  **uploaded_count: 0/5**  
  Blocker: no reviewed manifest clips remained available after candidate/review/duplicate filtering.

- **Hamza Unfiltered** — “How to heal trauma”  
  Source: https://www.youtube.com/watch?v=Kgc7XCtSRXY  
  **uploaded_count: 0/5**  
  Blocker: no reviewed manifest clips remained available after candidate/review/duplicate filtering.

- **Ryan Holiday / Daily Stoic** — “The Man Traveling America to Prove You Wrong About It”  
  Source: https://www.youtube.com/watch?v=V9sm6z2Ce7g  
  **uploaded_count: 0/5**  
  Blocker: all five exact-source acquisition attempts failed before rendering.
...
"method": "yt-dlp", "error": "skipped by --no-ytdlp or VIRAL_RADAR_DISABLE_YTDLP=1"
```

### Queue/backlog distinction

**queue_count=0 is not success.** No rendered clip failed upload, so nothing could be queued. The outstanding deficit is **15 never-rendered/never-uploaded clips** caused by review/candidate filtering and exact-source acquisition failure.

No NASA, space, unknown-creator, stock-filler, unrelated fallback, or historical queue clip was counted toward these source minimums.

### Next action

Reset/review the Huberman and Hamza candidate manifests, and restore source acquisition using the approved Classical Echos pytubefix OAuth session, fresh cookies, residential proxy, or a local/official creator MP4. Then replay five distinct windows per source. Any clip that renders but fails upload must remain in `UPLOAD_QUEUE` and use Classical Echos → Trapiistan/Sosai → fareed320 failover.

Artifacts:

- `OUTPUTS/strict-seed-20260808-cron.json`
- `OUTPUTS/strict-run-20260808-cron.json`
- `OUTPUTS/strict-live-progress-20260808-cron.json`
- `OUTPUTS/strict-live-*-20260808-cron.log`
