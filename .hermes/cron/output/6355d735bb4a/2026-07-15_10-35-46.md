...
- Metrics monitor: ok, but live stats blocked.
  - Trapiistan token: `invalid_grant`
  - Classical Echos token: `invalid_grant`

- Upload queue drain: attempted first.
  - Queue upload metadata count: `0`
  - Orphan queue MP4s without `.upload.json`: `3`
  - These can’t be safely drained by the uploader without rebuilt metadata.

- Latest watchlist manifest seeding: blocked.
  - Classical Echos OAuth refresh failed with `invalid_grant`.

- Strict upload loop:
  - Ran with `FORCE_UPLOAD=1`
  - Target: `5` public clips
  - Upload cap probe: `100`
  - Command timed out after 600s.
  - No new public uploads confirmed.

Uploaded URLs this run:
- None.

Per-source counts:
- New latest watchlist source: `0/5`
  - Blocked before fresh seeding completed.
- Chris Williamson — Why The AI Doomers Might Be Right: `3/5`
  - https://youtu.be/OpqMlPQrQns
  - https://youtu.be/8MMv11PHqUA
  - https://youtu.be/INSnVXMLbjg
- Chris Williamson — The Greatest Bluff In Military History: `3/5`
  - https://youtu.be/YpsCmfdxWhw
  - https://youtu.be/6DWp1k5DVik
  - https://youtu.be/uEbhhayO8RA
- GG33 — The Dark Truth about Earth and Your Soul: `4/5`
...
Queue count:
- Drainable queue metadata: `0`
- Orphan MP4s in `UPLOAD_QUEUE`: `3`

Blockers:
- Classical Echos YouTube OAuth is revoked/expired.
- Trapiistan YouTube OAuth is revoked/expired.
- Fresh watchlist seeding cannot run until Classical Echos is reauthed.
- Existing deficits are not “OK”; they are partial/backlog.
- Some queue files are rendered but missing upload metadata, so they need metadata recovery before retry.
...
- Then recover metadata for the 3 orphan queue MP4s, rerun latest manifest seeding, and continue top-ups until each selected long-form source reaches `>=5` public clips.

Saved report:
- `/opt/data/HeRmEz/projects/viral-clip-radar/STATE/strict_viral_radar_cron_2026-07-15_latest.md`
