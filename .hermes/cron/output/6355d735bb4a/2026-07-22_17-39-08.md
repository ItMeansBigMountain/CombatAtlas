...
Existing upload queue was drained first: **0 items**. Watchlist refresh seeded **10 long-form manifests**, including three sources dated July 22.

### Per-source results

**Chris Williamson — “High T & Deadbeat Dads: The Shocking Link” (`KnoWCHZRZiQ`) — 5/5 public**
...
The fifth manifest entry initially duplicated the first public title. It was corrected to the source-specific title **“What Fatherhood Does to Testosterone,”** rendered from the fifth distinct source segment, and uploaded successfully. Generated MP4s were removed only after verified uploads; manifests and upload logs were preserved.

**Kinobody / Greg O’Gallagher — “The REAL Reason You’ll Never Reach 10% Body Fat” (`X6oP4016kuE`) — 0/5**

- **BLOCKED:** The five planned clips use generic public hooks that collide with titles already present in the public duplicate ledger. The strict duplicate guard therefore returned:
```text
"status": "failed"
"error": "no reviewed manifest clips available; add/review a new clip manifest"
```
- These clips were never rendered, so the empty upload queue does **not** indicate completion.

**Hamza Unfiltered — “Greg Douche said 400ng testosterone is good” (`NhLnGyRo4Vw`) — 0/5**

- **BLOCKED/BACKLOG:** Its generated manifest has the same generic-title defect, including a duplicate first/fifth hook. No clips were rendered or uploaded.
- This deficit is not represented in `UPLOAD_QUEUE`.

### Queue and monitoring

- `queue_count`: **0**
- Rendered upload failures: **0**
- Never-rendered/planning deficits: **10 clips** across Kinobody and Hamza Unfiltered
- Metrics monitor: **OK**
  - OAuth metrics: `true`
  - Video IDs seen: `317`
...
Regenerate the Kinobody and Hamza manifests with five **unique, transcript/source-specific, truthful** hooks and titles, then process each exact source independently to 5/5. Do not queue them until clips are actually rendered; if a rendered upload fails, preserve that exact clip and metadata in `UPLOAD_QUEUE`.
