...
- Uploaded URLs: none.
- Uploaded count: 0/5.
- queue_count: 3 orphan rendered MP4s.
- Metrics monitor: ran, but OAuth metrics blocked by token errors.
- Queue drain: blocked.
  - Existing `UPLOAD_QUEUE` has 3 rendered MP4s.
  - They have no `*.upload.json` sidecars, so the queue uploader ignores them.
- Seed latest watchlist manifests: blocked.
  - Classical Echos OAuth token is `invalid_grant`.
- Upload auth: blocked.
  - Classical Echos token expired/revoked.
  - Trapiistan/Sosai token expired/revoked.
- No NASA/space/unknown filler used.
- No old queued clips counted toward new discovered-source minimums.

Per-source status:

- Newly discovered creator source: 0/5.
  - Deficit reason: discovery/upload auth blocked before a valid 5-clip source could complete.
- Existing rendered queue:
  ...
  - Deficit reason: rendered but missing upload metadata sidecars.

Next action:

- Reauth YouTube tokens first:
...
- Then recreate/upload sidecars for the 3 orphan MP4s.
- Then rerun strict Viral Radar with `>=5` public uploads per selected source.
