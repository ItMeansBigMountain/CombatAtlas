...
BLOCKED/PARTIAL — Viral Radar strict run did not hit 5/5.

- Metrics monitor:
  - Ran successfully.
  - Live metrics blocked by expired/revoked YouTube OAuth tokens.
  - Affected:
    - Classical Echos token: `invalid_grant`
    - Trapiistan/Sosai token: `invalid_grant`

- Queue drain:
  - Existing queue before run: empty.
  - A real rendered clip failed upload and was preserved in `UPLOAD_QUEUE`.
  - Current `queue_count`: `1`

- Newly attempted source:
  - Creator: `GG33`
  - Source: `The Dark Truth about Earth and Your Soul - GG33`
  - URL: `https://www.youtube.com/watch?v=qRPw-khPGz4`
  - Uploaded: `0/5`
  - Rendered/queued: `1`
  - Public URLs: none

- Queued failed upload:
  - File: `/opt/data/HeRmEz/projects/viral-clip-radar/UPLOAD_QUEUE/qRPw-khPGz4-viral-radar-daily.mp4`
  - Metadata: `/opt/data/HeRmEz/projects/viral-clip-radar/UPLOAD_QUEUE/qRPw-khPGz4-viral-radar-daily.mp4.upload.json`
  - Title: `The soul trap clip: what GG33 is really warning about`

- Blocker:
  - Upload auth is broken.
  - YouTube API refresh failed with:
    - `invalid_grant: Token has been expired or revoked`
  - Seed/latest watchlist refresh also fails for the same auth reason.
  - No upload URL was returned, so nothing was marked OK.
...
- Auth healthcheck:
  ...
  - Required next steps:
    ...
    - Then exchange callback and verify:
      - `python3 /opt/data/scripts/google_reauth_workflow.py youtube-exchange <profile> <localhost callback URL> --verify`
    - Then rerun:
      - `python3 /opt/data/scripts/youtube_auth_healthcheck.py --verbose`

Final status: BLOCKED/PARTIAL.  
Reason: selected creator source is at `0/5`; one real rendered clip is queued, but uploads cannot proceed until YouTube OAuth is reauthorized.
