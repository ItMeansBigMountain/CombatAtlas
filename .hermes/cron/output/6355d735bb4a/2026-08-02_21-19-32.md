...
**UPLOAD_QUEUE:** 0  
**Metrics monitor:** OK — OAuth metrics active; 365 statistics fetched from 391 logged videos.  
**Auth/channel preflight:** OK — no OAuth, channel-identity, dependency, or metrics problems detected.

### Per-source progress

- **Andrew Huberman — Resilience & Identity (`lxDf8uEypJU`): 0/5**
  - Blocker: no remaining reviewed, non-duplicate manifest clips.
- **Chris Williamson — Marriage/Polygamy (`BPNpK0nAbt0`): 2/5**
  - https://youtu.be/tIxn7HdSMzs
  - https://youtu.be/lTVVjAV-jbw
  - Blocker: remaining source acquisition previously failed via pytubefix HTTP 403 and pytube HTTP 400.
- **Kinobody — The Real Reason You’ll Never Reach 10% Body Fat (`X6oP4016kuE`): 1/5**
  - https://youtu.be/2F0bvsQwrjk
  - Blocker: four clips remain unrendered/unuploaded.
- **Cobratate — `MY_DATA_041013_5_0020` (`cXJoPxauVEU`): 5/5**
...
- **GG33 — Looks Into Islam (`NPL6J19nC34`): 4/5**
  - https://youtu.be/Rw7xKxwa3Vc
  - https://youtu.be/IGs9tLj_H4I
  - https://youtu.be/E2rmJEw3QsM
  - https://youtu.be/zzYHRyKow_0
  - Blocker: one planned clip remains incomplete; it was never rendered into the queue.
- **Mr Belmar — Fitness Is Just the Beginning (`DbYb6oDkHc8`): 0/5**
  - All five distinct clips were planned with source-specific filenames and titles.
  - Blocker: source download failed for every clip; nothing rendered, so the queue correctly remains empty.
- **Alex Hormozi — Honest Advice to Get Rich (`aRVv5NLVRwE`): 0/5**
  - Blocker: source/clip backlog remains unprocessed.
- **Hamza — She’s Amazing, But She Has Red Flags (`XrU_MlxGFRo`): 0/5**
  - Blocker: no remaining reviewed, non-duplicate manifest clips.
- **Hamza Unfiltered — What Lifestyle Makes You the Most Money? (`N8wsid0NC80`): 0/5**
  - Blocker: no remaining reviewed, non-duplicate manifest clips.
- **Daily Stoic — Fighting Modern Corruption (`0tzZs1_uFG0`): 0/5**
  - Blocker: no remaining reviewed, non-duplicate manifest clips.

### Current source failure

```text
Mr Belmar source: https://www.youtube.com/watch?v=DbYb6oDkHc8
pytubefix: urllib.error.HTTPError: HTTP Error 403: Forbidden
pytube: urllib.error.HTTPError: HTTP Error 400: Bad Request
yt-dlp: disabled by the configured no-yt-dlp workflow
log: /opt/data/HeRmEz/projects/viral-clip-radar/LOGS/daily_youtube_source_download/DbYb6oDkHc8
```

The empty queue is **not success**: these deficits were never rendered because source acquisition or manifest eligibility failed. No placeholders, unrelated stock, NASA/space material, or old queued clips were used.

**Next action:** provide or restore a source-ready MP4 for the blocked video IDs, or configure fresh YouTube cookies/residential proxy access. Then render and upload the missing clips, prioritizing GG33’s final `1`, Chris Williamson’s `3`, Kinobody’s `4`, and the five planned Mr Belmar clips.
