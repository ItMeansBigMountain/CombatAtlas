...
- **Uploaded URLs:** None
- **Per-source uploads:** N/A / 5 — OAuth failed before any new source could be selected
- **Active `queue_count`:** 0
- **`UPLOAD_QUEUE_HOLD`:** 2 files, excluded because held/old clips cannot satisfy new-source minimums
- **Metrics:** 436 video IDs seen; 0 live statistics fetched due to OAuth failure
- **Report:** `OUTPUTS/strict-discovered-report-20260810T115230Z.md`
- **Raw seed log:** `OUTPUTS/cron_logs/strict_seed_20260810T115229Z.log`

The active upload queue was checked first and was empty. Latest watchlist seeding was then attempted with a minimum source duration of 480 seconds and five clips per source, but failed before discovery:
...
2. Trapiistan/Sosai failover OAuth refresh grant is expired or revoked.
3. The remaining deficit was never planned/rendered because discovery failed; `queue_count=0` is therefore **not success**.

## Next action

Reauthorize both accounts, returning each **full localhost callback URL** with the indicated prefix.

**Classical Echos** — sign in as `classicalechos@gmail.com`:

<https://accounts.google.com/o/oauth2/auth?response_type=code&client_id=984335329962-jmsnmsu79o45n751hdguq832dqool860.apps.googleusercontent.com&redirect_uri=http%3A%2F%2Flocalhost%3A1%2F&scope=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.upload+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.force-ssl+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.readonly+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyt-analytics.readonly&state=zcmpB7AvvcRCEMMDHdTLqoNNtg3Tml&code_challenge=bQCeMv6f4AJ3LB3TboKFbpQBvJF0qXlcFVxxiN_EYms&code_challenge_method=S256&access_type=offline&include_granted_scopes=true&prompt=consent+select_account&login_hint=classicalechos%40gmail.com>

```text
youtube:classicalechos: <full localhost callback URL>
```

**Trapiistan/Sosai** — sign in as `trapiistan@gmail.com`:

<https://accounts.google.com/o/oauth2/auth?response_type=code&client_id=984335329962-jmsnmsu79o45n751hdguq832dqool860.apps.googleusercontent.com&redirect_uri=http%3A%2F%2Flocalhost%3A1%2F&scope=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.upload+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.force-ssl+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.readonly+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyt-analytics.readonly&state=Tc89T37th5FqAYqvqIxzX04S3LKlOm&code_challenge=dL0z_SPxC5wiZDJujWT9nXkevnrNsOxZhZ_u_QaZpUE&code_challenge_method=S256&access_type=offline&include_granted_scopes=true&prompt=consent+select_account&login_hint=trapiistan%40gmail.com>

```text
youtube:trapiistan: <full localhost callback URL>
```

After token exchange and health-check verification, replay the strict job to seed exact long-form sources and reach **5/5 public clips per selected source**.
