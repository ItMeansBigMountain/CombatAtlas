...
The strict run completed with `blocked_min_uploads_not_met` (exit code 2). It uploaded **3 new public clips**, below the run minimum of 5.

### Uploaded

Andrew Huberman / Sam Harris — meditation, focus, and consciousness:
...
- Andrew Huberman `LQI8tl8S2PE`: **4/5** total, including 3 uploaded this run
- Chris Williamson `qVdr4CQyiQw`: **1/5**
- Kinobody `X6oP4016kuE`: **0/5**
- Andrew Tate `cXJoPxauVEU`: **4/5**
...
- Existing queue was checked/drained first.
- Final `queue_count=0`.
- This empty queue **does not mean completion**: remaining deficits were not rendered because of source-acquisition failures or duplicate planning.
- No rendered upload failures occurred, so there were no failed assets to enqueue.

### Blockers

- Four source-acquisition attempts failed from this cloud host.
- Pytubefix returned HTTP 403; plain pytube returned HTTP 400.
- yt-dlp remained disabled under the configured lane policy.
- Huberman’s fifth planned clip reused the first clip’s public title/hook, causing the duplicate guard to leave the source at **4/5** rather than publishing duplicate metadata.
- The pipeline seeded 10 current long-form manifests, but no selected source reached 5 public uploads during this run.
...
Metrics monitor succeeded:

- OAuth metrics: enabled
- Video IDs seen: 349
- Statistics fetched: 323
- Token errors: none

Current performance leaders remain Alex Hormozi truth, power, and business clips.

### Next action

Restore source acquisition using a fresh Classical Echos pytubefix session, fresh cookies, residential proxy, or local/official MP4s. Regenerate Huberman clip 5 with unique, source-specific packaging, then prioritize all **4/5** sources before processing the larger unrendered deficits.

Raw preserved run log:

`STATE/strict_discovered_run_20260724T125219Z.log`
