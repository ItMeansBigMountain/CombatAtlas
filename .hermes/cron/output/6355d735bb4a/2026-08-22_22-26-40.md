...
- **Uploaded:** 0 public clips
- **Uploaded URLs:** None
- **Queue count:** 1 protected rendered clip
- **Metrics:** 477 video IDs found; 0 live statistics fetched because Classical Echos OAuth returned `invalid_grant`
- **Run artifact:** `/opt/data/HeRmEz/projects/viral-clip-radar/STATE/strict_cron_preflight_2026-08-22T222554Z.json`
...
- **uploaded_count/5:** **0/5 — BLOCKED**
- One valid 42-second, 1080×1920 rendered clip with audio remains protected at:
  `/opt/data/HeRmEz/projects/viral-clip-radar/UPLOAD_QUEUE/chris-williamson-q-a-becoming-a-dad-health-update-f-02-daily.mp4`
- The other four-clip deficit was never rendered this run. It is backlog, not an empty-queue success.

### Blockers

All upload destinations failed OAuth preflight:

```text
Classical Echos: invalid_grant — token expired or revoked
Sosai Oyama/Trapiistan: invalid_grant — token expired or revoked
A F/fareed320: invalid_grant — token expired or revoked
```

Therefore the existing queue could not be drained, and latest-watchlist seeding was also blocked because discovery uses the invalid Classical Echos token. Expensive source/render work was stopped at preflight rather than creating additional clips with no working upload destination.

The latest prior strict acquisition pass also recorded never-rendered source deficits from YouTube bot/age confirmation, `pytubefix` unavailable/HTTP 400, and `pytube` HTTP 400. Those deficits are not represented by `queue_count`.

### Next action

Use the fresh authorization URLs emitted by:

```bash
python3 /opt/data/scripts/youtube_auth_healthcheck.py --verbose
```

Authorize Classical Echos, Trapiistan/Sosai, and fareed320/A F, then return each **full localhost callback URL**. After token exchange, rerun the healthcheck and strict pipeline; it will drain the protected queue first, seed current watchlist manifests, and resume the 5-distinct-clips-per-source requirement. Source deficits additionally need fresh authenticated cookies, a residential proxy, or a rights-cleared local/official MP4 rather than another unchanged cloud-IP download attempt.
