...
BLOCKED/PARTIAL — strict Viral Radar cron

- Uploaded this run: 0/5.
- Upload URLs this run: none.
- UPLOAD_QUEUE drained first: yes.
- queue_count: 0.
- Latest watchlist seeding: blocked.
- Report saved:
  - `STATE/strict_viral_radar_cron_2026-07-14_latest.md`
...
- Chris Williamson — “The Greatest Bluff In Military History”
  - uploaded_count: 3/5
  - URLs:
...
- Alex Hormozi — “8 Entrepreneurs Compete for $100,000 Episode 1”
  - uploaded_count: 5/5 already logged.
  - URLs:
    - https://youtu.be/PCmczb7MGtQ
    - https://youtu.be/rxkZIc4mm4c
    - https://youtu.be/nDeGu8dXAZU
    - https://youtu.be/5SaIMIXq_Nc
    - https://youtu.be/9DyUED-Z2aw
  - Status: OK for that source, but not a new upload this run.

Blockers:
- Classical Echos token is `invalid_grant`, so fresh watchlist manifest seeding failed.
- Trapiistan token is also `invalid_grant`.
- Metrics monitor ran, but live channel stats are blocked by the same token issue.
- Queue is empty because no rendered upload failures remained.
- Remaining deficits are never-rendered/new-distinct-clip backlog, not queued failed uploads.
...
- Then rerun strict Viral Radar and top up partial sources to 5/5.
