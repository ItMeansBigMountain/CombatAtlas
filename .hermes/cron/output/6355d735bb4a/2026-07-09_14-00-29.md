...
Viral Radar strict run: `blocked_source`
...
- Newly discovered creator video processed:
  - Andrew Huberman
  ...
  - Plan: `/opt/data/HeRmEz/projects/viral-clip-radar/CLIP_PLANS/2026-07-09-andrew-huberman-the-science-treatment-of-obsessive-compulsive-disorder-ocd-AtU3EvMzZDY`

- Upload result:
  - Uploaded count: `0`
  - Public Shorts URLs/video IDs: none
  - Required minimum: `10`
  - Failed to reach 10: yes

- Blocker:
  - Could not acquire the exact YouTube source video.
  - `yt-dlp` was skipped by current no-ytdlp setting.
  - `pytubefix` failed: `HTTP Error 403: Forbidden`
...
- Per-source attempted clip hooks:
  - `The Desire Gap Nobody Admits — Huberman / Lab / Essentials`
  - `Why Attraction Gets Messy Fast — Huberman / Lab / Essentials`
  - `The Bedroom Truth People Dodge — Huberman / Lab / Essentials`

- Queue count:
  - Remaining queue count from run: `1`
  - No rendered clip uploads failed, so no new rendered upload URLs were produced.

- Other seeded watchlist candidates found by the pipeline but not processed as the “new creator video(s)” in this strict run:
  - Chris Williamson — `The Emotional Pattern That Predicts Career Failure - Alex Hormozi`
  - Kinobody / Greg O'Gallagher — `The Complete Guide to Fat Loss with GLP-1s (Tirzepatide)`
  - Cobratate / Andrew Tate — `MY_DATA_041013_5_0020`
  - GG33 — `Cue App DOMINATION`
  - Mr Belmar / Nate Belmar — `The truth about transforming yourself`
  - Alex Hormozi — `8 Entrepreneurs Compete For $100,000 - Episode 1`
  - Hamza — `My Insane 4am Daily Schedule To Hit $200k/Month (#8)`
  - Hamza Unfiltered — `I’m not stopping this extreme work season yet.`

- Skipped/blocker notes:
  - TateSpeech / Andrew Tate playlist lookup: `404 playlistNotFound`
  - Luke Belmar: no recent long-form videos found
  - Capital Club / Luke Belmar playlist lookup: `404 playlistNotFound`

Next fix needed: provide YouTube cookies, proxy, pytubefix OAuth/device-login source auth, or a local/direct source MP4 for `AtU3EvMzZDY`, then replay this exact plan.
