...
- **Uploaded URLs:** None
- **New-source result:** No current source was selected; latest-watchlist seeding failed before discovery.
- **Per-source uploads:** No newly discovered sources reached **5/5**.
- **Public uploads this run:** **0**
- **`UPLOAD_QUEUE` count:** **1**
  - Queue was drained first.
  - Retry failed with `invalid_grant`.
  - The exact Chris Williamson rendered clip and metadata remain protected in `UPLOAD_QUEUE`.
- **Never-rendered backlog:** 50 creator-source candidates failed acquisition. These deficits are not represented by the queue.
- **Metrics:** 477 video IDs found, 0 statistics fetched; Classical Echos OAuth is invalid.

### Blockers

1. Classical Echos, Sosai Oyama/Trapiistan, and A F refresh grants are expired or revoked.
2. Watchlist manifest seeding failed because Classical Echos OAuth could not refresh.
3. Existing creator sources encountered YouTube bot confirmation; pytubefix reported unavailable and pytube returned HTTP 400.
4. One attempted candidate was a YouTube Short and cannot support five distinct clips.
...
seed_error: google.auth.exceptions.RefreshError: invalid_grant
```

### Next action

Reauthorize all three profiles using the fresh URLs emitted by:

```bash
python3 /opt/data/scripts/youtube_auth_healthcheck.py --verbose
```

Return each **full localhost callback URL**, exchange and verify the tokens, then rerun the queue-first strict 5-per-source workflow. Source acquisition additionally needs fresh browser cookies, an approved residential proxy, or creator-original local MP4s; repeating the unchanged cloud-IP downloader attempts is not appropriate.

**Artifacts**

- Run state: `/opt/data/HeRmEz/projects/viral-clip-radar/STATE/strict-viral-radar-run-20260821T215031Z.json`
- Raw pipeline log: `/opt/data/HeRmEz/projects/viral-clip-radar/OUTPUTS/strict-cron-logs/2026-08-21T215031Z.log`
- Protected queue metadata: `/opt/data/HeRmEz/projects/viral-clip-radar/UPLOAD_QUEUE/chris-williamson-q-a-becoming-a-dad-health-update-f-02-daily.mp4.upload.json`
