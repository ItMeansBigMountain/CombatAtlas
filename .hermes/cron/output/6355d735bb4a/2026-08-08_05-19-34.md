...
## Viral Radar strict run — **BLOCKED/PARTIAL**

- **Fresh public uploads this run:** 0
- **UPLOAD_QUEUE drained first:** yes
- **Queue count after run:** 0
- **Watchlist manifests seeded:** 10 long-form creator sources
- **Metrics monitor:** OK — 436 video IDs seen, 410 metric records fetched, no OAuth errors
- **Auth/channel preflight:** OK — no token, channel, dependency, or metrics problems
- **Run artifact:** `/opt/data/HeRmEz/projects/viral-clip-radar/OUTPUTS/strict-run-20260808-cron.json`

### Per-source completion

- **Andrew Huberman — Control Your Brain Chemistry**: **0/5**
  - Blocker: all planned candidates were rejected as unavailable for review/selection. The manifest also repeats the first public title for clip 5 and has no transcript enrichment.
- **Chris Williamson — If Your Mind Is Racing, Give It Better Problems**: **0/5**
  - Blocker: source acquisition failed for all five planned clips. Pytubefix returned HTTP 403; plain pytube returned HTTP 400. yt-dlp was disabled by policy.
- **Kinobody — My Lowest Body Fat Score EVER**: **4/5**
  - Existing public clips:
    - https://youtu.be/Zuew8I7lA_Q
    - https://youtu.be/qnGhvASSaQw
    - https://youtu.be/7ydeVYwS9Fg
    - https://youtu.be/9fTpt8qEa7A
  - Blocker: the fifth planned clip repeats clip 1’s public title/hook, so the duplicate guard leaves an unrendered one-clip deficit.
- **Cobratate / Andrew Tate — MY_DATA_041013_5_0020**: **5/5**
  - https://youtu.be/sunS4nZNc2s
  - https://youtu.be/hGfVznx9DJw
  - https://youtu.be/g8xRZizqqi4
  - https://youtu.be/doVi9fN6cV0
  - https://youtu.be/X-P_2DTroxU
- **GG33 — Birthdays that should NEVER touch alcohol**: **0/5**
  - Blocker: no reviewed, nonduplicate manifest clips remained available.
- **Mr Belmar / Nate Belmar — Fitness is Just the Beginning**: **5/5**
  - https://youtu.be/kzkHJJJt3LM
  - https://youtu.be/TH_scJb29uU
  - https://youtu.be/yn743TZkVM8
  - https://youtu.be/qw-XNCuI9Mo
  - https://youtu.be/7RC4EQKv6Qk
- **Alex Hormozi — Answering Your Sales Question Live**: **1/5**
  - https://youtu.be/ebFSvowZJzM
  - Blocker: four-clip unrendered planning/duplicate deficit; the five-window manifest repeats a title and lacks transcript-specific packaging.
- **Hamza — She’s Amazing, But She Has Red Flags**: **0/5**
  - Blocker: no reviewed, nonduplicate manifest clips remained available.
- **Hamza Unfiltered — How to heal trauma**: **0/5**
  - Blocker: no reviewed, nonduplicate manifest clips remained available.
- **Ryan Holiday / Daily Stoic — 11 Final Takeaways From The Odyssey**: **2/5**
  - https://youtu.be/Y0K_tfTHWvI
  - https://youtu.be/tHbKRjCcbAA
  - Blocker: remaining source retrieval attempts failed with pytubefix HTTP 403 and pytube HTTP 400. The three-clip deficit was never rendered, so the empty upload queue is not success.
...
Regenerate the deficient manifests with five **unique, transcript-grounded titles and distinct timestamp windows**, then restore source media using fresh YouTube cookies, the approved Classical Echos pytubefix OAuth session, a residential proxy, or local/direct creator MP4s. Replay only the deficient sources. No failed rendered files need upload retry because **queue_count is 0; the deficits were never rendered or were rejected by duplicate/review guards**.
