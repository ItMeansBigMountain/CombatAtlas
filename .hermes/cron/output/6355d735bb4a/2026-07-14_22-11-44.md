...
BLOCKED/PARTIAL — strict Viral Radar run did not reach the 5-clip minimum.

- Metrics monitor:
  - Status: `ok`
  - Live stats blocked by OAuth token errors.
  - Trapiistan token: `invalid_grant`
  - Classical Echos token: `invalid_grant`

- Upload queue:
  - Drained first: attempted via daily pipeline.
  - Current `UPLOAD_QUEUE` count: `3`
  - I preserved rendered-but-not-uploaded clips in `UPLOAD_QUEUE`:
...
- New uploads this run:
  - None.

- Per-source status:
  - Hamza Unfiltered — “New Hamza analogy for getting girls and success”
    - Uploaded: `0/5`
    - Rendered/queued: `1`
    - Status: `PARTIAL`
  ...
  - TOGI — “Togi on How to Be Successful”
    - Already has older uploads, but this run did not produce/verify a fresh 5-clip batch.
    - Rendered/queued: `1`
    - Status: `PARTIAL`
  - Chris Williamson — “Why Everyone Is Drowning In Debt”
    - Uploaded: `0/5`
    - Status: `BLOCKED_SOURCE`
    - Reason: source acquisition previously produced invalid/corrupt file; pytubefix HTTP 403, pytube HTTP 400.
  - GG33 — “The Dark Truth about Earth and Your Soul”
    - Uploaded: `5/5`
...
- Blockers:
  - YouTube OAuth hard blocker:
    - Classical Echos upload token revoked/expired.
    - Trapiistan upload token revoked/expired.
  - Watchlist seeding blocked because `scripts/seed_latest_longform_manifests.py` defaults to the revoked Classical Echos token.
  - Daily upload command timed out after 600s and did not return successful video IDs.
  - Several selected sources are below the required `5/5`; this is not OK.

- Required next action:
  - Reauth YouTube tokens:
  ...
  - Then rerun:
    ...
    - `VIRAL_RADAR_DAILY_UPLOAD_CAP=100 python3 /opt/data/scripts/viral_radar_daily_upload.py`
