...
- **Uploaded URLs:** None
- **`queue_count`:** 0
- **Metrics monitor:** OK — OAuth metrics active; 457 videos seen, 431 statistics fetched
- **YouTube auth/channel preflight:** OK
- Existing `UPLOAD_QUEUE` was checked first.
- Three long-form creator sources were selected; no filler or placeholder content was used.

### Per-source results

1. **Andrew Huberman — “Using AI to Increase Your Intelligence & Enrich Humanity”**  
   Source: https://www.youtube.com/watch?v=N5AQFYtqx8Q  
   **Uploaded: 0/5**  
   Source acquisition failed: yt-dlp hit the cloud-IP bot check, pytubefix returned HTTP 403, and pytube returned HTTP 400. No clips rendered, so nothing could be queued.

2. **Chris Williamson — “Blue Zone Science Is A Total Scam”**  
   Source: https://www.youtube.com/watch?v=YGAjgLtJJFI  
   **Uploaded: 0/5**  
   All five planned windows were blocked by the same downloader failures. No clips rendered, so nothing could be queued.

3. **Hamza Unfiltered — “Hoes f*ck for free, good girls f*ck for lunch”**  
   Source: https://www.youtube.com/watch?v=1HC76d9RGH4  
   **Uploaded: 0/5**  
   Pipeline reported `no reviewed manifest clips available`. This is an unrendered planning/review deficit, not an empty-queue success.
...
```text
ERROR: [youtube] YGAjgLtJJFI: Sign in to confirm you’re not a bot.
pytubefix: urllib.error.HTTPError: HTTP Error 403: Forbidden
pytube: urllib.error.HTTPError: HTTP Error 400: Bad Request
```
...
Refresh authenticated YouTube browser cookies, configure a residential proxy, or place rights-cleared local MP4s at the manifest source paths for `N5AQFYtqx8Q` and `YGAjgLtJJFI`. Review/reseed Hamza’s five manifest clips, then replay each exact source. Any rendered upload failure must remain in `UPLOAD_QUEUE`.

Report artifact:  
`OUTPUTS/strict-discovered-report-20260811.md`
