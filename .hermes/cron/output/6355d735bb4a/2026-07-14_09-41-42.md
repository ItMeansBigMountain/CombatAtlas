...
BLOCKED/PARTIAL — strict Viral Radar run did not hit 5/5.

- Metrics monitor:
  - Status: ok.
  - But live metrics blocked by revoked/expired OAuth on Trapiistan + Classical Echos.

- Queue drain:
  - Drained existing `UPLOAD_QUEUE` first.
  - Uploaded 1 queued rendered clip:
    - Chris Williamson / “Why The AI Doomers Might Be Right”
    - https://youtu.be/INSnVXMLbjg
  - Queue count after run: `0`

- Per-source result:
  - Chris Williamson — `https://www.youtube.com/watch?v=TNCZJTduDpQ`
  - Uploaded count this run: `1/5`
  - Status: PARTIAL, not OK.

- Seed latest watchlist manifests:
  - Failed.
  - Blocker: YouTube discovery token refresh returned `invalid_grant`.
  - Auth healthcheck also confirms:
    - Classical Echos token: expired/revoked.
    - Trapiistan token: expired/revoked.

- Source/render/upload blockers:
  - Remaining deficit is not in queue because it was never successfully rendered as failed-upload clips.
  - Multiple candidate YouTube sources hit source acquisition blockers:
    - `pytubefix`: HTTP 403
    - `pytube`: HTTP 400
    - `yt-dlp`: skipped by current no-ytdlp path
  - Some rendered attempts produced no new uploads because duplicate guards/log history treated them as already uploaded or not distinct.

- Saved report:
  - `/opt/data/HeRmEz/projects/viral-clip-radar/STATE/strict_viral_radar_cron_2026-07-14_0931Z.md`

- Next action:
  - Reauth upload/discovery tokens:
  ...
  - Then rerun strict Viral Radar with fresh source access/cookies/proxy/local MP4s so each selected long-form source can hit `>=5` real public clips.
