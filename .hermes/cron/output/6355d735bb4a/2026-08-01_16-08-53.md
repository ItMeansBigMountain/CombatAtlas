...
- **Public uploads:** 0
- **Uploaded URLs:** None
- **Per-source result:** Not established/**5**
- **`queue_count`:** 1
- **Metrics:** 383 upload records found; 0 live statistics fetched due to invalid OAuth
- **Report:** `OUTPUTS/strict-daily-result-2026-08-01T1608Z.md`

### Queue-first result

The existing queue was attempted first, but upload authentication failed. The exact rendered clip and metadata remain safely queued:

- **Creator:** Kinobody / Greg O’Gallagher
- **Source:** https://www.youtube.com/watch?v=aGHkzgJFZxI
- **Queued render:** `UPLOAD_QUEUE/kinobody-greg-o-gallagher-a-follower-called-me-out-for-using-tir-auto-01-captioned-daily.mp4`

This old queued clip was **not** counted toward a newly discovered source’s five-clip minimum.

### Newly discovered sources

Latest-watchlist seeding was attempted but failed before selecting a current creator video. Therefore:

- Newly selected sources: **0**
- Clips planned/rendered from a new source: **0/5**
- This is a **never-planned/unrendered deficit**, not an empty-queue success.

### Blockers

1. Classical Echos OAuth token is expired or revoked.
2. Trapiistan/Sosai failover token is also expired or revoked.
3. Watchlist discovery depends on Classical Echos OAuth and could not select a fresh long-form source.
...
https://accounts.google.com/o/oauth2/auth?response_type=code&client_id=984335329962-jmsnmsu79o45n751hdguq832dqool860.apps.googleusercontent.com&redirect_uri=http%3A%2F%2Flocalhost%3A1%2F&scope=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.upload+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.force-ssl+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.readonly+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyt-analytics.readonly&state=SUgF4w8xJOCDdRi6AGpDDlEHsp1gPq&code_challenge=XUUspIIBUXW6cM__bmTJTqzk3K34nqN9tWmXG02ofvk&code_challenge_method=S256&access_type=offline&include_granted_scopes=false&prompt=consent&login_hint=trapiistan%40gmail.com

Return as:

`youtube:trapiistan: <full localhost callback URL>`

After verification, the next run must drain the preserved queue, seed a fresh long-form watchlist source, and upload at least **5 distinct public clips from that exact source**.
