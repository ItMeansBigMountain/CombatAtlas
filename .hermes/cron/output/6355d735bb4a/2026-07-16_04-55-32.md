...
- Uploaded URLs: none.
- Per-source count: no new source selected; **0/5**.
- `queue_count`: **1**
  - Preserved TOGI render: `UPLOAD_QUEUE/togi-togi-on-how-to-be-successful-10-captioned-daily.mp4`
  - Blocker: missing `.upload.json` metadata, so it cannot be safely uploaded or attributed.
- Discovery: failed before watchlist lookup.
  - Classical Echos OAuth: `invalid_grant`
  - Trapiistan OAuth: `invalid_grant`
- Metrics: 466 IDs found, 0 stats fetched due to revoked/expired tokens.
- No filler, placeholders, or old clips were counted toward a new source minimum.
- Run report: `OUTPUTS/strict-viral-radar-run-2026-07-16T045501Z.json`

**Next action**

- Reauthorize both channels:
...
- Reconstruct the TOGI upload metadata and drain it.
- Rerun discovery, then render/upload at least **5/5** clips per selected long-form source.
