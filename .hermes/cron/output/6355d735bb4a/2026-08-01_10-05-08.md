...
- **Public uploads:** 0
- **Uploaded URLs:** None
- **Per-source progress:** Not established/5 — discovery failed before a current source could be selected.
- **`queue_count`:** 1
- **Metrics:** Monitor completed; 383 upload records found, 0 live statistics fetched because OAuth refresh failed.
- **Report:** `OUTPUTS/strict-daily-result-2026-08-01T1004Z.md`

## Queue-first result

The existing queue was attempted first but could not upload. The rendered clip and metadata remain safely preserved:
...
- Duplicate queue creation was prevented.

This older queue item was **not** counted toward the newly discovered source minimum.

## Blockers

1. Classical Echos OAuth token is expired or revoked.
2. Trapiistan/Sosai failover OAuth token is also expired or revoked.
3. Latest-watchlist seeding failed before selecting a newly discovered source.
4. The five-clip deficit was never planned/rendered because discovery was blocked; this is not an empty-queue success.
...
https://accounts.google.com/o/oauth2/auth?response_type=code&client_id=984335329962-jmsnmsu79o45n751hdguq832dqool860.apps.googleusercontent.com&redirect_uri=http%3A%2F%2Flocalhost%3A1%2F&scope=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.upload+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.force-ssl+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.readonly+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyt-analytics.readonly&state=k0LuxgcUj07WjdWRqtxLBGLjtlF0ci&code_challenge=5KTRDClKuLt9G-WZrxdaFKaflLPBvvz_ffkkEZZoBeA&code_challenge_method=S256&access_type=offline&include_granted_scopes=false&prompt=consent&login_hint=trapiistan%40gmail.com

Return as:

`youtube:trapiistan: <full localhost callback URL>`

After verification, the strict run must drain the preserved queue, seed a fresh long-form source, and publish at least **5 distinct public clips from that exact source**.
