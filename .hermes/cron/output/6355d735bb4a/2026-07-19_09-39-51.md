...
**Product status:** `blocked_source` / `blocked_min_uploads_not_met`

- **Public uploads:** 0
- **Uploaded URLs:** None
- **Required minimum:** 5 distinct public clips per selected source
- **UPLOAD_QUEUE drained first:** Yes; no queued uploads existed
- **queue_count:** 0
- **New manifests seeded:** 10
- **Metrics monitor:** OK — 257 videos fetched from 283 logged IDs
- **YouTube auth/channel health:** OK
- **Run log:** `/opt/data/HeRmEz/projects/viral-clip-radar/LOGS/strict-new-source-2026-07-19.log`

### Newly seeded sources

All discovered videos were long enough to support five clips, but none reached source-ready public upload:

- Andrew Huberman — “The Science & Treatment of Bipolar Disorder” — **0/5**
- Chris Williamson — “The True Story Of The Odyssey” — **0/5**
- Kinobody / Greg O’Gallagher — “Why ‘Just Eat More Protein’ Doesn’t Work to Get Lean” — **0/5**
- Cobratate / Andrew Tate — “MY_DATA_041013_5_0020” — **0/5**
- GG33 — “BREAKING NEW ANDREW TATE WAS ARRESTED” — **0/5**
- Mr Belmar / Nate Belmar — “The Truth About Transforming Yourself” — **0/5**
- Alex Hormozi — “Why AI Won’t Make You Rich in 2026” — **0/5**
- Hamza — “Do Not Date The Girl Who Cannot Admit She’s Done Anything Wrong” — **0/5**
- Hamza Unfiltered — party/club guide — **0/5**
- Ryan Holiday / Daily Stoic — “The Fall Of The Roman Republic” — **0/5**

### Blockers

1. **YouTube source acquisition is blocked from this host.**
   - `pytubefix`: HTTP 403
   - plain `pytube`: HTTP 400
   - `yt-dlp`: disabled by current source policy
2. The pipeline exhausted its candidate attempts without acquiring the newly discovered source media.
3. Several old source-ready manifests rendered, but upload returned no public uploads because duplicate guards rejected them. They were correctly **not counted** toward the new-source minimum.
4. `queue_count=0` does **not** mean completion: the deficits were never rendered from the newly discovered sources, so there are no failed rendered clips to queue.
5. Watchlist discovery also reported stale/invalid channel mappings:
   - TateSpeech uploads playlist: 404
   - Capital Club Community uploads playlist: 404
   - Luke Belmar and TOGI: no recent long-form video found

### Raw application result
...
Provide source acquisition through one of the approved routes: refreshed logged-in YouTube cookies, a residential proxy, local/source-ready MP4s, or a working Classical Echos `pytubefix` OAuth device session. Then replay these exact seeded manifests and require **5/5 public uploads per source**. No NASA, space, unknown, filler, or old queued clips were used.
