...
- Fresh watchlist seed: **9 sources**
- Uploaded this run: **0**
- Retry `queue_count`: **0**
- Metrics: OK. OAuth metrics active.
- Auth health: OK.
- Report: `OUTPUTS/strict-viral-radar-run-2026-07-17T100000Z.json`

**Per source**

- Andrew Huberman `UTB5gAkjevk`: **4/5**
  - https://youtu.be/HrdYKG9-qGE
  - https://youtu.be/y-aWhG0yX9A
  - https://youtu.be/lu-6memZ-xw
  - https://youtu.be/7aH7AWAd1GA
- Chris Williamson `U0GSejBI_Bg`: **0/5**
- Kinobody `hIWOrZHI7wc`: **0/5**
- Cobratate `cXJoPxauVEU`: **0/5**
- GG33 `CvwPfe00CPs`: **4/5**
...
- Hamza `5bIQRtsIUz4`: **0/5**
- Hamza Unfiltered `hiBKCshZ9T4`: **0/5**

**Blockers**

- Forced replay exited `2`: minimum not met.
- Missing clips were never rendered or queued.
- Source fallback repeatedly hit pytubefix `403` and pytube `400`.
- Existing title-named source files were not resolved as `source.mp4`.
- One orphan TOGI MP4 has no upload metadata. It was not counted.
- Duplicate guard correctly rejected old rendered candidates.

**Repair applied**

- Fixed watchlist seeding to use the Hermes Python environment with Google API dependencies.
- Verified: `status=ok`, `seeded_count=9`.

**Next action**

- Normalize existing `SOURCES/<video_id>/*.mp4` files to `source.mp4`.
- Finish Huberman and GG33 first.
- Replay every exact 0/5 source.
- Refresh downloader OAuth/cookies or use residential/local source media where files remain unavailable.
