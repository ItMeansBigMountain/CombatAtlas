...
**BLOCKED/PARTIAL — strict 5-clip minimum not met**

- Uploaded this run: https://youtu.be/7aH7AWAd1GA
- Metrics monitor: OK. 397 videos checked via uploader OAuth.
- Auth health: OK.
- Discovery: 9 creator sources seeded.
- Queue count: **1**
  - Old orphan TOGI render without metadata.
  - Does not count toward new-source minimums.
  - No failed renders from today’s selected sources are queued.
...
- Andrew Huberman: **4/5**
  ...
  - Blocker: fifth clip was never rendered or queued.

- Chris Williamson: **0/5**
  - Blocker: source resolution failed. Pytubefix returned 403; pytube returned 400.

- Kinobody / Greg O’Gallagher: **0/5**
  - Blocker: existing title-named MP4 was not resolved as `source.mp4`. Download fallbacks then failed.

- Cobratate / Andrew Tate: **0/5**
  - Blocker: source acquisition failed. Five clips were never rendered.
...
- Hamza: **0/5**
  - Blocker: title-named local MP4 was not resolved. Five clips were never rendered.
...
- Normalize existing `SOURCES/<video_id>/*.mp4` files to `source.mp4`.
- Render and upload the fifth Huberman and GG33 clips first.
- Replay every exact selected manifest until each reaches 5/5.
- Refresh Classical Echos downloader OAuth/cookies or use residential/local acquisition for genuinely missing sources.

- Run report: `OUTPUTS/strict-viral-radar-run-2026-07-17T063005Z.json`
