...
The strict target was not achieved. The pipeline uploaded **3 public clips total**, but no processed source reached **5/5**, and the newly seeded watchlist sources were not rendered or uploaded.

### Public uploads

- Ryan Holiday / Daily Stoic — “Why Autonomy Isn't Always What You Want”  
  https://youtu.be/cEcUtv_Pc2I
- Chris Williamson — “Chad Out Stealing And — The Desire Gap Nobody Admits”  
  https://youtu.be/l3VbijHUBWQ
- Chris Williamson — “Chad Out Stealing And — The Bedroom Truth People Dodge”  
  https://youtu.be/dyWT3i-lQpU

### Per-source results

Uploaded during this run:

- Ryan Holiday, `ngKTRs6ghQ8`: **1/5**
- Chris Williamson, `RSMbltB_sg8`: **2/5**

Newly discovered/seeded sources:

- Andrew Huberman, `s_tkMm5U9aY`: **0/5**
- Chris Williamson, `Pi2WGB1Zdws`: **0/5**
- Kinobody/Greg O’Gallagher, `daHDcz053Wk`: **0/5**
- Cobratate/Andrew Tate, `cXJoPxauVEU`: **0/5**
- GG33, `tZHGULA_55U`: **0/5**
- Mr Belmar/Nate Belmar, `DbYb6oDkHc8`: **0/5**
- Alex Hormozi, `WeH2E1fGgaw`: **0/5**
- Hamza, `XrU_MlxGFRo`: **0/5**
- Hamza Unfiltered, `O1iEamKdvPE`: **0/5**
- Ryan Holiday/Daily Stoic, `28MUVg0oTGg`: **0/5**

All ten seeded videos are long enough to support five candidate clips. Their deficits are therefore backlog/blockers—not “too short” exemptions.

### Queue

- Queue drained first: **yes**
- `queue_count`: **0**
- The empty queue does **not** mean the deficits are complete.
- Remaining clips were never successfully rendered from the newly seeded sources, so there were no failed rendered uploads to queue.

### Blockers

1. Source acquisition repeatedly failed from the cloud host:
   - `pytubefix`: HTTP 403
   - plain `pytube`: HTTP 400
   - yt-dlp was disabled by the configured no-yt-dlp policy.
2. The supposedly strict run drifted into older manifests instead of processing each newly seeded source independently.
3. The two source-ready older videos only reached **1/5** and **2/5** before the source-attempt ceiling was exhausted.
4. TateSpeech and Capital Club watchlist playlist IDs returned `playlistNotFound`; Luke Belmar and TOGI had no recent qualifying long-form result.
5. No NASA, space, unknown-creator, stock, or placeholder clips were used.
...
- Videos seen: `420`
- Statistics fetched: `394`
- Current strongest pattern remains specific Alex Hormozi accountability/business hooks rather than generic titles.

### Artifacts

- Raw run log: `OUTPUTS/strict-cron-logs/2026-08-05T221535Z.log`
- Upload ledger: `UPLOADS/youtube_uploads.jsonl`
- Fresh manifests preserved under `CLIP_PLANS/`

### Next action

Provide source-ready media through fresh YouTube cookies, a residential proxy, or local/direct creator MP4s; then rerun each newly seeded manifest independently with exactly that plan prioritized until it reaches **5/5**. The pipeline’s strict-source selection also needs correction so older manifests cannot displace newly seeded sources.
