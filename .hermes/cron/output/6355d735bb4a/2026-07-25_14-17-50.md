...
Strict discovered-source workflow ran against 10 newly seeded long-form creator videos. Existing `UPLOAD_QUEUE` was checked first and was empty. OAuth/channel health and metrics monitoring passed.

### Public uploads this run

- Andrew Huberman: https://youtu.be/myiyfx1R46A
- Chris Williamson:
  - https://youtu.be/WhjpBugzCZU
  - https://youtu.be/rrd9XUggbOk
  - https://youtu.be/Q_4nwQ4KEJs
  - https://youtu.be/tHo-IePZ1j0
  - https://youtu.be/CxbbBZ7glX0
- Kinobody: https://youtu.be/2F0bvsQwrjk
- Cobratate: https://youtu.be/X-P_2DTroxU
- Daily Stoic: https://youtu.be/ZkwkSulUFPA

### Per-source public totals

- Andrew Huberman `LQI8tl8S2PE`: **5/5**
- Chris Williamson `WmRkGYB0xm0`: **5/5**
- Kinobody `X6oP4016kuE`: **1/5 — PARTIAL**
- Cobratate `cXJoPxauVEU`: **5/5**
- GG33 `_LMTdi9oyhc`: **0/5 — BLOCKED**
- Mr Belmar `BZ_7DELwN7w`: **4/5 — PARTIAL**
- Alex Hormozi `8VqSFBMMS4M`: **4/5 — PARTIAL**
- Hamza `Yoa67bvOAdI`: **4/5 — PARTIAL**
- Hamza Unfiltered `bkUbo-to768`: **0/5 — BLOCKED**
- Ryan Holiday / Daily Stoic `zt5RR2nghzk`: **5/5**

### Queue

- `queue_count=0`
- This does **not** mean the workflow is complete. The outstanding deficits were not rendered because source media could not be restored/downloaded or no unuploaded manifest candidate remained.
- No rendered upload failures occurred, so there was nothing valid to place in `UPLOAD_QUEUE`.

### Blockers

```text
GG33 _LMTdi9oyhc:
pytubefix/pytube source acquisition failed from the cloud host.
Logs: /opt/data/HeRmEz/projects/viral-clip-radar/LOGS/daily_youtube_source_download/_LMTdi9oyhc

Alex Hormozi 8VqSFBMMS4M:
source acquisition blocked by YouTube/cloud-IP checks.
Logs: /opt/data/HeRmEz/projects/viral-clip-radar/LOGS/daily_youtube_source_download/8VqSFBMMS4M

Hamza Yoa67bvOAdI:
source acquisition blocked by YouTube/cloud-IP checks.
Logs: /opt/data/HeRmEz/projects/viral-clip-radar/LOGS/daily_youtube_source_download/Yoa67bvOAdI

Hamza Unfiltered bkUbo-to768:
source acquisition blocked by YouTube/cloud-IP checks.
Logs: /opt/data/HeRmEz/projects/viral-clip-radar/LOGS/daily_youtube_source_download/bkUbo-to768

Mr Belmar BZ_7DELwN7w:
no reviewed, non-duplicate manifest clip remained for the fifth upload.

Kinobody X6oP4016kuE:
only 1 public upload is recorded against the exact discovered source; four source-specific clips remain unfulfilled.
```

Discovery also reported invalid/missing uploads playlists for TateSpeech and Capital Club, while Luke Belmar and TOGI had no recent qualifying long-form video among the latest 50 checked.
...
- Video IDs seen: `359`
- Statistics fetched: `333`
- Token errors: none

### Next action

Restore the exact blocked creator videos using the authenticated Classical Echos pytubefix device session, fresh YouTube cookies/residential proxy, or local/official MP4s. Then regenerate missing non-duplicate windows for Kinobody and Mr Belmar and replay only these deficient source manifests until each reaches **5/5**. No filler or historical queue items were counted.
