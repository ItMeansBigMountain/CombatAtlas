...
**New public uploads this run:** 0  
**UPLOAD_QUEUE drained first:** Yes  
**queue_count:** 0  
**Metrics monitor:** OK — OAuth metrics active; 397 videos fetched  
**YouTube auth/channel health:** OK  
**Discovery:** 9 watchlist source videos seeded with 5 planned clips each

### Per-source progress

- **Andrew Huberman — Bipolar Disorder Essentials (`UTB5gAkjevk`): 4/5**
  - https://youtu.be/HrdYKG9-qGE
  - https://youtu.be/y-aWhG0yX9A
  - https://youtu.be/lu-6memZ-xw
  - https://youtu.be/7aH7AWAd1GA
  - **Blocker:** One-clip deficit was never rendered during this run. Current source acquisition returns pytubefix HTTP 403 and pytube HTTP 400.

- **Chris Williamson — Sinister Plague / Annie Jacobsen (`ibtTcdyMc5Q`): 0/5**
  - **Blocker:** Newly discovered source could not be downloaded; pytubefix returned HTTP 403 and pytube returned HTTP 400.
  - Log: `LOGS/daily_youtube_source_download/ibtTcdyMc5Q/`

- **Kinobody / Greg O’Gallagher — Why More Protein Doesn’t Work (`hIWOrZHI7wc`): 0/5**
  - **Blocker:** Five clips were not rendered. Source resolution/download failed despite a title-named local media file being present.

- **Cobratate / Andrew Tate (`cXJoPxauVEU`): 0/5**
  - **Blocker:** Source acquisition failed with pytubefix HTTP 403 and pytube HTTP 400.

- **GG33 — Tiger Hunting (`CvwPfe00CPs`): 4/5**
  - https://youtu.be/UVgUqLY_56Q
  - https://youtu.be/O-vXk9GQ78Y
  - https://youtu.be/jFUmBNy5S6Y
  - https://youtu.be/dw7awEWT9EY
  - **Blocker:** One remaining clip was never rendered or queued.

- **Mr Belmar / Nate Belmar — Transforming Yourself (`BZ_7DELwN7w`): 5/5**
  - https://youtu.be/z6Secilmqkc
  - https://youtu.be/cfqyHOyEczI
  - https://youtu.be/kSyj0jG8Lvw
  - https://youtu.be/qM1OTDnwnOk
  - https://youtu.be/zUCiHuKjKFQ

- **Alex Hormozi — Reddit Questions (`32fVmwvLmTM`): 5/5**
  - https://youtu.be/5Ta-xkiuqf8
  - https://youtu.be/Lk8Lk4JgHmc
  - https://youtu.be/iIY7ZXllN4k
  - https://youtu.be/S-r5_IjWviA
  - https://youtu.be/A3v-ejSosxs

- **Hamza — Work Like a Maniac (`5bIQRtsIUz4`): 0/5**
  - **Blocker:** Source unavailable; five clips remain unrendered.

- **Hamza Unfiltered — Parties/Clubs Guide (`k71fqlGxWYI`): 0/5**
  - **Blocker:** Newly discovered source remained unavailable; five clips were not rendered.

### Important queue distinction

`queue_count=0` does **not** mean the backlog is complete. No rendered upload failures remain, but **27 required clips are still missing because they were never rendered**, primarily due to source acquisition/resolution failures. They therefore could not enter `UPLOAD_QUEUE`.

The strict runner also attempted source-ready **older manifests**, but duplicate checks rejected them. None were counted toward the newly discovered source minimums, and no filler/NASA/unknown clips were used.

### Next action

Normalize valid `SOURCES/<video_id>/*.mp4` assets to each manifest’s expected `source_file`, then render the exact missing Huberman and GG33 clips first. For absent sources, refresh Classical Echos downloader cookies/pytubefix session or acquire the exact creator videos through a residential/local source. The runner also needs its priority-manifest propagation corrected so a seeded strict run processes only the nine newly selected source videos rather than traversing older manifests.
