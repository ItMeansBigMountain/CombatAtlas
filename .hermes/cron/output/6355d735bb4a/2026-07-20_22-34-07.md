...
**Queue-first:** completed. No uploadable queue items existed.

**New public uploads:** 4
...
- **Andrew Huberman — “Accelerate Learning & Increase Cognitive Capacity”**: **4/5**
  - Four distinct clips rendered and uploaded publicly.
  - The fifth planned clip reused the first clip’s public packaging and did not upload. Its deficit was never rendered/queued.
  - The pipeline then removed the valid local `source.mp4`, incorrectly treating all associated clips as complete. A zero-byte title-named placeholder remains.
...
- **Hamza — “Do Not Date The Girl Who Can Not Admit She’s Done Anything Wrong”**: **4/5**
  ...
  - One never-rendered deficit remains; this source was not reached after the Huberman batch stopped below the run minimum.

- **Hamza Unfiltered — “Today’s teenagers don't get random boners anymore”**: **0/5**
  - No local source media; all five clips remain never-rendered backlog.

- **Chris Williamson — “14 Patterns Behind the World’s Greatest Minds”**: **0/5**
  - Source acquisition remains blocked; only a zero-byte placeholder exists.

- **Kinobody — “The Exact Tirzepatide Dose I Use (And If I’ll Ever Stop)”**: **0/5**
  - Newly seeded during the pipeline refresh but not reached for source acquisition/rendering.

### Queue

`queue_count: 0`

The queue is empty because no **rendered upload** failed. It does **not** mean the missing clips are complete: all remaining deficits were never rendered or were rejected before rendering.
...
```text
Huberman fifth clip source retry:
...
- Videos seen: 304
- Statistics fetched: 278
- Token errors: none

### Next action

Restore/provide source-ready media for the exact deficient videos, regenerate Huberman’s fifth clip with unique transcript-grounded packaging, then process Hamza’s fifth clip and complete Hamza Unfiltered, Chris Williamson, and Kinobody to **5/5 each**. Any subsequently rendered upload failures must remain in `UPLOAD_QUEUE`.

Run log: `OUTPUTS/strict-priority-2026-07-20T223113Z.log`
