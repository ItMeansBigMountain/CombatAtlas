...
## BLOCKED/PARTIAL — Viral Radar strict run

**Uploaded URLs:** None  
**Queue count:** 1  
**Report:** `OUTPUTS/strict-viral-radar-run-2026-07-30T092953Z.json`

### Queue drained first

The queued Kinobody render was attempted before this run at `09:00 UTC` and failed authentication. The fresh preflight confirmed every registered upload profile remains blocked, so the exact rendered clip remains safely preserved:

`UPLOAD_QUEUE/kinobody-greg-o-gallagher-a-follower-called-me-out-for-using-tir-auto-01-captioned-daily.mp4`
...
- **61** creator videos discovered and seeded into plan workspaces.
- **50** are YouTube Shorts and cannot support five distinct transformative clips.
- **11** longer creator videos were found as replacement candidates.
- Authenticated five-window manifest seeding failed before source acquisition/rendering.

### Long-form candidates

All remain **0/5 public uploads**:

- Andrew Huberman — https://www.youtube.com/watch?v=ssP31IenzYA — **0/5**
- Alex Hormozi — https://www.youtube.com/watch?v=0RjIw9M1j28 — **0/5**
- Alex Hormozi — https://www.youtube.com/watch?v=bpIW-dfKFoE — **0/5**
- Alex Hormozi — https://www.youtube.com/watch?v=lidFtb1Ga0o — **0/5**
- Hamza — https://www.youtube.com/watch?v=XrU_MlxGFRo — **0/5**
- Hamza Unfiltered — https://www.youtube.com/watch?v=h6-pnURj9lA — **0/5**
- Hamza Unfiltered — https://www.youtube.com/watch?v=o7VersXNRgA — **0/5**
- Kinobody — https://www.youtube.com/watch?v=X6oP4016kuE — **0/5**
- GG33 — https://www.youtube.com/watch?v=mfD5MpLn9Ac — **0/5**
- GG33 — https://www.youtube.com/watch?v=ZBBNNSHekUM — **0/5**
- Mr Belmar — https://www.youtube.com/watch?v=jzJ5RUXkDuY — **0/5**

These are **never-rendered deficits**, not an empty-queue success.

### Blockers

- `blocked_auth`: Classical Echos token expired/revoked.
- `blocked_auth`: Trapiistan/Sosai token expired/revoked.
- `blocked_auth`: fareed320 token expired/revoked.
- `blocked_seed`: the latest-long-form API seeder could not authenticate.
- `blocked_short_sources`: 50 discovered Shorts are too short for five distinct clips.
- Existing Kinobody backlog remains **0/5**: one rendered clip queued; four clips never rendered/planned.
- Metrics monitor saw 383 IDs but fetched 0 live statistics because of the OAuth failure.

### Required next action

Open an authorization URL and return the **complete localhost callback URL**.

**Classical Echos:**  
https://accounts.google.com/o/oauth2/auth?response_type=code&client_id=984335329962-jmsnmsu79o45n751hdguq832dqool860.apps.googleusercontent.com&redirect_uri=http%3A%2F%2Flocalhost%3A1%2F&scope=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.upload+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.force-ssl+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.readonly+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyt-analytics.readonly&state=n1uSyDqQcxMxvKleyZ9wmMeuhB7CH&code_challenge=GmlwJExatDuJvYpVcrAkqCEU-qzNxf4ixsPPnv6hhvU&code_challenge_method=S256&access_type=offline&include_granted_scopes=false&prompt=consent&login_hint=classicalechos%40gmail.com
...
After reauthorization: drain the exact queued render, rerun five-window seeding, replace the too-short sources with the discovered long-form candidates, and process each selected source to **5/5**.
