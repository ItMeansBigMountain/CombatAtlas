...
BLOCKED/PARTIAL — strict Viral Radar run stopped at auth preflight.
...
- Metrics monitor:
  ...
  - blocker: YouTube OAuth tokens invalid/revoked for:
    - Classical Echos
    - Trapiistan/Sosai

- Watchlist manifest seeding:
  - status: blocked_auth
  - `seed_latest_longform_manifests.py --clips-per-video 5` failed before discovery.
  - Error: `invalid_grant: Token has been expired or revoked`

- Uploads:
  - uploaded URLs: none
  - per-source uploaded_count/5:
    - none selected; latest creator discovery was blocked by OAuth before source selection.
  - queue_count: 0
  - Important: queue is empty because no clips were rendered/upload-attempted, not because deficits are complete.

- Blockers:
  - Classical Echos YouTube token invalid.
  - Trapiistan/Sosai YouTube token invalid.
  - Discovery + upload both blocked.
  - No NASA/space/unknown/old filler used.

- Next action:
  - Reauth Classical Echos first:
    - `python3 /opt/data/scripts/google_reauth_workflow.py youtube-auth-url classicalechos`
  - Reauth Trapiistan/Sosai failover:
  ...
  - Then rerun strict Viral Radar to seed latest source videos and hit `>=5` public clips per selected long-form source.
