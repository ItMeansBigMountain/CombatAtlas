...
PR [#14682 — Add BIS Loadouts](https://github.com/runelite/plugin-hub/pull/14682) remains **open, mergeable, and blocked only pending maintainer review**.

- Submission is correctly isolated to exactly one file: `plugins/bis-loadouts`
...
- Issue comments: only Plugin Hub bot status messages; no correction requested
- The two build annotations are upstream workflow deprecation warnings unrelated to BIS Loadouts
- PR is not merged and no action was needed this run
