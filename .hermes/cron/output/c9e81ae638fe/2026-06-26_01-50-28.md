...
```
{
  "workspace": "/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260626-014556-an-entire-herculaneum-scroll-has-been-read-for-the-first-time",
  "video": "/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260626-014556-an-entire-herculaneum-scroll-has-been-read-for-the-first-time/final.mp4",
  "uploaded": false
}
```
...
- Status: `blocked_auth`
- Metrics monitor: ran OK, but live stats blocked by revoked YouTube token.
  - Token error: `/opt/data/secrets/youtube-trapiistan/youtube_upload_token.json`
  - `invalid_grant: Token has been expired or revoked`

- Newsletter pipeline: generated 1 fresh video from Gmail source.
  - Topic/title: `Jalape o chip , Anthropic accuses Alibaba , Gemini computer use`
  - Video: `/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260626-014642-jalape-o-chip-anthropic-accuses-alibaba-gemini-computer-use/final.mp4`
  - TTS: Google TTS
  - Visuals: Pixabay video assets, manifest preserved.
  - Upload: failed before YouTube upload because OAuth refresh failed.
  - YouTube URL/ID: none returned.
  - Source email: not trashed.

- Verification:
  - `py_compile`: passed for `newsletter_batch_upload.py` and `run_trend_video.py`
  - `ffprobe`: newsletter video is vertical `1080x1920`, audio present, duration `87.35s`
  - Upload JSON/log: no new verified `video_id`

- Fallback wrapper dry-run also ran:
  - Video: `/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260626-015000-an-entire-herculaneum-scroll-has-been-read-for-the-first-time/final.mp4`
  - Upload mode: dry-run
  - Token present: yes, but real upload still blocked by revoked OAuth token.

- Blocker to fix:
  - Re-auth the Trapiistan/Sosai Oyama YouTube OAuth token at `/opt/data/secrets/youtube-trapiistan/youtube_upload_token.json`, then rerun upload.
