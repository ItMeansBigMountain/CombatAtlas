...
Daily Faceless cron report:

- Metrics monitor: ok.
  - OAuth metrics worked.
  - 147 video IDs seen.
  - 143 stats fetched.
- Original script error confirmed:
  - Missing: `/opt/data/scripts/run_faceless_video.sh`
- Newsletter pipeline attempted first:
  - `newsletter_batch_upload.py --limit 1`
  - Blocked by Gmail OAuth:
  - `invalid_grant: Token has been expired or revoked`
  - Profile: `personal-secondary`
- Fallback dry-run render completed:
  - Topic: Hyundai buys Boston Dynamics
  - Hook/title: `Stop Letting Trends Steal Your Discipline`
  - Video path: `/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260620-014627-hyundai-buys-boston-dynamics/final.mp4`
  - Upload status: dry-run only. No `video_id`.
- Verification:
  - `py_compile`: passed.
  - Uploader JSON: returned dry-run JSON, token present.
  - `ffprobe`: audio present, duration `65.85s`.
  - Blocker: rendered `1920x1080` horizontal, not required `1080x1920` vertical.
- Source emails:
  - Not trashed. No verified YouTube `video_id`.
- Research/QA brief saved:
  - `/opt/data/HeRmEz/projects/faceless-youtube-channel/STATE/cron_research_20260620_0146.json`

Blockers to fix before public upload:
- Recreate or update cron wrapper path. Current configured path points to missing `/opt/data/scripts/run_faceless_video.sh`.
- Reauthorize Gmail token for `personal-secondary`.
- Fix fallback renderer orientation to vertical `1080x1920` before allowing upload.
