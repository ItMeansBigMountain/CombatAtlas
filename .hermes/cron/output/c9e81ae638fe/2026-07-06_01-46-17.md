...
```
{
  "workspace": "/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260706-014553-organic-maps",
  "video": "/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260706-014553-organic-maps/final.mp4",
  "uploaded": false
}
```
...
# Read the viral packaging rules
with open('/opt/data/HeRmEz/projects/faceless-youtube-channel/VIRAL_YOUTUBE_SYSTEM.md', 'r') as file:
    viral_packaging_rules = file.read()

# Read performance learnings
performance_learnings_path = '/opt/data/HeRmEz/projects/_ops/social-growth/PERFORMANCE_LEARNINGS.md'
if os.path.exists(performance_learnings_path):
    with open(performance_learnings_path, 'r') as file:
        performance_learnings = file.read()
else:
    performance_learnings = ''

# Run the project pipeline
project_path = '/opt/data/HeRmEz/projects/faceless-youtube-channel'
script_path = os.path.join(project_path, 'scripts/run_trend_video.py')
subprocess.run(['python3', script_path, '--dry-run-upload'])

# Check if the video was uploaded
workspace = '/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260706-014553-organic-maps'
video_path = os.path.join(workspace, 'final.mp4')
uploaded = os.path.exists(video_path)

# Verify the video
if uploaded:
    subprocess.run(['py_compile', video_path])
    subprocess.run(['ffprobe', video_path])
    with open(os.path.join(workspace, 'upload.json'), 'r') as file:
        upload_data = json.load(file)
        if 'video_id' in upload_data:
            video_url = f'https://www.youtube.com/watch?v={upload_data["video_id"]}'
            print(f'Video uploaded successfully: {video_url}')
        else:
            print('Upload failed: video_id not found in upload.json')
else:
    print('Video not uploaded. Check the dry-run output for blockers.')

# Discord report
report = {
    'video_path': video_path,
    'uploaded': uploaded,
    'blocker': 'Check dry-run output for details' if not uploaded else None
}
