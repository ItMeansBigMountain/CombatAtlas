...
```
{
  "workspace": "/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260625-014551-we-re-making-bunny-dns-free",
  "video": "/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260625-014551-we-re-making-bunny-dns-free/final.mp4",
  "uploaded": false
}
```
...
- Status: `blocked_auth`
- Created newsletter video:
  - Topic: `Claude Tag, Seedance 2.5, Mistral OCR 4`
  - Path: `/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260625-014642-claude-tag-seedance-2-5-mistral-ocr-4/final.mp4`
- Upload: failed before YouTube returned `video_id`
  - Blocker: `/opt/data/secrets/youtube-trapiistan/youtube_upload_token.json`
  - Error: `invalid_grant: Token has been expired or revoked`
  - URL: none
- Source email: not trashed. No verified `video_id`.
- Verification:
  - `py_compile`: passed for `newsletter_batch_upload.py` + `run_trend_video.py`
  - TTS/visual preflight: ready
  - Voice used: Google TTS fallback
  - Visuals: Pixabay/Shutterstock assets saved in `visual_manifest.json`
  - `ffprobe`: vertical `1080x1920`, audio present, duration `94.8s`
- Metrics monitor:
  - Ran successfully, but live stats unavailable because same YouTube token is revoked.
- Fallback wrapper dry-run:
  - Ran `run_trend_video.py --dry-run-upload`
  - Dry-run JSON produced, no upload attempted.
  - Path: `/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260625-014954-we-re-making-bunny-dns-free/final.mp4`
- Needed fix:
  - Re-auth Trapiistan/Sosai Oyama YouTube upload token, then rerun upload.
