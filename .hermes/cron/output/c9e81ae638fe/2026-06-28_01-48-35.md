...
```
{
  "workspace": "/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260628-014506-dspark-speculative-decoding-accelerates-llm-inference-pdf",
  "video": "/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260628-014506-dspark-speculative-decoding-accelerates-llm-inference-pdf/final.mp4",
  "uploaded": false
}
```
...
- Status: `blocked_auth`
- Topic/hook: `DSpark: Speculative decoding accelerates LLM inference`
- Rendered video: `/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260628-014757-dspark-speculative-decoding-accelerates-llm-inference-pdf/final.mp4`
- Upload: failed. No YouTube URL/video_id.
- Blocker: YouTube OAuth token is expired/revoked:
...
- Metrics monitor:
  ...
  - `video_ids_seen: 147`
  - `stats_fetched: 0`
  - Same revoked-token blocker.
- Newsletter path:
  - Tried `newsletter_batch_upload.py --limit 1`.
  - It did not upload.
  - Blocked by visual asset gate on scene 6: `The ai team meeting`.
  - No source email was trashed.
- Verification:
  - `py_compile`: passed for `newsletter_batch_upload.py` and `run_trend_video.py`.
  - `ffprobe`: video has audio and is 68.9s.
  - Note: rendered file is `1920x1080` landscape, not `1080x1920` vertical Shorts format.
- Dry-run uploader JSON:
  - Produced valid dry-run body.
  - Public upload intended.
  - Token present but not refreshable.

Next required fix: re-auth the Trapiistan/Sosai Oyama YouTube upload token, then rerun the cron.
