...
Daily Faceless YouTube cron report:

- Metrics monitor:
  - Ran successfully.
  - Live stats blocked by revoked/expired YouTube token:
...
- Script error confirmed:
  ...
  - Actual wrapper exists at:
  - `/opt/data/.hermes/scripts/run_faceless_video.sh`

- Newsletter pipeline:
  - Ran `newsletter_batch_upload.py --limit 1`
  - Blocked before processing.
  - Reason: same revoked/expired Google OAuth token.
  - Processed: `0`
  - Uploaded: `0`
  - No Gmail source emails trashed.

- Fallback dry-run pipeline:
  - Ran `scripts/run_trend_video.py --dry-run-upload`
  - Created dry-run video:
  - `/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260621-014556-cssquake/final.mp4`
  - Topic/source: `CSSQuake`
  - Hook/title: `Stop Letting Trends Steal Your Discipline`
  - Upload status: dry-run only, no public upload.

- Verification:
  - `py_compile`: passed for core scripts.
  - Uploader JSON: produced dry-run JSON.
  - `ffprobe`: audio present, duration ~65s.
  - Blocker: video rendered `1920x1080`, not required vertical `1080x1920`, so it fails Shorts packaging gate.

- Main blockers:
  - Refresh/re-authorize the Trapiistan/Sosai Oyama YouTube/Gmail OAuth token.
  - Fix cron script path from `/opt/data/scripts/run_faceless_video.sh` to `/opt/data/.hermes/scripts/run_faceless_video.sh`, or create the expected wrapper path.
  - Trend fallback renderer needs vertical output before upload.
