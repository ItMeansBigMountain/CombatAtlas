...
```
Script timed out after 120s: /opt/data/scripts/run_faceless_video.sh
```
...
## Daily Stoic cron — partial upload / blocked quota

**Overall status:** `blocked_quota` after 2 verified public uploads.  
The original 120-second scheduler timeout was reproduced as a long-running batch rather than an authentication failure.

### Uploaded publicly to A F

- **Learn To Ask This of Your Actions**  
  https://youtu.be/jpnbE_TE798  
  Source: `personal-secondary / 19f3bd439274f68d`

- **They Have Been Misled**  
  https://youtu.be/zNErjYpYj-E  
  Source: `personal-secondary / 19f8e38d853bfc0a`

Both returned verified `video_id` values. Their source emails were trashed, upload-ledger entries were written, and their generated media directories were deleted.

### Blocked backlog

Nine Daily Stoic messages remain in `BACKLOG_DAILY_STOIC`, with their Gmail messages untrashed:
...
All are classified as `youtube.uploadLimitExceeded`. Failed media was retained; Gmail remains the canonical remake source.
...
- `personal-secondary`: **23** eligible, untrashed Daily Stoic messages
- `personal-main`: **0**

### Repair applied

The pipeline incorrectly treated videos shorter than 45 seconds as hard failures despite the faceless public-upload policy. I changed that duration check to a warning so valid renders proceed to public upload. The stale backlog file for the subsequently successful message `19f8e38d853bfc0a` was removed; historical ledger records remain preserved.

### Health and cleanup

- YouTube auth/channel health check: **OK**
- Expected A F channel enforced: `UCX_nUA3Yr9VR884DNanyMYA`
- Metrics monitor: **OK**, 431 statistics fetched; no token errors
- Successful-upload media: **deleted**
- Failed-render media and active backlog records: **preserved**
- Disk: **28 GB free** (`72%` used)
- Run logs:
  - `OUTPUTS/daily_stoic_cron_2026-08-11.log`
  - `OUTPUTS/daily_stoic_cron_2026-08-11_retry.log`
