...
Daily Faceless YouTube cron: `blocked_auth`

- Script error reported:
  - Missing configured script: `/opt/data/scripts/run_faceless_video.sh`
  - Also no `~/.hermes/scripts/run_faceless_video.sh` found.

- Metrics monitor:
  - Ran successfully.
  - YouTube metrics blocked by revoked Trapiistan token:
...
- Preflight:
  - `py_compile`: OK
  - `ffmpeg/ffprobe`: OK
  - ElevenLabs: key works, but free credits exhausted.
  - Google TTS: OK
  - Stock visuals: OK via Pixabay
  - Support URLs: present

- Newsletter pipeline:
  - Tried `newsletter_batch_upload.py --limit 1 --no-upload`
  - Blocked before render by Gmail OAuth:
    - `personal-secondary` token is expired/revoked.
  - No email trashed.

- Dry-run fallback:
  - Ran `scripts/run_trend_video.py --dry-run-upload`
  - Created: `/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260624-014634-unlimited-ocr-one-shot-long-horizon-parsing/final.mp4`
  - Upload mode: dry-run only.
  - `ffprobe`: 67.14s, audio present, but `1920x1080` horizontal.
  - Quality gate failed for Shorts/newsletter vertical lane, expected `1080x1920`.

- Upload:
  - Not attempted.
  - No YouTube `video_id`.
  - No cleanup/trashing done.

Blockers to fix:
- Restore/recreate `/opt/data/scripts/run_faceless_video.sh` or update cron to the real project wrapper.
- Reauth Gmail profile: `personal-secondary`.
- Reauth YouTube token: `/opt/data/secrets/youtube-trapiistan/youtube_upload_token.json`.
- Fix trend fallback renderer to output vertical `1080x1920` before public upload.
