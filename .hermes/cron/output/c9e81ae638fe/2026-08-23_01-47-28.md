...
```
{
  ...
  "blocked": true,
  "error": "RefreshError",
  "detail": "('invalid_grant: Token has been expired or revoked.', {'error': 'invalid_grant', 'error_description': 'Token has been expired or revoked.'})",
  "profile": "all-personal"
}
```
...
## Daily Stoic faceless-video run: `blocked_auth`

- **Processed:** 0
- **Uploaded:** 0
- **Uploaded URLs:** None
- **Destination:** A F (`fareed320`), expected channel `UCX_nUA3Yr9VR884DNanyMYA`
- **Remaining Daily Stoic source count:** Unknown—Gmail authentication failed before source enumeration.
- **Email cleanup:** No messages were trashed.
- **Media cleanup:** No media was generated, so nothing required deletion. Existing backlog records and upload ledgers were preserved.
- **Metrics monitor:** Completed, but live statistics were unavailable because the relevant OAuth token is revoked.

The requested command was executed:

```text
bash /opt/data/scripts/run_faceless_video.sh
```
...
The A F YouTube token independently failed the live health check with the same `invalid_grant` error. A durable blocker record was written to:

```text
/opt/data/HeRmEz/projects/faceless-youtube-channel/BACKLOG_DAILY_STOIC/all-personal-auth-preflight-20260823T014654Z.json
```

Because Gmail discovery could not start, no real Gmail message ID was available; the record uses `auth-preflight-20260823T014654Z` and explicitly marks source discovery incomplete.

### Reauthorization required

**1. Workspace/Gmail — `personal-secondary` / `fareed320@gmail.com`**

[Authorize Workspace access](https://accounts.google.com/o/oauth2/auth?response_type=code&client_id=984335329962-jmsnmsu79o45n751hdguq832dqool860.apps.googleusercontent.com&redirect_uri=http%3A%2F%2Flocalhost%3A1&scope=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fgmail.readonly+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fgmail.send+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fgmail.modify+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fgmail.settings.basic+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fcalendar+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fdrive+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fcontacts+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fspreadsheets+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fdocuments&state=9Av5MJRxEMLmg54KKd6YbdlwwCXAHs&code_challenge=xQfubIpB7VbAa9IIjCADib2Ara6QHjqfJaJy1CiKNIk&code_challenge_method=S256&access_type=offline&prompt=consent+select_account&include_granted_scopes=true&login_hint=fareed320%40gmail.com)

Return the complete redirect URL as:

```text
workspace:personal-secondary: <full localhost callback URL>
```

**2. YouTube — `fareed320@gmail.com` / A F**

[Authorize A F YouTube access](https://accounts.google.com/o/oauth2/auth?response_type=code&client_id=984335329962-jmsnmsu79o45n751hdguq832dqool860.apps.googleusercontent.com&redirect_uri=http%3A%2F%2Flocalhost%3A1%2F&scope=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.upload+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.force-ssl+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.readonly+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyt-analytics.readonly&state=OChqPQAFQFB1Nt0S0VpXrBnlUOY7kg&code_challenge=XuRiQ2EthuRWB6O3t4U6_GfWyf93zy3sMobH3xPHexc&code_challenge_method=S256&access_type=offline&include_granted_scopes=true&prompt=consent+select_account&login_hint=fareed320%40gmail.com)

Confirm that Google shows the **A F** channel, then return:
