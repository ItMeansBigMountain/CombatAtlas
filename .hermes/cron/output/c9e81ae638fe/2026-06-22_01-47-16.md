...
Daily Faceless YouTube cron report: `blocked_auth`

- Script error confirmed:
  - Missing path: `/opt/data/scripts/run_faceless_video.sh`
  - Actual wrapper exists at: `/opt/data/.hermes/scripts/run_faceless_video.sh`

- Viral system + performance memory read:
  - Applied hook/packaging rules.
  - Metrics monitor ran.
  - Metrics blocked by revoked YouTube token:
    - `/opt/data/secrets/youtube-trapiistan/youtube_upload_token.json`
    - `invalid_grant: Token has been expired or revoked.`

- Preflight:
  - `py_compile`: passed for newsletter/preflight/trend scripts.
  - FFmpeg/ffprobe: ready.
  - ElevenLabs: live, but free tier exhausted: `10000 / 10000 chars`.
  - Google TTS: live.
  - Stock visuals: ready via Pixabay.
  - YouTube upload token: blocked/revoked.
  - Gmail profile `personal-secondary`: blocked/revoked token.

- Newsletter pipeline:
  - Ran: `python3 scripts/newsletter_batch_upload.py --limit 1 --no-upload`
  - Result: blocked before render due Gmail OAuth `invalid_grant`.
  - No source emails trashed.

- Wrapper dry-run fallback:
  - Ran: `python3 scripts/run_trend_video.py --dry-run-upload`
  - Rendered dry-run asset:
    - `/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260622-014627-identity-verification-on-claude/final.mp4`
  - Upload: dry-run only.
  - ffprobe:
    - Duration: `66.04s`
    - Audio: present
    - Size: `1,218,192 bytes`
    - Resolution: `1920x1080`
  - Quality blocker: not vertical `1080x1920`, so not upload-ready for Shorts/newsletter lane.

- Upload status:
  - No public upload.
  - No verified `video_id`.
  - Nothing cleaned up/trash-deleted.

- Needed fix:
  - Reauth Gmail profile `personal-secondary`.
  - Reauth YouTube token for Trapiistan/Sosai Oyama.
  - Fix cron script path from `/opt/data/scripts/run_faceless_video.sh` to `/opt/data/.hermes/scripts/run_faceless_video.sh`.
  - Fix/replace dry-run fallback so it renders vertical `1080x1920`.
