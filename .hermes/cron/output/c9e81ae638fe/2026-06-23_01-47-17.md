...
Daily Faceless YouTube cron result: `blocked_auth` / `blocked_quality`

- Script error confirmed:
  - Missing configured path: `/opt/data/scripts/run_faceless_video.sh`
  - Actual wrapper found at: `/opt/data/.hermes/scripts/run_faceless_video.sh`
  - Wrapper is not executable, so I ran it with `bash`.

- Metrics monitor:
  - Ran successfully.
  - OAuth metrics token is revoked/expired:
    - `/opt/data/secrets/youtube-trapiistan/youtube_upload_token.json`
    - `invalid_grant: Token has been expired or revoked`

- Newsletter upload attempt:
  - Ran `scripts/newsletter_batch_upload.py --limit 1`
  - Blocked before processing/upload:
    - `RefreshError invalid_grant`
  - Processed: `0`
  - Uploaded: `0`
  - No Gmail source emails trashed.

- Dry-run fallback render:
  - Generated video:
    - `/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260623-014625-steam-machine-launches-today/final.mp4`
  - Dry-run uploader JSON:
    - `/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260623-014625-steam-machine-launches-today/result.json`
  - Topic/hook:
...
- Verification:
  ...
  - `ffprobe`: audio present, duration ~65.7s
  - Quality blocker: video is `1920x1080` horizontal, not required Shorts vertical `1080x1920`
  - Upload status: dry-run only, no YouTube URL/video_id

- Main blockers to fix:
  - Reauth YouTube token for Trapiistan/Sosai Oyama upload lane.
  - Fix cron path from `/opt/data/scripts/run_faceless_video.sh` to `/opt/data/.hermes/scripts/run_faceless_video.sh` or add the expected script.
  - Make wrapper executable or call it via `bash`.
  - Fix renderer output orientation to vertical `1080x1920` before public uploads.
