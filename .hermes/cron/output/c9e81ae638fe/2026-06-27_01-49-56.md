...
```
{
  "workspace": "/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260627-014501-previewing-gpt-5-6-sol-a-next-generation-model",
  "video": "/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260627-014501-previewing-gpt-5-6-sol-a-next-generation-model/final.mp4",
  "uploaded": false
}
```
...
Daily Faceless YouTube cron result: `blocked_quality`

- Metrics monitor: ran OK, but live stats blocked by revoked Trapiistan YouTube token.
  - Error: `invalid_grant: Token has been expired or revoked`
- Py compile: passed for:
  - `newsletter_batch_upload.py`
  - `newsletter_video_preflight.py`
  - `run_trend_video.py`
  - `upload_youtube.py`
- Preflight:
  - FFmpeg/ffprobe OK
  - Google TTS OK
  - ElevenLabs key present, but credits exhausted: `10000 / 10000`
  - Stock provider ready via Pixabay
  - YouTube token problem remains from metrics monitor

Newsletter pipeline:
- Ran `newsletter_batch_upload.py`
- Processed 3 newsletter candidates.
- Uploaded: `0`
- Blocker: visual quality gate stopped render.
  - Pexels returned `403`
  - Pixabay/Shutterstock/Wikimedia found no usable asset for required scenes.
  - Example failed queries:
    - `The ai team meeting`
    - `The cybersecurity hacker server room security busy office`
- No source emails were trashed.

Fallback wrapper dry-run:
- Ran: `bash ~/.hermes/scripts/run_faceless_video.sh --dry-run-upload`
- Dry-run video:
  - `/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260627-014921-previewing-gpt-5-6-sol-a-next-generation-model/final.mp4`
- Dry-run uploader JSON returned OK.
- Upload URL/ID: none, dry-run only.
- ffprobe:
  - Video: `1920x1080`, `68.125s`
  - Audio: present, `68.158s`
- Quality issue: not vertical `1080x1920`, so I did not treat it as publish-ready.

No upload happened. No cleanup/trash action taken.
