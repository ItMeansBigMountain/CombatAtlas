...
```
{
  "workspace": "/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260709-014515-decoding-the-obfuscated-bash-script-on-a-uniqlo-t-shirt",
  "video": "/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260709-014515-decoding-the-obfuscated-bash-script-on-a-uniqlo-t-shirt/final.mp4",
  "uploaded": false
}
```

Run the user's Daily Faceless YouTube full video generation + upload pipeline as a full growth loop. First read /opt/data/HeRmEz/projects/faceless-youtube-channel/VIRAL_YOUTUBE_SYSTEM.md and /opt/data/HeRmEz/projects/_ops/social-growth/PERFORMANCE_LEARNINGS.md if present. Apply viral packaging: 1-3s high-contrast hook, no intro, hook→context→receipts→implication→identity/action close, short on-screen captions, visual changes every 2-4s, title/on-screen text/description semantically aligned. Then run the configured project pipeline from /opt/data/HeRmEz/projects/faceless-youtube-channel. Prefer newsletter_batch_upload.py for real newsletter videos when Gmail/Youtube/TTS are ready; otherwise run the wrapper dry-run and report the blocker. Verify with py_compile, ffprobe, and uploader JSON before reporting. Do not trash source emails unless YouTube returns a verified video_id. Keep the final Discord report concise with video path/URL, upload status, and any blocker.

## Response

Daily Faceless YouTube cron result: `blocked_provider`

- Metrics monitor: `ok`
  - OAuth metrics worked.
  - 204 stats fetched.
  - Learnings updated/read.

- Packaging docs read:
  - `VIRAL_YOUTUBE_SYSTEM.md`
  - `PERFORMANCE_LEARNINGS.md`

- Preflight:
  - YouTube/TTS/stock readiness mostly OK.
  - Google TTS OK.
  - ElevenLabs key OK but free credits exhausted.
  - Pixabay stock configured.
  - Pexels returning `403`.
  - No AI video provider key.
  - Preflight said `quality_gate_ready: true`.

- Real newsletter pipeline:
  - Command: `/opt/hermes/.venv/bin/python3 scripts/newsletter_batch_upload.py --limit 1`
  - Failed before render/upload.
  - Blocker: visual gate blocked on scene 3.
  - Query: `Then ai server room`
  - Pexels: `403 Forbidden`
  - Pixabay/Shutterstock/Wikimedia: no downloadable result.
  - No source email was trashed.
  - No YouTube upload happened.

- Wrapper dry-run fallback:
  - Generated dry-run video:
    - `/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260709-014725-decoding-the-obfuscated-bash-script-on-a-uniqlo-t-shirt/final.mp4`
  - Upload mode: dry-run only.
  - Uploaded: `false`
  - Topic/source: “Decoding the obfuscated bash script on a Uniqlo t-shirt”
  - Dry-run title: “Stop Letting Trends Steal Your Discipline”

- Verification:
  - `py_compile`: passed for core scripts.
  - `ffprobe`: passed audio/video presence.
  - Video is `1920x1080`, duration ~67.4s.
  - Audio duration ~67.4s.
  - Uploader JSON returned dry-run metadata, not a public `video_id`.

- Public URL:
  - None. Upload blocked before verified public publish.
