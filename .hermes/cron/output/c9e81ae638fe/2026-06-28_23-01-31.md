...
```
{
  "workspace": "/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260628-225936-marfa-public-radio-puts-you-to-sleep",
  "video": "/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260628-225936-marfa-public-radio-puts-you-to-sleep/final.mp4",
  "uploaded": false
}
```
...
- Newsletter pipeline:
  ...
  - Gmail message: `19f041f9b6802084`
  - Blocker: stock visual gate failed.
  - Pexels returned `403`.
  - Pixabay/Shutterstock/Wikimedia had no usable asset for scene 2.
  - No source email trashed.

- Fallback wrapper dry-run:
  - Created video:
    - `/opt/data/HeRmEz/projects/faceless-youtube-channel/videos/20260628-230101-marfa-public-radio-puts-you-to-sleep/final.mp4`
  - Upload mode: dry-run only.
  - Uploaded: `false`
  - Token present: `true`
  - Proposed title:
    - `Stop Letting Trends Steal Your Discipline`

- Verification:
  - `py_compile`: passed for `newsletter_batch_upload.py` and `run_trend_video.py`
  - `ffprobe`: video has audio.
  - Duration: ~66s.
  - Important issue: output is `1920x1080`, not vertical `1080x1920`, so it does not meet Shorts packaging spec.

- Upload URL/ID: none.
- Cleanup: no generated source/email cleanup performed because no verified `video_id` returned.
