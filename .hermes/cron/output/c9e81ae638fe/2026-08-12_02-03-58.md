...
**Status: `ok_uploaded` with partial `blocked_provider` backlog**

The original scheduled run timed out after 120 seconds:

```text
Script timed out after 120s: /opt/data/scripts/run_faceless_video.sh
```

I reran the Daily Stoic-only workflow to completion from the project directory. YouTube/Gmail authentication and the A F channel guard passed.

### Public uploads to A F (`UCX_nUA3Yr9VR884DNanyMYA`)

- **This Is How You Change** — https://youtu.be/HqAP8aHUl0o  
  Source: `personal-secondary / 19f83ed2bee46ea2`
- **The Wealthiest Person is a Pauper** — https://youtu.be/pL3hSnD1SO8  
  Source: `personal-secondary / 19f7ec6829977417`

Both uploads returned verified `video_id` values, were logged, and their source emails were removed from the eligible Gmail set. Their generated workspaces were deleted immediately after upload.

### Backlogged this run

Eight messages were retained in Gmail and written to durable `BACKLOG_DAILY_STOIC` records:
...
Blocker: stock/API visual acquisition failed during rendering. Pexels returned HTTP 403, while Pixabay, Shutterstock-preview, and Wikimedia fallbacks returned no downloadable result. Most failures occurred for `journaling repeated practice morning`; one failed for `Roman emperor statue dark cinematic`.

Durable records:

- `BACKLOG_DAILY_STOIC/`
- `UPLOADS/daily_stoic_backlog.jsonl`
- Raw preserved run log: `UPLOADS/cron-run-20260812.log`

### Remaining and cleanup

- Remaining eligible Daily Stoic sources: **21**
  - `personal-secondary`: 21
  - `personal-main`: 0
- Uploaded workspaces from this run: deleted
- Failed source emails: retained as canonical remake sources
- Backlog and upload ledgers: preserved
- Existing `videos/` tree: **1.1 GB**; this includes older retained artifacts and was not broadly deleted
- Free disk space: **28 GB**
- Metrics monitor: **OK**, OAuth metrics active; 431 statistics records fetched from 457 known video IDs.
