...
Critical upload contract:
- This job must upload a MINIMUM of 10 public Viral Radar Shorts per source/run when source material is available, not 1.
- A single long-form source must create at least 10 candidate clips and can create/upload up to 50 clips when transcript quality, duration, and clip opportunities support it. Use VIRAL_RADAR_MAX_CLIPS_PER_SOURCE=50.
- 10 is only the floor. Use best judgment based on source duration, transcript quality, and clip opportunities; long/high-signal videos can produce more.
- Upload queued clips first from /opt/data/HeRmEz/projects/viral-clip-radar/UPLOAD_QUEUE/ before creating new clips.
- Upload each clip as it is created. If upload fails or the daily YouTube upload safety cap is reached, queue the rendered clip plus metadata in UPLOAD_QUEUE for the next workflow run; do not delete it.
- Maximize usage without hitting YouTube hidden account-level limits. Default safety cap is VIRAL_RADAR_DAILY_UPLOAD_CAP=12/day for Classical Echos based on observed quota behavior; override only after evidence supports it.
- If fewer than 10 uploads are possible for available long-form material, report blocked_min_uploads_not_met with exact source/render/auth/quota failures and queue counts instead of ok_uploaded.
- Viral Radar uploads must use classicalechos@gmail.com / Classical Echos only.
- Keep hashtags out of titles; put #Shorts and related tags in description/tags.
- Description hashtags must follow what the title + transcript/context says. Keep #Shorts and #ViralRadar, but rotate topic tags such as #Fitness, #DatingAdvice, #Business, #Numerology, #Neuroscience, etc. based on the actual clip.
- Never publish internal planning labels like “the part people will replay” or “clip 1.” Public title/on-screen hook/subtitle must be unique to the video, derived from source title + transcript/summary. Make it edgy, raunchy/catchy when the source supports it, but not explicit, misleading, or generic.
- Description format must be skimmable with blank lines: title, context/subtitle, Source, Original source URL on its own line, edit/attribution note, relevant hashtags.
- Do NOT include cron cohort, run IDs, or other internal metadata in public descriptions.
- Use transcript/context when available for title/description quality.
- Preserve attribution and transformative framing.

Report: metrics status, queued_replay_uploaded_count, remaining_queue_count, daily cap/used count, seeded candidates, uploaded_count, each URL/video_id, failures, and cleanup.

## Response

- Status: `blocked_min_uploads_not_met`
- Main blocker: YouTube upload cap hit on Classical Echos.
  - API error: `uploadLimitExceeded`
  - Message: “The user has exceeded the number of videos they may upload.”
- Metrics monitor: `ok`
  - OAuth metrics working.
  - 268 video IDs seen.
  - 184 stats fetched.
- Auth/channel health: `ok`
  - No token/channel/dependency problems found.
- Daily cap:
  - Configured: `VIRAL_RADAR_DAILY_UPLOAD_CAP=12`
  - Actual: YouTube hidden account upload limit is already reached.
  - Uploaded this run: `0`
- Queue replay:
  - Tried queued/new Kinobody clip first.
  - Upload failed due to quota.
  - Re-queued safely:
    - `/opt/data/HeRmEz/projects/viral-clip-radar/UPLOAD_QUEUE/20260707171632-kinobody-greg-o-gall-the-future-of-fat-loss-is-here-tir-02-daily.mp4`
- Remaining queue:
  - 10 queued MP4s.
  - 18 queue files total including metadata.
- Seeded candidates found in fresh plans:
  - Hamza Unfiltered: 3 clips
  - Chris Williamson: 3 clips
  - GG33: 1 clip
  - Total fresh visible candidates: 7
- Upload URLs/video IDs:
  - None this run.
- Failures:
  - `uploadLimitExceeded`
  - Minimum 10 uploads not met because YouTube rejected the first upload attempt.
- Cleanup:
  - No generated/queued clips deleted.
  - Queue and metadata preserved for next run.
