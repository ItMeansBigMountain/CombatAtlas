...
- **Public uploads:** `0/5`
- **Uploaded URLs:** None
- **Upload queue:** `1` rendered Kinobody/Greg O’Gallagher clip, with metadata preserved
- **Queue replay uploads:** `0`
- **Result artifact:** `/opt/data/HeRmEz/projects/viral-clip-radar/OUTPUTS/strict-daily-result-2026-08-01.md`

### Exact blockers

- **Upload authentication:** Classical Echos and Trapiistan/Sosai OAuth tokens are expired or revoked.
- **Discovery authentication:** Fresh creator seeding failed with the same `invalid_grant`.
- **Source acquisition:** Fresh Alex Hormozi and Andrew Huberman sources failed:
  - `pytubefix`: HTTP 403
  - `pytube`: HTTP 400
  - `yt-dlp`: disabled
- Five older candidates reported one render each but produced no uploads. They were not added as new failed-upload queue entries, so they do not count toward the selected-source minimum.
...
The preceding 120-second scheduler timeout was avoided by running the pipeline with a longer timeout; it completed and returned the blocked result above.
