...
```
{
  ...
  "manifest": "/opt/data/HeRmEz/projects/viral-clip-radar/TMP/daily-cron-manifests/2026-06-21/RbYILvKalxs-viral-radar.json",
...
- Viral Radar run: blocked. No upload happened.
...
- Metrics monitor: ran, but upload token refresh failed:
  - `invalid_grant: Token has been expired or revoked`
  - token: `/opt/data/secrets/youtube-trapiistan/youtube_upload_token.json`
- Candidate refresh: failed for the same revoked/expired Google OAuth token.
- Source download: failed because YouTube required bot/sign-in verification.
...
- Cleanup: no generated/uploaded media to clean.
- Needed fix: refresh the YouTube OAuth token and add either YouTube cookies/proxy/local MP4 source or an official clipping provider key.
