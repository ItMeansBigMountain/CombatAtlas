...
Priority rule: Viral Radar is higher priority than faceless/newsletter YouTube. If there is a Viral Radar queue/backlog of videos to clip or queued clips to upload, the goal is to finish uploading all of them. Replay the upload queue first, then continue clipping/uploading real creator backlog items until the queue/backlog is empty or a real blocker/rate-limit is reached. Use all configured failovers as needed: Classical Echos first, Trapiistan/Sosai on upload-limit failover, then fareed320 if needed/active. Do not preserve capacity for faceless work while Viral Radar has queued real clips.

Working directory: /opt/data/HeRmEz/projects/viral-clip-radar.

Execute /opt/data/scripts/viral_radar_daily_upload.py with FORCE_UPLOAD=1 and VIRAL_RADAR_UPLOAD_QUEUE_FIRST=1. Set VIRAL_RADAR_MIN_UPLOADS high enough to drain the current upload queue when queued clips exist. Report uploaded URLs, remaining queue count, queued failures, and blockers. Do not use filler/placeholder sources.

## Response

Viral Radar cron result: `blocked_auth + blocked_source`

- Queue replay:
  - Remaining upload queue: `0`
  - Queued MP4s: `0`
  - Uploaded from queue: `0`

- Full requested run:
  - Ran `FORCE_UPLOAD=1 VIRAL_RADAR_UPLOAD_QUEUE_FIRST=1 VIRAL_RADAR_MIN_UPLOADS=100`
  - It timed out after `600s`.
  - No active process remained afterward.

- Verification run:
  - Ran with `VIRAL_RADAR_MIN_UPLOADS=10`, `VIRAL_RADAR_MAX_SOURCE_ATTEMPTS=3`
  - Result: `blocked_min_uploads_not_met`
  - Uploaded: `0`
  - Remaining queue: `0`

- Metrics status:
  - Metrics monitor ran.
  - It found `453` logged video IDs.
  - Live stats fetched: `0`
  - Both YouTube tokens are revoked/expired:
    - Classical Echos: `invalid_grant`
    - Trapiistan/Sosai: `invalid_grant`

- Auth healthcheck:
  - `blocked_auth_hardening`
  - Classical Echos upload token invalid.
  - Trapiistan/Sosai upload token invalid.
  - This also blocks fresh Google/YouTube API seeding.

- Source acquisition blockers:
  - Fresh seeding failed from revoked Google token.
  - Several creator YouTube sources hit downloader blockers:
    - `pytubefix`: HTTP `403`
    - `pytube`: HTTP `400`
    - `yt-dlp` was skipped/disabled by current settings.
  - Examples blocked:
    - Chris Williamson / George Mack: `Kbh71hXk6MM`
    - Chris Williamson / Robert Wright: `TNCZJTduDpQ`
    - Chris Williamson / Caleb Hammer: `110TkoGDyC8`

- Render/backlog behavior:
  - Some older real creator manifests rendered, but uploaded `0`.
  - Uploads were empty, likely duplicate-guard/already-uploaded suppression.
  - No filler/NASA/unknown placeholders were used.

Reauth URLs generated:

- Classical Echos:
  - Expected account: `classicalechos@gmail.com`
  - Auth URL:
  `https://accounts.google.com/o/oauth2/auth?response_type=code&client_id=984335329962-jmsnmsu79o45n751hdguq832dqool860.apps.googleusercontent.com&redirect_uri=http%3A%2F%2Flocalhost%3A1%2F&scope=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.upload+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.force-ssl+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.readonly+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyt-analytics.readonly&state=VXmG13Nx6z194pREh8Gn6hQgSJig2N&code_challenge=QDhkcCUYR4MiRg-TdjLEuadiVWkwxh6qu39QbMtWAb4&code_challenge_method=S256&access_type=offline&include_granted_scopes=false&prompt=consent&login_hint=classicalechos%40gmail.com`

- Trapiistan/Sosai:
  - Expected account: `trapiistan@gmail.com`
  - Auth URL:
  `https://accounts.google.com/o/oauth2/auth?response_type=code&client_id=984335329962-jmsnmsu79o45n751hdguq832dqool860.apps.googleusercontent.com&redirect_uri=http%3A%2F%2Flocalhost%3A1%2F&scope=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.upload+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.force-ssl+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.readonly+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyt-analytics.readonly&state=Qrv52QCd1LKJrWkiDTWYjcxl1hy56U&code_challenge=qwojIwlm3CKh0UqZZegR1DAf7Ft_yoAO72G3HVWddLI&code_challenge_method=S256&access_type=offline&include_granted_scopes=false&prompt=consent&login_hint=trapiistan%40gmail.com`

Next unblock:
- Reauth Classical Echos first.
- Then Trapiistan/Sosai.
- Exchange callback with:
  - `python3 /opt/data/scripts/google_reauth_workflow.py youtube-exchange classicalechos '<localhost callback URL>' --verify`
  - `python3 /opt/data/scripts/google_reauth_workflow.py youtube-exchange trapiistan '<localhost callback URL>' --verify`
- Then rerun the strict Viral Radar queue/backlog job.
