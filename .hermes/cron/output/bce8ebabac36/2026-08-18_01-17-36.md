...
- **Uploaded:** `0/5` required public clips
- **Uploaded URLs:** None
- **Protected retry queue:** `1` rendered clip plus metadata
- **Selected source:** Chris Williamson — *Q&A: Becoming A Dad, Health Update & Favourite Peptides*
- **Source URL:** https://www.youtube.com/watch?v=8Oj3NxSLP1U
- **Run report:** `/opt/data/HeRmEz/projects/viral-clip-radar/OUTPUTS/strict-viral-radar-run-2026-08-18T0116Z.json`

### Blockers

- Classical Echos OAuth: expired/revoked
- Sosai Oyama/Trapiistan failover OAuth: expired/revoked
- A F/fareed320 final failover OAuth: expired/revoked
- Fresh source discovery/seeding failed for the same OAuth reason
- The required minimum of five public clips was not met
...
The failed Chris Williamson render remains safely protected at:

`UPLOAD_QUEUE/chris-williamson-q-a-becoming-a-dad-health-update-f-02-daily.mp4`

Its complete upload metadata also remains queued. Ten older Alex Hormozi candidate manifests produced no uploads and were **not** counted toward the selected Chris Williamson source minimum.

### Metrics

- Monitor ran
- Videos found in logs: `477`
- Live statistics fetched: `0`
- Blocker: Classical Echos metrics token returned `invalid_grant`

### Required next action

Reauthorize all three upload profiles, then return each **full localhost callback URL**:

1. **Classical Echos:** run  
   `python3 /opt/data/scripts/google_reauth_workflow.py youtube-auth-url classicalechos`
2. **Trapiistan/Sosai:** run  
   `python3 /opt/data/scripts/google_reauth_workflow.py youtube-auth-url trapiistan`
3. **A F:** run  
   `python3 /opt/data/scripts/google_reauth_workflow.py youtube-auth-url fareed320`

After OAuth exchange and health-check verification, replay the protected queue first and then complete the remaining clips to reach at least `5/5`.
