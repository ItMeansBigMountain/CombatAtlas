...
- **Uploaded this run:** **0/5**
- **Uploaded URLs:** None
- **Queue count:** **0**
- **Auth status:** Healthy; YouTube auth/channel health check returned `status: ok`.
- **Metrics status:** Healthy; 428 video IDs seen, 402 statistics fetched, no token errors.

### Blocker

The strict uploader exceeded both the preceding job’s 120-second limit and this recovery run’s 600-second limit without producing a completed result:
...
The upload ledgers were not modified during this run. Their latest entry remains the previous partial Chris Williamson upload from August 6:

- https://youtu.be/V66VrhTNuAw
- Source: `https://www.youtube.com/watch?v=YV7xNa3QO0c`
- Previous source result: **1/5**, with three planned clips still pending.

The latest preserved pipeline diagnostics identify the underlying source-acquisition blocker:
...
The queue is empty because no newly rendered clip reached an upload failure. The deficit consists of clips that were **never rendered due to source acquisition/pipeline timeout**, so there is nothing valid to place in `UPLOAD_QUEUE`.

### Next action

Run the source-ready backlog using a local/direct creator MP4, fresh authenticated downloader session/cookies, or a residential proxy; then render and upload the remaining four Chris Williamson clips plus enough distinct clips to reach **5/5**. The uploader also needs its long manifest-seeding phase separated from the upload execution so scheduler timeouts cannot erase progress.
