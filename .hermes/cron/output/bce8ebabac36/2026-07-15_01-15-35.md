...
- **Uploaded:** 0/5 public clips.
- **Uploaded URLs:** none.

- **Queue drain check:**
  - `UPLOAD_QUEUE` has 3 rendered MP4s.
  - `*.upload.json` sidecars: 0.
  - Usable queue count: 0.
  - Blocker: queue MP4s are orphaned, so `upload_pending_queue()` ignores them.

- **Auth blocker:**
  - Classical Echos YouTube token: `invalid_grant`, expired/revoked.
  - Trapiistan/Sosai token: `invalid_grant`, expired/revoked.
  - Metrics monitor also confirmed both token errors.

- **Daily upload run:**
  - Full run timed out after 600s.
  - Bounded probe returned `blocked_min_uploads_not_met`.
  - Probe uploaded 0/5.
  - Seed step failed on Classical Echos OAuth refresh.
  - Probe rendered 5 candidate clips but uploaded 0.

- **Queue files preserved:**
  - `hamza-unfiltered-new-hamza-analogy-for-getting-girl-01-daily.mp4`
  - `mr-belmar-nate-belma-the-truth-about-transforming-yours-01-daily.mp4`
  - `togi-togi-on-how-to-be-successful-10-captioned-daily.mp4`

- **Report saved:**
  - `/opt/data/HeRmEz/projects/viral-clip-radar/STATE/strict_daily_run_2026-07-15_report.json`

- **Next action:**
  - Reauth Classical Echos first, then Trapiistan.
  - Recreate upload metadata for the 3 orphaned queue MP4s or map them back to manifests.
  - Rerun with `FORCE_UPLOAD=1`.
  - Do not mark OK until it proves `>=5` public URLs.
