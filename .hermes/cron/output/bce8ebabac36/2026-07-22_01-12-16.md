...
- **Public uploads:** **0/5**
- **Uploaded URLs:** None
- **Upload queue:** **0 before → 0 after**
- **Pipeline result:** `blocked_min_uploads_not_met` — exit code `2`
- **Discovery:** Succeeded; **10 creator videos seeded**
- **Metrics monitor:** Healthy — 284 videos fetched, no token errors
- **YouTube auth/channel health:** Healthy; no OAuth problems detected

### Blockers

- Source acquisition failed across selected YouTube videos:
  - `yt-dlp` was disabled by pipeline policy.
  - `pytubefix` encountered player-JS interpretation errors or HTTP 403.
  - Plain `pytube` returned HTTP 400.
- Some source-ready/previously rendered candidates were processed, but none produced public uploads. Duplicate/non-upload outcomes do not count toward the fresh-source minimum.
- The queue is empty because no rendered upload failure reached queueing. The missing clips were not successfully acquired/rendered/uploaded; an empty queue does **not** mean the backlog is complete.
- TateSpeech and Capital Club playlist IDs returned 404. Luke Belmar and TOGI had no recent eligible long-form videos.
...
Refresh Classical Echos downloader cookies/device OAuth, provide a residential proxy, or stage local source MP4s. Then replay the exact newly discovered manifests and produce at least five distinct public clips per selected source.

Report saved to:

`/opt/data/HeRmEz/projects/viral-clip-radar/OUTPUTS/strict-viral-radar-run-2026-07-22T011148Z.json`
