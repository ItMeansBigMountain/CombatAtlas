...
**Viral Radar — BLOCKED/PARTIAL (`blocked_source`)**

- **Selected source:** Hamza Unfiltered — “Hoes f*ck for free, good girls f*ck for lunch”
- **Source URL:** https://www.youtube.com/watch?v=1HC76d9RGH4
- **Uploaded:** **0/5**
- **Uploaded URLs:** None
- **Retry queue:** **0 files**
- **Metrics monitor:** OK — OAuth metrics active; 457 videos found and 431 statistics fetched.
- **YouTube auth/channel preflight:** OK; no token, scope, or channel-identity problems.
- **Prior feeder error:** `/opt/data/scripts/viral_radar_daily_upload.py` timed out after 120 seconds.
- **Current direct run:** completed with exit code 2 and `blocked_min_uploads_not_met`.

**Exact blocker:** YouTube source acquisition failed before rendering. The local source is a zero-byte placeholder and cannot pass `ffprobe`; therefore no clips were rendered and there are no failed rendered clips to place in `UPLOAD_QUEUE`.

```text
ERROR: [youtube] 1HC76d9RGH4: Sign in to confirm you’re not a bot.
pytubefix: urllib.error.HTTPError: HTTP Error 403: Forbidden
pytube: urllib.error.HTTPError: HTTP Error 400: Bad Request
```

The empty queue does **not** mean the source was completed: all five required clips remain never-rendered deficits.

**Next action:** restore source access using fresh authenticated YouTube browser cookies, a residential proxy, or a rights-cleared local MP4 for this exact video; validate it with `ffprobe`, then replay the source until it reaches 5/5. No filler or unrelated backlog clips were used.

**Run log:**  
`/opt/data/HeRmEz/projects/viral-clip-radar/OUTPUTS/cron-strict-20260812T010359Z.log`
