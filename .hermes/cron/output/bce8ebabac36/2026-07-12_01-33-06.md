...
- Ran metrics first:
  - Metrics monitor: `ok`
  - Viral Radar uploads tracked: `233`
  - Current winners still skew Alex Hormozi / Luke Belmar / GG33.

- Ran strict Viral Radar command:
  - `FORCE_UPLOAD=1`
...
- Upload queue:
  ...
  - `UPLOAD_QUEUE_HOLD`: `1`
  - Held item: Alex Hormozi clip, blocked by prior YouTube `uploadLimitExceeded`.

- Uploaded URLs this run:
  - None confirmed.

- What happened:
  - First run hung on a YouTube source download and hit timeout.
  - I verified the partial source for GG33 `sHPtBc1LPyo` became a valid MP4 after the interrupted attempt.
  - Re-ran the pipeline.
  - It attempted the real creator backlog, no filler/NASA/unknown sources.
  - It exited with code `2` because it could not reach the minimum upload count.

- Main blocker:
  - YouTube source acquisition is failing across creator backlog.
  - Common errors:
    - `pytubefix`: `HTTP Error 403: Forbidden`
    - `pytube`: `HTTP Error 400: Bad Request`
    - `yt-dlp`: skipped by current no-yt-dlp mode / env.
  - Affected real creator sources included Huberman, Hamza, Kinobody, GG33, Luke Belmar, Mr Belmar, Cobratate.

- Remaining queue/failures:
  - Active upload queue: `0`
  - Held quota-failed clip: `1`
  - New rendered failed-upload queue items: none found.
  - Source download logs are under:
...
- Next unblocker:
  - Need one of:
    ...
    - official/direct creator media mirrors,
    - or allow the downloader ladder to use yt-dlp again where acceptable.
