...
- **Uploaded URLs:** None
- **Upload queue:** **0**
- **Discovery:** 10 source-video plans seeded
- **Source failures:** 46
- **Render failures:** 4
- **YouTube auth/channel health:** OK
- **Metrics monitor:** OK — 436 upload records seen, 410 statistics fetched

### Blockers

- Source acquisition failed across the candidate fallback set:
  - `pytubefix`: YouTube **HTTP 403 Forbidden** or **400 Bad Request**
  - `pytube`: **HTTP 400 Bad Request**
  - `yt-dlp`: disabled by the configured no-yt-dlp policy
- Four locally referenced source files were missing:
  - `SOURCES/UHMBbvyJYpE/source.mp4`
  - `SOURCES/47lEBX2e_uA/source.mp4`
  - `SOURCES/s8RSjblOel4/source.mp4`
  - `SOURCES/F7EICO5PXzU/source.mp4`
- TateSpeech and Capital Club discovery also returned invalid/missing YouTube upload-playlist IDs.
- Luke Belmar and TOGI had no recent qualifying long-form videos among the 50 checked.
...
"source_failure_count": 46
"render_failure_count": 4
```

The queue is empty because no clips reached a rendered-upload-failure state; the deficits occurred during source acquisition or before rendering. Nothing was uploaded, so no failed rendered media needed queueing.

**Next action:** provide fresh authenticated YouTube cookies, a residential proxy, or local/direct source MP4s for the seeded creator videos, then rerun the same plans. Full preserved log:

`/opt/data/HeRmEz/projects/viral-clip-radar/OUTPUTS/cron_logs/strict_daily_20260809T0104Z.log`
