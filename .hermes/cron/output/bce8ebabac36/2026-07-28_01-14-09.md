...
- **Public uploads:** `0/5`
- **Uploaded URLs:** None
- **Queue:** `1` rendered clip plus its upload metadata remains safely in `UPLOAD_QUEUE`
- **Queued source:** Kinobody / Greg O’Gallagher — *A Follower Called Me Out For Using Tirzepatide*
- **Source URL:** https://www.youtube.com/watch?v=aGHkzgJFZxI
- **Blockers:**
  - Classical Echos OAuth token: expired/revoked (`invalid_grant`)
  - Trapiistan/Sosai OAuth token: expired/revoked (`invalid_grant`)
  - Queue upload failed and remained queued as required
  - The full daily pipeline timed out after 600 seconds while handling the blocked queue; therefore no new source reached a proven 5-clip result
  - Metrics monitor found 383 logged videos but fetched no live statistics because Classical Echos authentication failed
...
<https://accounts.google.com/o/oauth2/auth?response_type=code&client_id=984335329962-jmsnmsu79o45n751hdguq832dqool860.apps.googleusercontent.com&redirect_uri=http%3A%2F%2Flocalhost%3A1%2F&scope=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.upload+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.force-ssl+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.readonly+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyt-analytics.readonly&state=prwDNcv2NHx4rJLF1pifrclniUItqh&code_challenge=Kd6Ax51OET1IRmhrOzGb32Z7GvwMtZI1plj9mt9nZYQ&code_challenge_method=S256&access_type=offline&include_granted_scopes=false&prompt=consent&login_hint=trapiistan%40gmail.com>
...
**Next action:** exchange and verify both callbacks, drain the preserved queued Kinobody clip first, then replay the exact-source pipeline. This run is not successful because it produced `0/5` public uploads.
