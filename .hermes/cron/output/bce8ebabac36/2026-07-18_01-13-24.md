...
- **Uploaded URLs:** None
- **Upload queue:** **0**
- **Metrics monitor:** OK — OAuth metrics active; 397 videos fetched from 491 IDs
- **YouTube auth/channel health:** OK
- **Discovery:** 9 creator videos seeded

### Blockers

- Source acquisition failed for **47 distinct creator videos**:
  - `yt-dlp` was disabled by current policy/environment.
  - `pytubefix` returned HTTP 403 on 46 attempts.
  - Plain `pytube` returned HTTP 400 on 46 attempts.
- Three source-ready candidates rendered, but all three were rejected by the pre-upload **duplicate guard**. Duplicate skips do not count toward the five-clip minimum.
- There were **no upload API failures**, so nothing new was placed in `UPLOAD_QUEUE`. The queue is empty because the remaining clips were never rendered due to source failures or were duplicates—not because failed rendered uploads were lost.
- TateSpeech and Capital Club have invalid/missing YouTube upload-playlist IDs. Luke Belmar and TOGI had no recent long-form candidates.
- The preceding collection invocation also timed out after 120 seconds; this full replay completed with `blocked_min_uploads_not_met`.

### Next action

Restore source access using fresh logged-in YouTube cookies, a residential proxy, local/official MP4s, or the approved Classical Echos `pytubefix` OAuth downloader session. Then rerun with the five-upload minimum and repair the TateSpeech/Capital Club registry IDs.

Report saved to:

`/opt/data/HeRmEz/projects/viral-clip-radar/STATE/strict_daily_run_2026-07-18_report.json`
