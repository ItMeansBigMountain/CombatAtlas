...
- Metrics monitor: `ok`
  - Viral Radar median public views: `2`
  - Current winners: Alex Hormozi, Luke Belmar, GG33 style clips.
- Queue-first run executed:
  - `FORCE_UPLOAD=1`
  - `VIRAL_RADAR_UPLOAD_QUEUE_FIRST=1`
  - `VIRAL_RADAR_MIN_UPLOADS=14`
  - `VIRAL_RADAR_DAILY_UPLOAD_CAP=100`

- Uploaded: `1`
  - Alex Hormozi: `Download Free Scaling Roadmap — The Money Mistake That Looks Smart`
  - URL: https://youtu.be/g50xyVeC4tA
  - Source: https://www.youtube.com/watch?v=j2TZMFkj71Q
  - Privacy: `public`
  - Render cleanup confirmed: exported MP4 deleted after upload.

- Queue replay:
  - `queued_replay_uploaded_count`: `0`
  - Script-reported remaining upload queue: `0`
  - Existing influencer source queue file still has `14` entries:
    - `/opt/data/HeRmEz/projects/viral-clip-radar/OUTPUTS/viral-radar-influencer-source-queue-2026-06-29.json`

- Fresh discovery seeded `9` creator videos:
  - Andrew Huberman — OCD science/treatment
  - Chris Williamson — Caleb Hammer debt/income
  - Kinobody/Greg — GLP-1 fat loss
  - Cobratate/Andrew Tate — older long-form source
  - GG33 — stream with Hassan
  - Mr Belmar/Nate Belmar — transforming yourself
  - Alex Hormozi — Fix Your Business in 90 Days
  - Hamza — 4am schedule
  - Hamza Unfiltered — “She’s not into you”

- Blockers:
  - Main blocker: YouTube source acquisition.
  - Multiple real creator sources failed download.
  - Pattern:
    - `yt-dlp` skipped by config / disabled path.
    - `pytubefix` hit `HTTP 403 Forbidden` or `HTTP 400`.
    - `pytube` hit `HTTP 400 Bad Request`.
  - Needed next source options from logs:
    - YouTube cookies
    - browser cookies
    - residential proxy
    - official/direct creator mirror
    - pytubefix OAuth/device login
    - local/source-ready MP4s

- Upload/auth health:
  - Tokens did not show as the immediate upload blocker.
  - Healthcheck returned `blocked_auth_hardening` because code guard text is missing:
    - `/opt/data/HeRmEz/projects/viral-clip-radar/scripts/upload_to_youtube.py`
    - Missing snippet: `Classical Echos upload limit hit; failed over to`

- No filler/placeholder sources were used.
- No NASA/space/unknown fallback was uploaded.
- Final product status: `blocked_source` after `1` real public upload.
