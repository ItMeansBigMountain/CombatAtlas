...
```
{
  ...
  "marker": "/opt/data/HeRmEz/projects/viral-clip-radar/STATE/viral_radar_daily_upload_2026-07-16.done",
...
- Uploaded URLs: none
- Queue count: **1**
  - Old orphan TOGI render.
  - Missing upload metadata.
  - Already published as https://youtu.be/22SjZImdGZY
  - Preserved, but duplicate guard correctly prevented re-upload.
- Discovery found **9 long-form creator videos**, including Huberman, Chris Williamson, Kinobody, Tate, GG33, Nate Belmar, Hormozi, and Hamza.
- Main blocker: **source download/auth**
  - `pytubefix`: HTTP 403
  - `pytube`: HTTP 400
  - `yt-dlp`: disabled by policy/config
  - No fresh source clips could be rendered and uploaded.
- Render failures: none.
- Upload quota/rate limit: not reached.
- YouTube OAuth/channel health: **OK**
- Metrics monitor: **OK**
  - 393 videos fetched.
  - Viral Radar median: 2 views.
  - Current winner: Hormozi blame/power clip at 1,335 views.
- Next action: provide fresh YouTube cookies, a residential proxy, or local/direct source MP4s, then replay all nine discovered source plans and publish at least five distinct clips per source.
