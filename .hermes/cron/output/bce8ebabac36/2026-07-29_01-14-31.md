...
- **Uploaded:** `0/5`
- **Public URLs:** None
- **Queue:** **1 rendered clip** plus its upload sidecar remain safely preserved.
- **Queued source:** Kinobody / Greg O’Gallagher — *A Follower Called Me Out For Using Tirzepatide*
- **Source:** https://www.youtube.com/watch?v=aGHkzgJFZxI
- **Per-source status:** **0/5**
  - 1 rendered clip failed upload and remains queued.
  - 4 additional clips were never rendered/planned; they are not represented by the queue count.
- No NASA/space/unknown/filler content was used.

### Exact blockers

1. **Upload authentication:** Both configured upload profiles return `invalid_grant`.
   - Classical Echos token expired or was revoked.
   - Trapiistan/Sosai token expired or was revoked.
   - fareed320 has no verified token available.
2. **Discovery/metrics:** OAuth failure prevented current watchlist statistics; metrics saw 383 logged videos but fetched 0 live statistics.
3. **Pipeline timeout:** The strict upload command was run for 600 seconds, after the preceding cron’s 120-second timeout, but timed out again with buffered/empty output.
4. **Minimum deficit:** No selected source reached the required five public uploads.

```text
youtube_auth_healthcheck status=blocked_auth_hardening
trapiistan refresh_error=invalid_grant: Token has been expired or revoked.
classicalechos refresh_error=invalid_grant: Token has been expired or revoked.
