...
- ok_uploaded: processed 3 unread approved TLDR/source emails from `personal-secondary` / `fareed320@gmail.com`.
- Uploaded public to Sosai Oyama / Trapiistan token:
  - `US vs. OpenAI, state of AI economy, scaling laws`
    - https://youtu.be/WE-Ibg1fTwY
    - duration: 118s
    - source email trashed after verified `video_id`
  - `Polymarket $3M Hack, Klue Data Extortion, GasLight macOS Bug`
    - https://youtu.be/epH3MJygVpM
    - duration: 272s
    - source email trashed after verified `video_id`
  - `Solopreneur boom, AI era experiments, new moats`
    - https://youtu.be/N9Xp4eWV3IQ
    - duration: 180s
    - source email trashed after verified `video_id`

- Checks passed:
  - Gmail token valid for `fareed320@gmail.com`.
  - YouTube token verified as `Sosai Oyama` / `UCsxzQlusqwmMUdjMvKAJDfA`.
  - Google TTS live.
  - Pixabay stock provider live.
  - ffprobe confirmed 1080x1920 video + audio.

- I also fixed the visual-search query fallback in the newsletter renderer.
  - The first render hit a quality gate because stock providers rejected an overlong query.
  - I changed it to retry short stock-style queries before blocking.
