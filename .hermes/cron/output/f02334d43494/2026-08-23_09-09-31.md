...
- **hold_queue:** `1`
- **pipeline_exit:** `2`
- **Durable log:** `/opt/data/HeRmEz/projects/viral-clip-radar/OUTPUTS/backlog-processor-logs/2026-08-23T090007Z.log`

The rendered retry clip and its metadata remain protected in `UPLOAD_QUEUE`; nothing was deleted:

- `chris-williamson-q-a-becoming-a-dad-health-update-f-02-daily.mp4`
- `chris-williamson-q-a-becoming-a-dad-health-update-f-02-daily.mp4.upload.json`

The hold queue also retains one Alex Hormozi clip.

### Blockers

1. **Classical Echos discovery OAuth expired/revoked**
2. **YouTube source acquisition blocked:** yt-dlp repeatedly encountered bot verification; pytubefix reported videos unavailable; pytube returned HTTP 400.
3. No candidate could be rendered/uploaded, so the required minimum was not met.
