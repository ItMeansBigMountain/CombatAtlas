...
- **queue_after:** `0`
- **hold_queue:** `1`
- **pipeline_exit:** `124` — wrapper exceeded the required 600-second execution timeout
- **Durable raw log:** `OUTPUTS/backlog-processor-logs/2026-08-16T090037Z.log` — created but remained empty because application output was buffered before termination
- **No backlog process remains running**
- **No failed rendered clips were deleted**

### Product result

The queue was already empty. The pipeline discovered three real influencer sources, but uploaded **0/15 required clips**:

- Andrew Huberman: `0/5` — https://www.youtube.com/watch?v=wbuPQPu-03Y
- Hamza: `0/5` — https://www.youtube.com/watch?v=ne-hYpiOnF0
- Hamza Unfiltered: `0/5` — https://www.youtube.com/watch?v=Egwjll72fuU

These are never-rendered source/planning deficits, not upload-queue failures. The existing hold item remains protected in `UPLOAD_QUEUE_HOLD`.

### Blocker: `blocked_source` plus planning deficit

Raw source-acquisition errors:

```text
ERROR: [youtube] wbuPQPu-03Y: Sign in to confirm you’re not a bot. Use --cookies-from-browser or --cookies for the authentication.
```

```text
pytubefix.exceptions.BotDetection: wbuPQPu-03Y This request was detected as a bot.
```
...
The generated manifests lacked transcripts and contained generic description-derived hooks, so the review gate correctly prevented misleading clips from being rendered or published. YouTube upload OAuth and channel identity checks passed; therefore, no OAuth recovery URL is required.

Detailed run report: `OUTPUTS/strict-discovered-report-20260816-run2.md`
