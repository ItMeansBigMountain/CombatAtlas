...
- `blocked_auth`
- Stopped safely before rendering/uploading.

What I verified:
- Gmail profile works:
  - `personal-secondary`
  - email: `fareed320@gmail.com`
  - unread source candidates found: `18`
- TTS is ready:
  - ElevenLabs auth OK, but free credits are exhausted: `10000 / 10000`
  - Google TTS OK and usable
- Stock visuals are ready:
  - Pixabay key present
  - Pexels missing
  - ffmpeg/ffprobe OK
- Metrics monitor ran, but Trapiistan/Sosai YouTube token is revoked.

Exact blocker:
- YouTube upload token for Sosai/Trapiistan is invalid:
  - token: `/opt/data/secrets/youtube-trapiistan/youtube_upload_token.json`
  - error: `invalid_grant: Token has been expired or revoked`
  - expected channel: `Sosai Oyama`
  - expected channel id: `UCsxzQlusqwmMUdjMvKAJDfA`

Why I did not continue:
- The job requires public upload only after quality gates pass.
- Source emails can only be trashed after a verified YouTube `video_id`.
- With upload auth revoked, rendering videos now would risk stale backlog artifacts and no safe cleanup path.

Next repair command:
- `python3 /opt/data/scripts/google_reauth_workflow.py youtube-auth-url trapiistan`

Sample queued unread candidates:
- TLDR AI: `US vs. OpenAI, state of AI economy, scaling laws`
- TLDR InfoSec: `Polymarket $3M Hack, Klue Data Extortion, GasLight macOS Bug`
- TLDR Founders: `Solopreneur boom, AI era experiments, new moats`
- TLDR IT: `AI’s Infrastructure Crunch, The Undersea Internet Lockdown`
- TLDR Crypto: `Bitcoin recovering, Post-Quantum Private ETH, Fed Crypto Payment Accounts`
