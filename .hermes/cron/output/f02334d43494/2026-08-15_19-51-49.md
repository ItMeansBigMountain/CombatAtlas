...
- **queue_after:** 0
- **hold_queue:** 1
- **pipeline_exit:** 124 — command exceeded the required 600-second timeout
- **Durable log:** `OUTPUTS/backlog-processor-logs/2026-08-15T194106Z.log`
- **Durable log status:** Empty (0 bytes); the Python process did not flush output through `tee` before termination.
- **Failed rendered clips deleted:** No

### Blocker

The run spent its execution window attempting source acquisition across discovered influencer videos. No clips were rendered or uploaded. Current attempts failed through all tested YouTube download paths, leaving zero-byte source files. Representative raw errors:

```text
ERROR: [youtube] 8Oj3NxSLP1U: Video unavailable
```
...
This is a **blocked_source / pipeline timeout**, not upload-auth failure, so no fresh OAuth recovery URL is applicable.

The protected hold queue still contains one rendered Alex Hormozi clip. Its recorded upload blocker is:
...
That held clip and its complete upload metadata remain preserved under `UPLOAD_QUEUE_HOLD`; nothing was deleted.
