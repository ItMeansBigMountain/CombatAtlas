...
- Exact blocker:
  - Gmail source token is revoked/expired for `personal-secondary` / `fareed320@gmail.com`.
  - YouTube upload token is revoked/expired for `trapiistan` / Sosai Oyama.
- Verified with live OAuth probes:
  - Workspace: `invalid_grant: Token has been expired or revoked`
  - YouTube: `invalid_grant: Token has been expired or revoked`
- Provider readiness:
  - FFmpeg/ffprobe: OK
  - Google TTS: OK
  - ElevenLabs key responds, but free-tier credits are exhausted.
  - Stock visuals: OK via Pixabay.
- I did not trash any emails.
- I did not upload placeholders/static/flite drafts.
...
After those callbacks are exchanged, the cron can resume one-email → one-public-video processing.
