...
- Status: `blocked_auth`
- I did not render or upload anything.
- I did not trash any Gmail source emails.

Blocker:
- Sosai / Trapiistan YouTube token is revoked/expired.
...
Workspace status:
- Gmail profile `personal-secondary` works.
- Verified email: `fareed320@gmail.com`
- Gmail probes passed.

Provider preflight:
- `EllevenLabsKey`: present.
- `PIXABAY_API_KEY`: present.
- `GOOGLE_APPLICATION_CREDENTIALS`: present.
- Pexels key missing, but Pixabay is approved fallback.

Metrics note:
- Metrics monitor ran.
- It also hit the same Trapiistan YouTube token refresh failure.

Reauth needed:
- Open this URL while signed into `trapiistan@gmail.com`.
- After approval, copy the full localhost redirect URL.

`https://accounts.google.com/o/oauth2/auth?response_type=code&client_id=471944085234-kgi6lv4ib5lvfcdiarv2olg6gratiduj.apps.googleusercontent.com&redirect_uri=http%3A%2F%2Flocalhost%3A5000%2F&scope=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.upload+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.force-ssl+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyoutube.readonly+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fyt-analytics.readonly&state=TNUtLmVkgEiSrHnaCoUHNVjJqjZLC1&code_challenge=b6Z0SPAnHP6RjTYm8pNfl4JQezvf06LyrING6hWJOJ0&code_challenge_method=S256&access_type=offline&include_granted_scopes=false&prompt=consent&login_hint=trapiistan%40gmail.com`

Callback format to return:
- `youtube:trapiistan: <full localhost URL>`

Once that token verifies, the newsletter job can safely resume public uploads and only trash source emails after confirmed `video_id`.
