...
- **hold_queue:** 1
- **pipeline_exit:** 3
- **Durable log:** `/opt/data/HeRmEz/projects/viral-clip-radar/OUTPUTS/backlog-processor-logs/2026-08-02T090950Z.log`
- **Failed rendered clips deleted:** None

A separate Viral Radar uploader was active, so the exclusive lock correctly prevented concurrent uploads and possible duplicates. Live process observed:

```text
374013 /opt/hermes/.venv/bin/python3 /opt/data/scripts/viral_radar_daily_upload.py
```
...
```json
{
  ...
  "status": "blocked_already_running",
  "lock": "/opt/data/HeRmEz/projects/viral-clip-radar/STATE/viral_radar_upload.lock",
  "reason": "another Viral Radar upload run is active; refusing concurrent upload to prevent duplicates"
}
```

Post-run verification confirms `UPLOAD_QUEUE` contains **0** upload manifests and `UPLOAD_QUEUE_HOLD` contains **1**. No authentication failure occurred, so OAuth recovery URLs are not required.
