...
Processed newsletter videos for `personal-secondary` / `fareed320@gmail.com`.

- Preflight: OK.
  - Gmail profile verified: `fareed320@gmail.com`
  - YouTube auth/channel guard: OK
  - TTS: ElevenLabs live, Google TTS live fallback
  - Stock visuals: Pixabay active
  - Render gate: ffmpeg/ffprobe OK

Uploaded public to Sosai/Trapiistan only:

- Plaid IPO / prediction markets / AI fintech labor
  - https://youtu.be/_uGPnpcRmG8
  - Source email trashed after verified `video_id`

- JadePuffer AI attack / Anthropic vs Alibaba / macOS bug
  - https://youtu.be/f70thu6DimE
  - Source email trashed after verified `video_id`

- Hire a near peer / AI ROI / fighting churn
  - https://youtu.be/9JMwH_ZeRJ0
  - Source email trashed after verified `video_id`

- Chrome extension rules / Meta AI cloud / AI security
  - https://youtu.be/3TeU_uMcwk4
  - Source email trashed after verified `video_id`

- Aave Monad market / Ethereum roadmap / crypto cards
  - https://youtu.be/F_YOgNykF7A
  - Source email trashed after verified `video_id`
  - Note: duration was ~151s, outside ideal Shorts range, but upload continued under the current public-upload policy.

Notes:
- No provider/auth blocker hit.
- One background retry initially used the wrong `python3` without Google deps. Re-ran with `/opt/hermes/.venv/bin/python3` and completed successfully.
- There are still many unread approved source emails remaining in `personal-secondary`, mostly TLDR/Daily Stoic.
