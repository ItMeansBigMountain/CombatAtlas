...
- **hold_queue:** `1`
- **pipeline_exit:** `2`
- **Rendered queue integrity:** Preserved — `1` upload metadata file and `1` corresponding media file remain in `UPLOAD_QUEUE`. No failed rendered clips were deleted.
- **Durable raw log:** `/opt/data/HeRmEz/projects/viral-clip-radar/OUTPUTS/backlog-processor-logs/2026-08-21T090043Z.log`

### Blockers

1. **Authentication:** The Classical Echos refresh token is expired or revoked, preventing fresh manifest seeding.
2. **Source acquisition:** All attempted reviewed YouTube sources failed the downloader ladder. `yt-dlp` encountered bot/age authentication checks; `pytubefix` reported unavailable or HTTP 400; plain `pytube` returned HTTP 400.
