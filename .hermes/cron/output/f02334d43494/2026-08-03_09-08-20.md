...
## Viral Radar backlog drain — **PARTIAL / BLOCKED**

- **Status:** `blocked_min_uploads_not_met`
- **Uploaded:** `1 / 10` required public Shorts
- **Public URL:** https://youtu.be/rbux-ZLWUsc
- **Video ID:** `rbux-ZLWUsc`
- **Creator:** Chris Williamson
- **Source:** https://www.youtube.com/watch?v=LHddeOEEWYE
- **queue_before:** `0`
- **queue_after:** `0`
- **hold_queue:** `1`
- **pipeline_exit:** `2`
- **Render failures:** `0`
- **Durable raw log:** `/opt/data/HeRmEz/projects/viral-clip-radar/OUTPUTS/backlog-processor-logs/2026-08-03T090038Z.log`

The empty active upload queue does **not** mean the backlog is complete: nine required uploads were never completed. One held item remains:

`UPLOAD_QUEUE_HOLD/alex-hormozi-8-entrepreneurs-compete-for-100-00-02-daily.mp4.upload.json`

Failed rendered clips were not deleted. The successfully uploaded MP4 was cleaned up, while its source MP4 was preserved because three associated clips remain pending.

### Blocker

Source acquisition repeatedly failed. `yt-dlp` was disabled, `pytubefix` received HTTP 403, and plain `pytube` received HTTP 400. This was **not** an upload OAuth failure, so no YouTube OAuth recovery URL is required.
...
The same source-download error pattern affected additional backlog candidates. The pipeline’s prescribed recovery is to provide authenticated YouTube cookies, a suitable proxy, local/direct source MP4s, or another rights-safe official source URL.
