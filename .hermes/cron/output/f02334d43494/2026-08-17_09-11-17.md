...
- **hold_queue:** 1
- **pipeline_exit:** 124
- **Durable log:** `OUTPUTS/backlog-processor-logs/2026-08-17T090048Z.log`
- **Failed rendered clips deleted:** No

The backlog processor exceeded the required 600-second execution limit. The newest durable log exists but is empty, so the child pipeline emitted no raw application output before termination. No authentication error was exposed, so no OAuth recovery URL is applicable.

Protected hold item remains intact:
...
**Blocker:** `blocked_timeout_no_application_output` — the pipeline stalled or buffered all output for ten minutes before the scheduler terminated it. This run is not a product success, and no public upload was verified.
