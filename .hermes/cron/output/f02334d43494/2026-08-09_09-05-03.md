...
- **queue_before:** `0`  
  Inferred from `queued_replay_uploaded_count: 0`, no uploaded batches, and no remaining queue entries.
- **queue_after:** `0` — verified after execution
- **hold_queue:** `1` — verified after execution
- **pipeline_exit:** `2`
- **Uploaded:** `0/10`
- **Public URLs:** None
- **Durable raw log:** `/opt/data/HeRmEz/projects/viral-clip-radar/OUTPUTS/backlog-processor-logs/2026-08-09T090029Z.log`
- **Failed rendered clips deleted:** No

### Blocker

Source acquisition failed across all attempted candidates:

- `48` source failures
- `2` render failures caused by missing source files
- YouTube authentication did **not** fail, so no OAuth recovery URL is required.
- The primary blocker is YouTube source downloading: `yt-dlp` was disabled, `pytubefix` returned HTTP 403/400, and `pytube` returned HTTP 400.
...
```text
source file missing for clip chris-williamson-everything-you-learned-in-school-is-a-lie-topup-auto-10: /opt/data/HeRmEz/projects/viral-clip-radar/SOURCES/tK1ctEZjF4w/source.mp4
```

Ten fresh creator plans were seeded, but none reached render/upload. The next required recovery is a working source-acquisition route—valid YouTube cookies/device-login, a non-cloud proxy, or local/official direct source media—then replay the backlog.
