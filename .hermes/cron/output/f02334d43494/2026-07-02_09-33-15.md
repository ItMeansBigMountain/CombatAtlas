...
Newsletter cron run complete.

- Metrics precheck: OK.
- Gmail profile verified: `personal-secondary` = `fareed320@gmail.com`.
- YouTube token verified: `trapiistan` / Sosai Oyama channel.
- Providers ready:
  - Google TTS OK.
  - ElevenLabs key valid but free credits exhausted, so Google TTS fallback used.
  - Stock visuals OK via Pixabay.
  - ffmpeg/ffprobe OK.

Uploaded public videos:

- `Exploitarium 0-Day Drop, Chrome Longinus Flaw, GLM 5.2 Beats Claude`
  - https://youtu.be/3csfJEZmel0
  - Source email trashed after verified `video_id`.

- `Hiring with AI, AI crash aftermath, primitive as product`
  - https://youtu.be/NgUTYNh2BBk
  - Source email trashed after verified `video_id`.

- `Fable returns, AWS bets $1B on AI deployment, Microsoft adds new Copilot coding model`
  - https://youtu.be/Ron_ImHiEQ8
  - Source email trashed after verified `video_id`.

- `UK New Crypto Framework, OpenUSD Launched, SharpLink buys more Eth`
  - https://youtu.be/EzWWb8nVR3U
  - Source email trashed after verified `video_id`.
  - Note: this one rendered long at ~456s before I hardened the duration gate.

- `Docker and Vercel, Safe Feature Flags, Discord Cost Attribution`
  - https://youtu.be/Ytl2vKdwTNo
  - Source email trashed after verified `video_id`.

- `Claude Sonnet 5, Reddit’s anti-spam internals, Expo 57`
  - https://youtu.be/NS45Dr_LLzU
  - Source email trashed after verified `video_id`.

- `Retail media's vibe shift, social media SEO, attribution models`
  - https://youtu.be/X5qLdE9DDd4
  - Source email trashed after verified `video_id`.

- `Claude Sonnet 5, Meta Kalshi talks, Fable returns`
  - https://youtu.be/8nLcJ2HC5nw
  - Source email trashed after verified `video_id`.

Safety/quality changes made:

- Updated the newsletter discovery query to only process `is:unread` approved source emails.
- Hardened the upload gate so future renders with duration outside the allowed range are blocked before upload instead of only warning.

Remaining:

- There are still at least 10 unread eligible newsletter/source emails in `personal-secondary`.
- No auth/provider blocker right now.
- I stopped after this batch to avoid continuing long unattended uploads after finding the duration-gate issue.
