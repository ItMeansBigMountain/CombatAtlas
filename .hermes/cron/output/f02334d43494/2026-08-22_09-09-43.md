...
- **hold_queue:** `1`
- **pipeline_exit:** `2`
- **Durable log:** `/opt/data/HeRmEz/projects/viral-clip-radar/OUTPUTS/backlog-processor-logs/2026-08-22T090056Z.log`
- **Failed rendered clips:** Preserved; no queue or hold-queue items were deleted.

### Blockers

1. **Classical Echos OAuth token expired or was revoked.** This blocked discovery and the queued upload.
2. **Source acquisition blocked:** attempted YouTube sources repeatedly triggered bot/age authentication checks; pytubefix reported unavailable and pytube returned HTTP 400.
3. The required minimum of 10 uploads was therefore not met.
...
```text
ERROR: [youtube] s8RSjblOel4: Sign in to confirm you’re not a bot. Use --cookies-from-browser or --cookies for the authentication.
