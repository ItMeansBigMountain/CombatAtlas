...
- Processed fareed320 / `personal-secondary`.
- Auth checked OK:
  - Gmail profile: `fareed320@gmail.com`
  - YouTube token: Sosai Oyama / Trapiistan channel `UCsxzQlusqwmMUdjMvKAJDfA`
- Provider preflight OK:
  - Google TTS live.
  - Pixabay stock active.
  - ffmpeg/ffprobe OK.
  - ElevenLabs auth OK but credits are exhausted, so the run used Google TTS.

Uploaded 12 public videos:

- `Azure CLI Password Spray, Hijacked AI Endpoints, Apple Hide My Email Bug`
  - https://youtu.be/YZ9ENtwZ3ws
  - 86s
  - source email trashed

- `GCash to raise $1.5B, UK dilutes stablecoin capital requirement, World Cup chargebacks`
  - https://youtu.be/_yjPtO_QXTY
  - 221s
  - source email trashed

- `Azure CLI Attack Hits 78+ Accounts, Meta Eyes AI Cloud, AI Race Moves Down the Stack`
  - https://youtu.be/kT8HaXLiRPE
  - 97s
  - source email trashed

- `Robinhood Chain Live, Cloudflare x402 Gateway, Crypto and Robotics`
  - https://youtu.be/2N_vMs8PXHY
  - 169s
  - source email trashed

- `Don’t do rewrites, redeploying Fable 5, Meta caps token spend`
  - https://youtu.be/AoIpOHHDttU
  - 117s
  - source email trashed

- `Influencers drive discovery, pitch deck workflow, branded subreddits`
  - https://youtu.be/wQRCl-Ngawo
  - 88s
  - source email trashed

- `SpaceX phone, OpenAI govt equity, Meta Ray Ban apps`
  - https://youtu.be/3_ZqrTdgxy4
  - 90s
  - source email trashed

- `Google’s Tabular Foundation Model, Meta’s Data Eng Agent, LLM Spark Debugger`
  - https://youtu.be/kjIP4QBWmPA
  - 78s
  - source email trashed

- `Devin Fusion, DeepSeek DSpark, economy of tokens`
  - https://youtu.be/ZiFPfJuMoTo
  - 85s
  - source email trashed

- `Linux DirtyClone Flaw, Signal Recovery Scam, Tata Hack Expose iPhone 18 Details`
  - https://youtu.be/v1BIjOJ_B10
  - 110s
  - source email trashed

- `Clarity Act Odds, Franklin Crypto, Final Boss of Cryptography`
  - https://youtu.be/ceoCjo35isI
  - 111s
  - source email trashed

- `Mythos Rivals Rise, AI Data Centers Boom, Okta Governs Agents`
  - https://youtu.be/wBohUcVxWVg
  - **7549s / about 2h05m**
  - source email trashed

Important blocker / safety stop:

- I stopped the batch manually after the pipeline uploaded the 7549s video.
- Exact blocker: the script has a disabled duration gate:
  - it only warns with `duration_outside_ideal_range_public_upload_continued`
  - then continues uploading under `faceless_public_upload_no_quality_gate`
- That conflicts with this run’s instruction: upload only after quality gates pass.
- I killed the active batch so it would not keep uploading more potentially overlong videos.

Remaining:

- There are still at least 20 eligible unread newsletter/source emails in `personal-secondary`.
- I did not continue processing them because the duration quality gate needs to be fixed first.
