...
## Viral Clip Radar backlog drain — **PARTIAL / BLOCKED**

- **Status:** `blocked_min_uploads_not_met`
- **queue_before:** 3
- **queue_after:** 0
- **hold_queue:** 0 (no `HOLD_QUEUE` directory exists)
- **pipeline_exit:** 2
- **Uploaded:** 4 public Shorts total
  - 3 queued replays
  - 1 newly processed clip
- **Required minimum:** 10
- **Deficit:** 6 uploads

### Public uploads

1. https://youtu.be/kxLSF_iQ9hs  
2. https://youtu.be/vLv0_SPzXdI  
3. https://youtu.be/xX7tbIJ0oWw  
4. https://youtu.be/lC8cz-n558Q  

### Blocker

Source acquisition failed across the remaining candidates. `yt-dlp` was disabled, while `pytubefix` and `pytube` encountered YouTube HTTP errors. This was **not an upload OAuth failure**, so no OAuth recovery URL is required.
...
"min_uploads": 10,
"uploaded_count": 4,
"queued_replay_uploaded_count": 3,
...
Additional discovery errors:

```text
TateSpeech / Andrew Tate: HttpError 404 ... playlistNotFound
Capital Club Community / Luke Belmar: HttpError 404 ... playlistNotFound
Luke Belmar: no recent long-form videos found
TOGI: no recent long-form videos found
```

The empty upload queue means all three previously rendered retries uploaded successfully; it does **not** mean the backlog/minimum is complete. The six-upload deficit consists of clips that could not be sourced/rendered, so there were no failed rendered files to delete or queue.

**Durable raw log:**  
`/opt/data/HeRmEz/projects/viral-clip-radar/OUTPUTS/backlog-processor-logs/2026-08-11T001119Z.log`
