...
- Blocked safely.
- No newsletter videos were rendered or uploaded.
- Source Gmail profile `personal-secondary` / fareed320 is not usable right now:
  - `RefreshError: invalid_grant`
  - Google says token is expired or revoked.
- Sosai/Trapiistan YouTube upload token is also blocked:
  - `/opt/data/secrets/youtube-trapiistan/youtube_upload_token.json`
  - `RefreshError: invalid_grant`
- Quality providers checked:
  - ElevenLabs: live, but free credits are exhausted.
  - Google TTS: live and usable.
  - Stock visuals: Pixabay key present and usable path ready.
  - ffmpeg/ffprobe: present.
- Metrics monitor also confirmed the same Trapiistan YouTube token problem.
- I did not trash any Gmail source emails.
- I did not upload placeholder/static/flite drafts.

Needed fix:
- Reauth `personal-secondary` Gmail.
- Reauth the Sosai/Trapiistan YouTube upload token.
- Then the newsletter pipeline can safely process one email = one public video and trash only after verified `video_id`.
