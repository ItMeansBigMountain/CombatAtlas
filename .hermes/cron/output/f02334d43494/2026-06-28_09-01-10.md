...
- blocked_auth: YouTube upload token for Sosai/Trapiistan is expired/revoked.
- I stopped safely before rendering/uploading.
- No source emails were trashed.
- No placeholder/static/flite draft was uploaded.

Preflight:
- Gmail profile `personal-secondary` works.
- Confirmed Gmail account: `fareed320@gmail.com`.
- Found `19` unread approved newsletter/source emails.
- TTS is usable:
  - ElevenLabs auth works, but free-tier credits are exhausted.
  - Google TTS works and is usable.
- Stock visuals are usable:
  - Pixabay key active.
- ffmpeg/ffprobe are installed.
- Metrics monitor also hit the same Trapiistan token issue.
