...
- **Uploaded:** `0 / 10` required
- **Public URLs:** None
- **queue_before:** `0`
- **queue_after:** `0`
- **hold_queue:** `1`
- **pipeline_exit:** `2`
- **Durable log:** `/opt/data/HeRmEz/projects/viral-clip-radar/OUTPUTS/backlog-processor-logs/2026-08-08T090018Z.log`
- **Held item preserved:** `UPLOAD_QUEUE_HOLD/alex-hormozi-8-entrepreneurs-compete-for-100-00-02-daily.mp4.upload.json`

Ten new creator plans were seeded, but no clips could be published. The primary blocker was source acquisition: `yt-dlp` was disabled, `pytubefix` received HTTP 403 responses, and plain `pytube` received HTTP 400 responses. Two render attempts also found their expected source MP4s missing.

Raw representative errors:
...
```text
source file missing for clip gg33-new-studio-01: /opt/data/HeRmEz/projects/viral-clip-radar/SOURCES/mZSVFjJoDCQ/source.mp4
...
This was a **source-readiness failure**, not an upload OAuth failure, so no OAuth recovery URL was generated. No failed rendered clips were deleted.
