...
Run the user's post-morning Agentic portfolio market scan for Robinhood Agentic account 433711041 / account ending 1041. This job runs after the Morning operator report. It is primarily research/reporting unless a separate active autonomous trading policy is clearly present in /opt/data/HeRmEz/projects/trading-journal/playbook/autonomous-policy.md AND current account/broker state is certain. Do not ask questions; final response is delivered to Discord.

Required workflow:
1. Inspect live Robinhood account state using available Robinhood MCP tools if present: portfolio/buying power, equity positions, option positions if any, open equity orders, recent orders. If MCP tools are unavailable in this cron environment, state that clearly and continue with market/news research only.
2. Inspect broad market: SPY, QQQ, IWM quotes and recent historicals when available. Summarize bullish/bearish/neutral in one line.
3. Scan candidates beyond stale user watchlists. Use: recent source/newsletter signals from routed Gmail labels when accessible (TLDR, Robinhood Snacks, Daily Stoic/Kino Body only if relevant to markets/health stocks), web/news search, Robinhood popular/daily movers if tools are available, and live quotes/historicals/tradability.
4. Produce technical analysis for top candidates: price, daily move, trend vs recent moving averages if available, volume/ATR if available, support/resistance/invalidation, setup quality.
5. Produce lightweight fundamental/catalyst analysis: what changed, earnings/news/sector narrative, why it matters, and what would disconfirm it.
6. Update the local trading journal under /opt/data/HeRmEz/projects/trading-journal/YYYY-MM-DD/post-morning-market-scan.md with the account state, candidate list, best setup(s), and any tool failures/upgrades needed.
7. Include a small 'Tool / system upgrades' section: what data source, scanner, script, or MCP/tool improvement would make future scans better. Do not modify tools unless it is a harmless local journal/runbook update.
8. If no clean setup exists, say no trade; do not force a trade.
9. Never trade options unless explicitly authorized. Never trade non-Agentic accounts. Stop all trading consideration if account value is below $10, broker state is uncertain, or risk cannot be calculated.
10. If an autonomous policy is active and a trade is placed within policy, use review before placement when possible, journal the preview and execution, and report the exact order/result. Otherwise, do NOT place orders; only provide setups.

Output style: concise Discord bullets, no Markdown tables. Sections: Decision, Agentic Portfolio, Market Read, Top Candidates, Best Setup, Risk / Invalidation, Tool Upgrades, Journal.
...
- No trade placed.
- Autonomous policy is active and Agentic account ••••1041 is above the $10 kill switch, but fresh market/broker timing is not certain enough for execution: Robinhood quotes were latest from 2026-06-18 regular/after-hours, not fresh current-session prints.
- Best action: hold current HOOD/NVDA positions; watch TSM and SMCI for clean retest/confirmation rather than chase extended semiconductor strength.
...
- Recent orders:
  - HOOD: filled agentic buy, $50 on 2026-06-12, avg ~$93.32
  - NVDA: filled agentic buy, $40 on 2026-06-15, avg $210.36
- Current positions:
  - HOOD: 0.535786 shares, avg $93.32, last usable after-hours quote ~$108.00; est P/L +$7.87 / +15.7%
  - NVDA: 0.190150 shares, avg $210.36, last usable after-hours quote ~$210.33; est P/L about flat
- Risk budget from current value:
  - Target per-trade risk: ~$2.08
  - Target aggregate planned risk: ~$6.24

# Market Read

- Broad read: bullish/constructive for tech, but not actionable for fresh autonomous orders until fresh session data is confirmed.
- SPY: ~$746.93 after-hours, +~1.1% vs adjusted prior close; recovered but below early-June highs.
- QQQ: ~$739.83 after-hours, +~2.4%; strongest major ETF, AI/semis leading.
- IWM: ~$295.16 after-hours, +~1.8%; constructive, but less clean than QQQ.
- Web/news scan: semiconductor/AI strength was the main confirmed theme; chip names rallied after Asia strength, though earlier June semis volatility remains a warning.

# Top Candidates

- TSM
  - After-hours quote: ~$462.00, +~6.9% vs prior close.
  - Technicals: broke above recent $442–$450 resistance on strong volume; support now $438–$442, resistance/new-high area ~$465.
  - Catalyst: AI/semiconductor strength and Asia chip rally.
  - Setup quality: best clean trend candidate, but extended after a large move.

- SMCI
  - After-hours quote: ~$30.72, +~10.6% vs prior close.
  - Technicals: high-volume rebound from a sharp selloff; support $28.20–$29.00, resistance $31.80–$32.10 then ~$36.50.
  - Catalyst: semiconductor rebound / AI infrastructure sympathy.
  - Setup quality: liquid and sandbox-sized, but speculative and volatile.

- AVGO
  - After-hours quote: ~$411.46, +~4.7%.
  - Technicals: reclaiming $405–$410 after prior weakness; support $405 then $392, resistance ~$426.
  - Catalyst: chip-sector rebound, but prior AI-revenue disappointment/headline risk remains.
  - Setup quality: watch only; less clean than TSM.

- AMD
  - After-hours quote: ~$536.99, +~4.8%.
  - Technicals: near $539–$548 resistance; support $526 then $507.
  - Setup quality: strong, but extended and high per-share volatility makes $2-risk sizing tight.

- MU
  - After-hours quote: ~$1151.80, +~10.4%.
  - Technicals: momentum leader but very extended.
  - Setup quality: poor sandbox fit; chase risk too high.

- Existing HOOD
  - Still the portfolio winner; support ~$103.46 then ~$95.76, resistance ~$110.73.
  - No add unless it retests/holds cleanly or breaks out with fresh confirmation.

- Existing NVDA
  - Around cost basis; support ~$203–$206, resistance ~$212.7 then ~$224.
  - Hold/watch; no add while already exposed to AI/semis and fresh data is stale.

# Best Setup

- Best watchlist setup: TSM long on retest, not immediate buy.
  - Trigger: hold/retest $442–$450 with fresh volume, or break/reclaim above ~$465 after market reopens.
  - Stop/invalidation: sustained break below ~$438.
  - Target 1: ~$480
  - Target 2: ~$500 if sector strength continues.
  - Starter size if confirmed: $25–$40 fractional equity, keeping max planned loss near ~$2.
  - Why no order: quote freshness/market session state uncertain and entry is extended.

- Speculative secondary setup: SMCI only on confirmation.
  - Trigger: hold $29–$30 and reclaim ~$31 on volume.
  - Stop/invalidation: below ~$28.20.
  - Target 1: ~$32
  - Target 2: ~$36.50
  - Why no order: rebound is interesting but volatile; needs confirmation.

# Risk / Invalidation

- Stop all trading consideration if account value falls below $10, broker state is uncertain, or risk cannot be calculated.
- Today’s execution blocker: quote/session freshness uncertainty.
- Do not add to HOOD/NVDA unless a fresh setup improves risk/reward.
- Do not chase MU/TSM/SMCI gaps without retest or confirmed continuation.
- Options remain disabled; no options scanned for trades.

# Tool Upgrades

- Restore Google Workspace auth for the cron profile or point the job to the correct profile token; Gmail scan failed with `NOT_AUTHENTICATED: No token at /opt/data/google_token.json`.
- Add a market-hours/session freshness guard so the job explicitly labels data as regular, after-hours, holiday, or stale before any autonomous review/place action.
- Add a local scanner script for 5/10/20-day MAs, ATR%, relative volume, gap%, support/resistance, and risk sizing from Robinhood historicals.
- Cache Robinhood Daily Movers + web headline candidates so recurring names can be tracked across scans.

# Journal

- Updated local journal:
  - `/opt/data/HeRmEz/projects/trading-journal/2026-06-19/post-morning-market-scan.md`
