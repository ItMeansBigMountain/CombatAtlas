...
**Decision**
- No new trade placed.
- Reason: Agentic account is already ~72% deployed (inside the 70%–90% target), current holdings are green, and today’s best movers are extended gap names that need retest/news confirmation before entry.
- Autonomous policy is active, but “no clean immediate setup” means no order review/placement.

**Agentic Portfolio**
- Account: Robinhood Agentic ending **1041**.
- Portfolio value: **$194.66**.
- Equity value: **$140.92**.
- Cash / buying power: **$53.74**.
- Positions:
  - **SOFI**: 4.477580 shares, avg $17.87; live mark about $18.105, approx **+$1.05 / +1.32% unrealized**.
  - **AMD**: 0.115059 shares, avg $521.47; live mark about $527.04, approx **+$0.64 / +1.07% unrealized**.
- Options: none.
- Open equity orders: none found across new / queued / confirmed / unconfirmed / partially-filled states.
- Recent agentic activity: SOFI buy filled 6/26; AMD buy filled 6/25; NVDA and HOOD exits last week.

**Market Read**
- **SPY +1.30%, QQQ +1.35%, IWM -0.75%** at scan time.
- Read: bullish for large-cap / tech, but small-cap weakness makes the tape selective rather than fully risk-on.

**Top Candidates**
- **IRDM**: ~**+21.8%** day. Strongest gap/momentum name from Daily Movers; fractional-tradable. Web checks showed halt/news-pending and extended-strength references, but catalyst still needs confirmation. Too extended to chase here.
- **CHTR**: ~**+15.3%** day. Gap leader; web checks mention possible SpaceX/mobile collaboration and insider-buy interest. Needs VWAP/opening-range retest before entry.
- **CMCSA**: ~**+10.0%** day. Liquid and reclaiming short-term levels, but prior technical context was weak and catalyst quality was mixed.
- **SOFI**: current holding, ~**+1.3%** day. Still constructive above recent support; hold rather than add.
- **AMD**: current holding, ~**+1.0%** day. Semis/AI still participating, but AMD remains volatile; hold rather than add.
- Rejected:
  - **RPAY**: +19.7%, but under $5 / lower liquidity profile.
  - **BLD**: -10.2%, breakdown not a long setup.

**Best Setup**
- Best actionable idea is **watch-only**:
  - **CHTR** retest setup: wait for hold/reclaim around **$147–$150** with volume support; target zone roughly **$160–$165**; invalidate on failed retest.
  - **IRDM** retest setup: wait for confirmed catalyst and hold above roughly **$50–$51**; target roughly **$56–$58**; invalidate below failed retest.
- No immediate entry because both are post-gap extended.

**Risk / Invalidation**
- Kill switch not triggered: account value is well above $10.
- SOFI: hold while above roughly **$17.35–$17.00**; review exit if it loses that zone.
- AMD: hold while above roughly **$500–$505**; review exit if semiconductor strength fades or that support breaks.
- No options, no shorts, no non-Agentic account activity.

**Tool Upgrades**
- Add a compact Daily Movers scanner that computes SMA10/SMA20/ATR/20-day range automatically from Robinhood quotes/historicals.
- Add a cleaner catalyst/news feed for mover names; web search was noisy and not always definitive.
- Fix cron path assumption: working Google API script path is `/opt/data/skills/productivity/google-workspace/scripts/google_api.py`; `/opt/data/HeRmEz/skills/...` did not exist.
- Gmail probe worked for `personal-main`, but no TLDR/Robinhood Snacks matches were found in the last 3 days; Calendar/Drive scopes remain insufficient and should be reported separately.

**Journal**
- Updated: `/opt/data/HeRmEz/projects/trading-journal/2026-06-29/post-morning-market-scan.md`
