...
**Decision**
- No new trade.
- Account/broker state was available and the autonomous policy is active, but the cleanest movers are too extended to chase.
- Current equity deployment is already ~72.9%, inside the 70%–90% target range, so best action is **hold AMD/SOFI and wait for retests**.
...
- QQQ: ~$732.96, **+1.23%**
- IWM: ~$300.02, **+0.35%**
- Read: **Constructive/bullish, led by QQQ/AI beta, but gap leaders are extended and need discipline.**

**Top Candidates**
- **AMD**: +3.44% today, above recent short-term average; strongest existing holding. AI-chip momentum supports it, but it is extended after the move. Prefer hold, not add.
- **SOFI**: -0.91% today but still above recent short-term average; constructive digestion. Hold while it stays above roughly $17.50–$17.70.
- **AVAV**: +21.71% after strong earnings / record-revenue catalyst. Catalyst is real, but current price is far above recent average; no chase. Watch for retest near $150–$155 or consolidation above ~$160.
- **FCEL**: +24.63%, very extended/speculative. Momentum exists, but stop/invalidation is too messy for the sandbox.
- **HOOD**: +0.27%, liquid and constructive, but no fresh edge versus existing SOFI/AMD exposure.
- **NVDA**: +1.59%, AI bounce, but AMD has cleaner relative strength today.
- **MAMA / CNXC**: large negative moves; avoid broken momentum for this long-only sandbox.

**Best Setup**
- Best setup is **no new entry / hold existing positions**.
- AMD:
  - Hold above ~$540 intraday / ~$532–$535 swing support.
  - Target zone: ~$565–$575 if QQQ/AI strength persists.
  - Invalidation: failed breakout and close back below ~$532 or negative AI-chip reversal.
- SOFI:
  - Hold above ~$17.50–$17.70.
  - Target zone: ~$18.70–$19.25.
  - Invalidation: close below ~$17.30 or broad fintech/small-cap weakness.
- AVAV watch-only:
  - No chase at ~$169.
  - Cleaner entry would be a pullback/retest near $150–$155, or tight consolidation above ~$160 with volume confirmation.
  - Invalidation: fade back below ~$150 or full earnings-gap retrace.

**Risk / Invalidation**
- Kill switch not triggered: account value **$198.62 > $10**.
- Risk state was calculable, but no new order was placed because current candidates lacked clean entry/stop math at live prices.
- Do not add to AMD after a sharp move unless it retests and holds.
- Do not chase AVAV/FCEL gap moves without a better base.

**Tool Upgrades**
- Add a compact scanner script for Robinhood Daily Movers + popular lists that filters OTC/low-liquidity names and outputs SMA10/SMA20/ATR14/20-day range in small JSON.
- Add exact Gmail label probes for routed finance/newsletter labels instead of only broad sender search.
- Standardize Google Workspace cron path discovery: expected `/opt/data/HeRmEz/skills/...` was missing; actual script was under `/opt/data/hermes-agent/skills/...`.
- Add a journal helper that auto-writes account snapshot, open-order matrix, candidate metrics, and no-trade reason.

**Journal**
- Updated local journal:
  - `/opt/data/HeRmEz/projects/trading-journal/2026-06-30/post-morning-market-scan.md`
