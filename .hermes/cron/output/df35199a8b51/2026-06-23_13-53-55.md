...
# Decision
- No new trade placed.
- Autonomous policy is active for Agentic 1041, but today’s setup does **not** justify adding risk: QQQ/tech are sharply weak, existing equity deployment is already ~70%, and current positions are correlated to the selloff.
- Best action: hold/manage HOOD + NVDA; keep $60 cash for confirmed retests/reclaims rather than forcing deployment.
...
- Deployment: ~`70.3%` equities / ~`29.7%` cash
- Options: none
- Positions:
  - HOOD: `0.993769` shares, avg `$100.63`, live about `$104.14`; roughly `+3.5%` vs avg.
  - NVDA: `0.190150` shares, avg `$210.36`, live about `$203.04`; roughly `-3.5%` vs avg.
- Recent order:
  - `2026-06-22`: agentic `$50` HOOD market buy filled at avg `$109.1742`.
- Open orders: none surfaced in the recent order query.

# Market Read
- Bearish / risk-off, led by tech.
- SPY: about `$736.69`, down ~`1.0%` vs prior close.
- QQQ: about `$718.62`, down ~`2.6%` vs prior close.
- IWM: about `$295.17`, down ~`1.0%` vs prior close.
- Web/news check: CNBC result described a tech rout/global selloff; NVDA/SOXX weak. This argues against adding AI/semiconductor exposure immediately.

# Top Candidates
- HOOD
  - Live about `$104.14`, down ~`1.5%`.
  - Strong recent trend: prior close `$105.71` vs 10d `$95.45` / 20d `$89.51`.
  - Support/invalidation: `$100–$101`.
  - Resistance: `$109–$112`.
  - Catalyst: AI/trading features and app/product narrative; disconfirmed if it loses `$100` or fintech risk-off accelerates.
  - Already held; no add while below yesterday’s impulse/fill zone.

- NVDA
  - Live about `$203.04`, down ~`2.7%`.
  - Prior close `$208.65`; near 10d `$207.12`, below 20d `$211.25`.
  - Support/invalidation: `$199–$200`.
  - Catalyst: still a core AI infrastructure leader; BofA/Yahoo result highlighted long-term chip favorites including NVDA/AVGO.
  - Today’s tape is too weak to add.

- PLTR
  - Live about `$119.82`, slightly green while QQQ is weak, but still near recent lows.
  - Needs reclaim of `$126–$130` before a clean long setup.
  - Invalidation: below `$119`.
  - Catalyst: AI/software deal and upgrade narratives; valuation/risk-off remain disconfirming factors.

- AVGO / AMD
  - Long-term AI infrastructure themes remain, but both are too volatile today.
  - AVGO is below its 20d; AMD is down sharply after a strong run.
  - Watch only; no chase/catch.

- ZETA / ARQQ / BLZE from Robinhood Daily Movers
  - ZETA is the cleanest of that group technically, but catalyst confidence is thin.
  - ARQQ/BLZE are too jumpy for the sandbox without stronger confirmation.
  - AMC/IONR fail quality/price/liquidity preference.

# Best Setup
- No active entry now.
- Conditional watchlist:
  - HOOD add only if it reclaims/holds `$106–$107` with market stabilization; invalidation below `$100–$101`; target `$112`.
  - NVDA add only if QQQ/SOXX stabilize and NVDA reclaims `$208–$210`; invalidation below `$199–$200`; target `$218–$220`.
  - PLTR starter only if it reclaims `$126–$130`; invalidation below `$119`; target `$140`.

# Risk / Invalidation
- Trading kill switch not triggered: account value is above `$10`.
- Broker/account state was clear enough for research, but market/risk state is not clean enough for a new entry.
- Current portfolio is already at the lower end of the target deployment range.
- Do not add to HOOD/NVDA while QQQ remains under heavy pressure.
- Do not move risk farther away; if HOOD loses `$100` or NVDA loses `$199–$200`, next scan should reassess exit/trim.

# Tool Upgrades
- Build a local Daily Movers scanner that ranks: quote, daily %, spread %, 20d avg volume, ATR %, MA position, fractional tradability, and catalyst notes.
- Add a profile-scoped Gmail collector for TLDR / Robinhood Snacks / market newsletters; default `google_api.py` token was not authenticated, but the `personal-main` profile token worked.
- Add sector ETF checks for SOXX/SMH/XLF/ARKK when managing NVDA/HOOD.
- Add explicit open-order state queries instead of relying only on recent order history.

# Journal
- Updated: `/opt/data/HeRmEz/projects/trading-journal/2026-06-23/post-morning-market-scan.md`
