...
Pitfall: only assign tasks to profiles that exist in `hermes profile list` / `hermes kanban assignees`. On small setups the only valid assignee may be `default`; invented specialist names leave cards sitting in `ready`.
...
- **Discord gateway config inventory/probes**: See `references/discord-gateway-env.md` for the verified Discord setup workflow, root-vs-`hermes` gateway launch pattern, passwordless sudo option, and a source-grounded inventory of Discord config/env knobs.
- **Discord full-control/private-server setup**: For a private single-user server, configure `/opt/data/.env` (or `hermes config env-path`) with `DISCORD_BOT_TOKEN=<token>` and `GATEWAY_ALLOW_ALL_USERS=true`; in config set `discord.require_mention: false` for free-response, `discord.server_actions: ''` to expose all available Discord admin actions, and include `discord` + `discord_admin` in `toolsets` when using an explicit toolset list. Invite the bot with Administrator permission if the user explicitly wants total server control. Still recommend disabling `@everyone`/role mentions via `discord.allow_mentions.everyone: false` and `roles: false` unless they ask otherwise. For the user's preferred natural Discord chat UX, disable auto-threading (`discord.auto_thread: false`) and reply references (`discord.reply_to_mode: off`) before restarting the gateway.
